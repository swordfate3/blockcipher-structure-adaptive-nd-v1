...
\end{minipage} & \begin{minipage}[t]{0.09\columnwidth}\raggedright
原始回收并本地重裁决，gate=\texttt{pass}；只读复用 K1-CA 缓存\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.09\columnwidth}\raggedright
K1-BV\strut
\end{minipage} & \begin{minipage}[t]{0.09\columnwidth}\raggedright
...
\textbf{表 3 uKNIT-BC r5 在 262144/class 冻结项目协议下的五模型比较}
...
K1-CA/K1-CB 均从远程运行根 fallback
回收原始产物，随后通过清单校验、计划一致性验证和本地重裁决，并非从完整验证结果分支回收。K1-CA
恰好创建四份训练/验证缓存，K1-CB 对这四份缓存完成 12
次参数匹配复用且零新建，因此五模型比较没有因重复生成数据而引入额外数据差异。K1-BS/K1-BZ
的 2048/class
...
已在本地完成规范验证与 gate；K1-U、K1-CA、K1-CB 和 DMC2
从远程运行根 fallback
回收后完成了清单校验、协议验证和本地重裁决，但不是完整验证结果分支回收；K1-BV
和 DMC1
...
uKNIT K1-CA\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\nolinkurl{docs/experiments/innovation1-uknit-r5-k1ca-invariant-autond-paper-closeout-plan.md}；\nolinkurl{outputs/remote_results_incomplete/i1_uknit_r5_k1ca_invariant_autond_262144_s3s4_20260803/}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
远程完成、fallback 原始回收并本地重裁决，gate=\texttt{pass}；四份缓存创建和四次复用符合计划\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
uKNIT K1-CB\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\nolinkurl{docs/experiments/innovation1-uknit-r5-k1cb-published-network-paper-comparison-plan.md}；\nolinkurl{outputs/remote_results_incomplete/i1_uknit_r5_k1cb_published_comparison_262144_s3s4_20260803/}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
远程完成、fallback 原始回收并本地重裁决，gate=\texttt{pass}；12 次 K1-CA 缓存复用、零新建；公开架构适配而非精确复现\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
uKNIT K1-BV\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
