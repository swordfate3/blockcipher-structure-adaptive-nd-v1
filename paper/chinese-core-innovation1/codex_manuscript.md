# 面向异构 SPN 分组密码的运行时结构先验神经区分方法

> 稿件状态说明：本文为中文核心期刊投稿风格初稿，实验数据截至 2026 年 8 月 2 日。为避免把训练完成误写为证据闭环，正文区分“本地完整闭环”“远程 gate 已完成但归档待闭环”和“协议无效”三类状态。K1-BT 因同一 run id 对应两套冲突产物且缓存复用契约被破坏，已从结果主表撤下。投稿定稿时应在补齐远程归档、正式规模与参考文献后删除本说明，并按目标期刊体例调整篇幅。

## 摘要

针对异构代换—置换网络（substitution-permutation network，SPN）中非连续状态单元、逐轮非线性原语以及多线性层拓扑难以被通用神经区分器显式利用的问题，提出一种运行时结构先验神经区分方法。该方法将公开密码结构编码为可执行说明书，包括比特到原生单元的映射、单元内比特角色、S 盒真值语义、逐轮 GF(2) 线性连接关系和观察窗口；在统一的结构消费接口下，分别构造适用于 uKNIT-BC 的五阶段原生单元位置直方图专家和适用于 Dialga-128 的 Runtime-E4 拓扑专家。本文通过通用 AutoND/DBitNet、Zhang/Wang MCND 风格网络、Liu 等 Case-3 Conv2D-ResNet 风格网络、错误 S 盒、扰动拓扑、无拓扑及同一冻结检查点替换说明书等控制，考察性能增益是否依赖正确结构。在 uKNIT-BC 5 轮、16 密文对、每类 2048 个训练样本的本地 K1-BS/K1-BZ 诊断中，结构专家在两个随机种子上的 AUC 为 0.902802/0.932539，AutoND/DBitNet 为 0.511321/0.526423，MCND 适配器为 0.493508/0.493233，raw Case-3 Conv2D 适配器为 0.526742/0.505930；两种新增公开架构适配器均未通过双种子晋级门。在 Dialga-128 4 轮的本地 D1、D2 实验中，D2 在同一冻结检查点下仅替换运行时说明书，正确拓扑 AUC 为 0.958417/0.958679，扰动拓扑为 0.925256/0.918380，无拓扑为 0.517403/0.526351。Dialga DMC1 中等规模远程 gate 支持 4 轮拓扑效应，但其归档与本地重裁决尚未闭环。结果支持在特定密码、轮数、差分和小样本协议下进一步研究结构说明书与差异化专家；当前证据不支持一个共享权重网络适配任意 SPN，不代表对 Zhang/Wang、Liu 或 AutoND 原论文协议的复现，也不构成公开最佳攻击轮数的突破。
...
AutoND/DBitNet 一类方法试图通过统一输入编码和自动化网络构造降低手工设计成本[4]。该类模型适合作为“没有显式消费目标密码运行时结构”的通用基线，但通用性不等于在每个密码上均达到最强效果。本文不把 AutoND 的中等规模诊断等同于其公开论文的大规模训练协议；特别地，本文每模型、每种子的 65536/class 训练总量为 131072 行，明显小于公开代码常见的千万级总训练样本。因而本文的比较用于回答同预算下的架构与结构归因问题，不用于判定 AutoND 的论文规模能力上限。
...
为防止远程训练完成被误写为论文证据闭环，本文使用表 1 的三级状态。这里的“gate”指依据预先记录的阈值、必要控制和计划一致性对结果作出的研究裁决，不等同于训练脚本正常退出。
...
| 远程 gate 已完成，归档待闭环 | 远程验证和研究 gate 已完成，但最终归档、本地重裁决或索引不完整 | uKNIT K1-BV、Dialga DMC1 |
| 协议无效 | 产物来源、缓存、配置或 gate 的必要条件发生冲突；指标不得用于结论 | uKNIT K1-BT |

该分级直接限制可写结论：本地闭环结果可用于当前协议下的机制与边界判断；远程 gate 结果只能作为预备证据并显式说明归档缺口；协议无效结果即使数值有利也必须撤出论文结果表，不能择取其中一套产物作趋势证据。
...
全部实验均针对公开算法的离线约化轮区分。优化器为 Adam，损失函数为均方误差，训练 10 个 epoch；每项比较冻结设备、数据协议、负样本定义与优化预算，不在同预算比较中途切换设备。表中 `samples/class` 表示每类样本数，总训练行数为其两倍。K1-BS/K1-BZ 的 2048/class 仅为本地小样本架构诊断；DMC1 的 65536/class 对应每模型、每 seed 共 131072 个训练样本，也只属于中等规模确认，不称为正式规模或论文规模。除报告 accuracy 外，裁决优先使用不依赖固定阈值的 AUC。
...
K1-BV 使用 uKNIT-BC r6，训练 2048/class，跨密钥验证 1024/class，seed 为 3、4。比较 exact 4-pair、exact 16-pair 与 wrong-S-box 16-pair。该实验是远程小规模边界诊断，不用于估计 r6 的方法上限。

### 5.3 Dialga 协议

D1 使用 Dialga-128 prefix-r4，4 pairs/sample，训练 2048/class，验证 1024/class，seed 为 0、1，分别训练正确拓扑、扰动拓扑和无拓扑模型。D2 复用 D1 正确拓扑检查点及相同验证协议，只替换推理说明书。DMC1 保持 prefix-r4 和 4 pairs/sample，将训练规模提高到 65536/class、验证提高到 16384/class，比较正确拓扑、扰动拓扑和 AutoND/DBitNet。Runtime-E4 有 442466 个参数，AutoND/DBitNet 有 797633 个参数；两者数据和优化预算一致，但并非参数量匹配。D3 使用 prefix-r5、2048/class、4 pairs/sample、seed 0/1，检查相邻轮数窗口能否复制 r4 结果。
...
### 6.2 uKNIT K1-BV：6 轮密文对放大未获支持

K1-BV 的主要裁决依据是 AUC，而非阈值相关的 accuracy。由表 4 可见，seed 3 从 exact 4-pair 增至 exact 16-pair 的 AUC 增量为 0.028973，但 wrong-S-box 16-pair 反而比 exact 16-pair 高 0.001780；seed 4 的 pair 增量仅为 0.001619，exact 16-pair 比 wrong-S-box 低 0.006320。两个 seed 均未形成“增加 pair 后正确语义稳定优于错误语义”的必要组合。

**表 4  uKNIT-BC r6 K1-BV 的 AUC 与 gate 派生量**
...
作为补充，三种条件的 accuracy 分别为：seed 3 下 0.498535、0.508789、0.500000；seed 4 下 0.500977、0.500977、0.501953。图 2 展示该阈值预测边界，但不替代 AUC gate。

![图 2 uKNIT K1-BV 的 accuracy 边界结果](figures/fig_uknit_k1bv_boundary.svg)

**图 2  uKNIT K1-BV 的 accuracy 边界结果（裁决以表 4 的 AUC 为主）**
...
### 6.3 Dialga D1：4 轮拓扑控制

D1 已完成本地结果、验证和 gate 闭环。表 5 中，正确拓扑在两个 seed 上均约为 0.9585，分别高于扰动拓扑 0.022107 和 0.020863；无拓扑结果接近随机，正确拓扑优势超过 0.455。扰动拓扑仍保留较高 AUC，说明 D1 信号并非全部来自精确接线，基础输入统计与部分结构可能已经提供区分信息；但正确拓扑在两个 seed 上方向一致的增量表明精确连接仍有贡献。

**表 5  Dialga-128 prefix-r4 D1 本地闭环结果**
...
### 6.4 Dialga D2：同一冻结检查点的反事实说明书替换

D2 不重新训练模型，只在 D1 正确拓扑检查点上更换推理时结构说明书。表 6 显示，正确说明书的 AUC 保持为 0.958417/0.958679，扰动说明书下降至 0.925256/0.918380，无拓扑下降至 0.517403/0.526351。相对于扰动拓扑，正确拓扑优势为 0.033161 和 0.040299；相对于无拓扑，优势为 0.441014 和 0.432328。

**表 6  Dialga D2 同检查点结构替换结果**
...
![图 3 Dialga D2 同一检查点下的结构说明书替换结果](figures/fig_dialga_d2_same_checkpoint.svg)

**图 3  Dialga D2 同一检查点下的结构说明书替换结果**
...
### 6.5 Dialga DMC1：4 轮中等规模预备证据

DMC1 将训练规模提高到 65536/class。正确拓扑 AUC 为 0.978121/0.978711，扰动拓扑为 0.964932/0.964382，正确拓扑增量为 0.013189/0.014329；AutoND/DBitNet 为 0.507461/0.500494。两个 seed 的方向一致，且远程 gate 给出 `pass`，决策为 `innovation1_dialga_dmc1_medium_topology_supported`。

**表 7  Dialga-128 prefix-r4 DMC1 中等规模结果**

| 条件 | seed 0 AUC | seed 1 AUC | seed 0 accuracy | seed 1 accuracy |
|---|---:|---:|---:|---:|
| 正确运行时拓扑 | 0.978120916 | 0.978711151 | 0.912872314 | 0.929199219 |
| 扰动拓扑 | 0.964931685 | 0.964382136 | 0.900024414 | 0.902465820 |
| AutoND/DBitNet | 0.507460726 | 0.500493946 | 0.504943848 | 0.499114990 |
| 正确−扰动 | +0.013189230 | +0.014329014 | +0.012847900 | +0.026733399 |
| 正确−AutoND | +0.470660189 | +0.478217205 | +0.407928466 | +0.430084229 |

![图 4 Dialga DMC1 两个随机种子下的 AUC 对比](figures/fig_dialga_dmc1_auc.svg)

**图 4  Dialga DMC1 两个随机种子下的 AUC 对比**

该结果在远程已完成验证和研究 gate，但最终打包因归档 `logs` 路径缺失而失败，本地规范归档、重裁决和结果索引尚未闭环。因此本文把 DMC1 记为“远程 gate 已完成、归档待闭环”的预备证据，而不是本地完整闭环结果。还应注意，Runtime-E4 参数少于本实验的 AutoND/DBitNet，但二者并未进行严格参数量匹配，不能仅依据表 7 分解出结构、容量和优化难度各自的独立效应。

### 6.6 Dialga D3：相邻 5 轮窗口未复制

D3 已在本地完成 gate，状态为 `hold`，决策为 `innovation1_dialga_runtime_e4_d3_adjacent_window_not_replicated`。表 8 中，正确拓扑在 seed 0 上略高于扰动拓扑 0.002948，却低于无拓扑 0.036830；seed 1 上同时低于扰动和无拓扑。方向不一致且整体接近随机，不能支持 prefix-r5 的拓扑效应。

**表 8  Dialga-128 prefix-r5 D3 本地边界结果**
...
AutoND/DBitNet 回答“同数据和同优化预算下，不显式使用目标运行时结构的通用网络表现如何”；MCND 与 Case-3 Conv2D 适配器回答“其他公开网络几何在同一 uKNIT 原始可观测量上是否恢复信号”；wrong-S-box、扰动拓扑和无拓扑则回答“结构专家的增益是否来自正确语义”。三类基线作用不同，不能相互替代。K1-BZ 排除了“只与 AutoND 比较”的单一基线问题，却没有完成参数量匹配或原论文超参数搜索；DMC1 同时包含扰动拓扑，且 D2 又在同一检查点上完成说明书替换，其机制链相对更完整。

### 7.3 成功窗口与失败边界应同时报告

K1-BS 在 r5 uKNIT 的小样本协议中显示可学习信号，D1/D2/DMC1 则在 prefix-r4 Dialga 中提供不同等级的拓扑证据；K1-BZ 表明把另外两种公开网络骨干直接迁移到同一 raw-observable 协议并不能稳定复制该信号。K1-BV 和 D3 进一步说明，增加 pair 或沿用相同网络并不能机械拓展一轮。失败边界有助于防止选择性报告，也提示下一步应优先审计差分传播、表示与观察窗口，而不是直接增加训练样本。对 uKNIT，只有新的表示归因设计或协议有效的中等规模修复结果才能解锁下一阶段；对 D3，应检查 prefix-r5 的输入差分、拓扑窗口和标签协议是否仍与 D1 的归因假设一致。
...
第一，证据闭环尚不一致。K1-BS、K1-BZ、D1、D2、D3 已在本地完成规范验证与 gate；K1-BV 和 DMC1 虽有远程 gate，但归档和本地重裁决未完成。K1-BT 已被来源冲突和缓存契约破坏判为协议无效，不能通过补写说明恢复其指标；只有使用新 run id、干净产物根和正确缓存复用的独立修复运行才可能重新建立中等规模 uKNIT 证据。

第二，当前样本规模不足以支持正式规模主张。K1-BS/K1-BZ 只有 2048/class；DMC1 的 65536/class 也只是中等规模，且均只有两个 seed。它们远低于 AutoND/DBitNet 公开代码中每轮 \(10^7\) 总训练行、\(10^6\) 总验证行等量级，因而不能据此宣称通用模型或新增公开架构失效、达到性能上限。若用于投稿主结果，应至少补充协议有效的更高规模、多 seed、独立最终测试集，并明确训练、验证、最终评估的总量与每类数量。

第三，比较并非完全容量匹配。K1-BS/K1-BZ 中结构专家、AutoND/DBitNet、MCND 和 Liu Conv2D 适配器的参数量分别为 214316、985985、650177 和 130945；DMC1 中 Runtime-E4 与 AutoND/DBitNet 分别为 442466 和 797633。相同训练预算不能消除容量、归纳偏置与优化难度差异。更严格的实验应加入参数量近似的无结构控制或等容量通用基线。
...
第五，K1-BZ 只进行了冻结超参数下的架构适配，没有复现原论文的数据构造、规模和超参数搜索。尤其 Liu 行不含逆轮表示，不能代表其完整 Case-3 框架；MCND 行也不代表其 PRESENT 原协议。uKNIT r5 仍缺少协议有效的中等规模 wrong-S-box 和同检查点反事实控制，语义归因弱于 Dialga D2。

第六，本文尚未系统报告训练时间、推理吞吐、显存占用、置信区间和校准误差。两个 seed 只能说明初步重复性，不能提供充分的方差估计。

## 9 结论

本文提出一种面向异构 SPN 的运行时结构先验神经区分方法：把公开算法中的 cell、比特角色、S 盒语义和 GF(2) 拓扑编码为可执行说明书，再由与结构机制相匹配的专家进行消费。uKNIT K1-BS 的本地小样本闭环表明，五阶段原生单元位置直方图专家在冻结 r5 协议下优于三个通用架构；K1-BZ 进一步加入 Zhang/Wang MCND 与 Liu raw Case-3 Conv2D 骨干，但两者均未通过双种子晋级门。Dialga D1、D2 的本地闭环结果表明，正确拓扑不仅优于扰动和无拓扑，而且在同一冻结检查点下直接影响预测；DMC1 的远程 gate 为中等规模 r4 拓扑效应提供了待归档的预备支持。uKNIT K1-BV 与 Dialga D3 则分别关闭了当前配置下的 pair 机械放大和相邻轮直接延伸路线。

因此，现有证据支持把“统一结构说明书与控制协议、针对机制采用差异化专家”作为继续验证的研究路线，但不支持一个共享权重网络自动适配任意 SPN，也不支持攻击轮数突破。后续应先完成 K1-BV、DMC1 的本地归档闭环；uKNIT 中等规模证据必须以新 run id 独立修复，并加入同规模错误 S 盒或更强反事实控制。K1-BZ 已关闭两种 raw 架构适配器的直接扩样路线，若重访 Liu 路线，应把逆轮表示、raw 骨干和扰动算子作为新的预注册归因实验，而不是对现有两行做事后调参。
