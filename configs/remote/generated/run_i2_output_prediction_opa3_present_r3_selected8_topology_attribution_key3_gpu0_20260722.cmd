...
set RUN_ID=i2_output_prediction_opa3_present_r3_selected8_topology_attribution_key3_gpu0_20260722
set REMOTE_DIR=i2_opa3_ptopo_k3_20260722
set ARCHIVE_NAME=i2_opa3_ptopo_k3_20260722
...
set OPA2_GATE=%RUNS_ROOT%\i2_opa2_pspn_k3_20260722\results\gate.json
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_opa3_present_r3_selected8_topology_attribution_key3.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-opa3-present-r3-topology-attribution-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_opa3_present_r3_selected8_topology_attribution_key3_gpu0_20260722.json
...
"%PY%" -c "import hashlib,json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); gate_path=pathlib.Path(r'%OPA2_GATE%'); gate=json.loads(gate_path.read_text(encoding='utf-8')); digest=hashlib.sha256(gate_path.read_bytes()).hexdigest(); common=plan['common']; assert digest==cfg['opa2_gate_sha256']==plan['opa2_authority']['gate_sha256']; assert gate['run_id']==plan['opa2_authority']['run_id'] and gate['status']=='pass' and gate['decision']=='innovation2_selected8_architecture_priority_independently_confirmed' and gate['candidate_architecture']=='present_spn' and gate['metrics']['priority_passed'] is True and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()); assert cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['expected_result_rows']==24 and cfg['expected_history_rows']==300 and cfg['expected_checkpoints']==3 and cfg['seed']==3 and cfg['sample_classification'] is False and common['sample_classification'] is False and len(plan['rows'])==3; print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed

echo started>"%LOG_DIR%\%RUN_ID%_started.marker"
"%PY%" scripts\run-innovation2-selected-output-topology-attribution ^
...
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opa2_gate.json summary.json gate.json checkpoint_manifest.json) do if not exist "%RESULTS_DIR%\%%F" goto incomplete_results
...
for %%F in (present_spn_exact_p_true_output_final.pt present_spn_identity_p_true_output_final.pt present_spn_wrong_p_true_output_final.pt) do if not exist "%RESULTS_DIR%\models\%%F" goto incomplete_results
set RESULT_LINES=0
...
"%PY%" -c "import csv,hashlib,json,pathlib; root=pathlib.Path(r'%RESULTS_DIR%'); gate=json.loads((root/'gate.json').read_text(encoding='utf-8')); meta=json.loads((root/'metadata.json').read_text(encoding='utf-8')); cache=json.loads((root/'data'/'cache_metadata.json').read_text(encoding='utf-8')); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text(encoding='utf-8')); history=list(csv.DictReader((root/'history.csv').open(encoding='utf-8'))); opa2=root/'opa2_gate.json'; assert hashlib.sha256(opa2.read_bytes()).hexdigest()=='97943c59a8d88f8bbc1b6845aa6372a8b91d5693e0abd3a23e63b41259601284' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==3 and meta['config']['mode']=='topology_attribution' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==300 and len(checkpoints)==3 and all(row['sha256'] for row in checkpoints)" || goto incomplete_results

if not exist "%ARCHIVE_DIR%" mkdir "%ARCHIVE_DIR%" || goto failed
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opa2_gate.json summary.json gate.json checkpoint_manifest.json) do copy /Y "%RESULTS_DIR%\%%F" "%ARCHIVE_DIR%\%%F" > nul || goto failed
