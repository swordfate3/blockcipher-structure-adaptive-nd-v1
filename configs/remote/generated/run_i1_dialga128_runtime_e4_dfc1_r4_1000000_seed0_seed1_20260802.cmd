...
set RUN_ID=i1_dialga128_runtime_e4_dfc1_r4_1000000_seed0_seed1_20260802
...
set PLAN=configs\experiment\innovation1\innovation1_spn_dialga128_runtime_e4_dfc1_r4_1000000_seed0_seed1.csv
set REMOTE_CONFIG=configs\remote\innovation1_dialga_dfc1_r4_formal_1000000_seed0_seed1_gpu0_20260802.json
...
if errorlevel 1 goto failed
"%PY%" scripts\gate-dialga-r4-dfc1 --plan "%PLAN%" --results "%RESULTS_DIR%\results.jsonl" --progress "%LOG_DIR%\progress.jsonl" --checkpoint-root "%CHECKPOINT_DIR%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --output-root "%RESULTS_DIR%" > "%LOG_DIR%\%RUN_ID%_gate_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_gate_stderr.txt"
if errorlevel 1 goto failed
...
"%PY%" scripts\package-dialga-r4-dfc1 --run-root "%RUN_ROOT%" --source-root "%SOURCE_ROOT%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --archive-root "%ARCHIVE_DIR%" > "%LOG_DIR%\%RUN_ID%_package_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_package_stderr.txt"
if errorlevel 1 goto failed
...
git add -f "results_archive\%RUN_ID%" || goto result_sync_failed
git commit -m "results: %RUN_ID% project-formal evaluation" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto result_sync_failed
