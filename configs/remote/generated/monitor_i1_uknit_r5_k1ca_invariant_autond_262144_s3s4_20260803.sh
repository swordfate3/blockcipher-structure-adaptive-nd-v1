...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_uknit_k1ca_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || exit 7
...
retrieve_archive() {
  ...
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/validate-results --plan "${destination}/plan.csv" --results "${destination}/results.jsonl" --expected-rows 4 --output "${destination}/validation.local.json" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || return 1
  mkdir -p "${destination}/local_adjudication"
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/gate-uknit-r5-k1ca-closeout --plan "${destination}/plan.csv" --results "${destination}/results.jsonl" --progress "${destination}/progress.jsonl" --checkpoint-root "${destination}/checkpoints" --source-commit-file "${destination}/git_revision.txt" --expected-source-commit-file "${destination}/source_expected_commit.txt" --output-root "${destination}/local_adjudication" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || true
