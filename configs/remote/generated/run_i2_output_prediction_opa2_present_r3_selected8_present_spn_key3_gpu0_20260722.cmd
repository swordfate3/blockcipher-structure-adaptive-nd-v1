...
set RUN_ID=i2_output_prediction_opa2_present_r3_selected8_present_spn_key3_gpu0_20260722
set REMOTE_DIR=i2_opa2_pspn_k3_20260722
set ARCHIVE_NAME=i2_opa2_pspn_k3_20260722
...
set PHASE_A_GATE=%RUNS_ROOT%\i2_output_prediction_opa1_present_r3_selected8_architecture_screen_key2_gpu0_20260721\results\gate.json
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_opa2_present_r3_selected8_present_spn_key3.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-opa2-conditional-architecture-confirmation-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_opa2_present_r3_selected8_present_spn_key3_gpu0_20260722.json
...
"%PY%" -c "import hashlib,json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); gate_path=pathlib.Path(r'%PHASE_A_GATE%'); gate=json.loads(gate_path.read_text(encoding='utf-8')); digest=hashlib.sha256(gate_path.read_bytes()).hexdigest(); common=plan['common']; assert digest==cfg['phase_a_gate_sha256']==plan['phase_a_authority']['gate_sha256']; assert gate['run_id']==plan['phase_a_authority']['run_id'] and gate['status']=='pass' and gate['decision']=='innovation2_selected8_architecture_candidate_requires_confirmation' and gate['metrics']['selected_candidate_for_phase_b']=='present_spn' and gate['metrics']['candidate_gates']['present_spn']['passed'] is True and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()); assert cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['expected_result_rows']==32 and cfg['expected_history_rows']==400 and cfg['expected_checkpoints']==4 and cfg['seed']==3 and cfg['candidate_architecture']=='present_spn' and cfg['sample_classification'] is False and common['sample_classification'] is False and len(plan['rows'])==4; print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed

echo started>"%LOG_DIR%\%RUN_ID%_started.marker"
"%PY%" scripts\run-innovation2-selected-output-architecture-confirmation ^
  --mode phase_b_confirmation ^
  --phase-a-gate "%PHASE_A_GATE%" ^
  --run-id "%RUN_ID%" ^
  --device cuda ^
...
for %%F in (results.jsonl progress.jsonl history.csv metadata.json phase_a_gate.json summary.json gate.json checkpoint_manifest.json) do if not exist "%RESULTS_DIR%\%%F" goto incomplete_results
...
for %%F in (selected8_mlp_true_output_final.pt selected8_mlp_label_shuffle_final.pt selected8_present_spn_true_output_final.pt selected8_present_spn_label_shuffle_final.pt) do if not exist "%RESULTS_DIR%\models\%%F" goto incomplete_results
set RESULT_LINES=0
...
echo expected_rows=32 >> "%LOG_DIR%\%RUN_ID%_result_gate.txt"
if not "%RESULT_LINES%"=="32" goto incomplete_results
"%PY%" -c "import csv,json,pathlib; root=pathlib.Path(r'%RESULTS_DIR%'); gate=json.loads((root/'gate.json').read_text(encoding='utf-8')); meta=json.loads((root/'metadata.json').read_text(encoding='utf-8')); cache=json.loads((root/'data'/'cache_metadata.json').read_text(encoding='utf-8')); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text(encoding='utf-8')); history=list(csv.DictReader((root/'history.csv').open(encoding='utf-8'))); assert gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and gate['candidate_architecture']=='present_spn' and meta['sample_classification'] is False and meta['config']['seed']==3 and meta['config']['mode']=='phase_b_confirmation' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==400 and len(checkpoints)==4 and all(row['sha256'] for row in checkpoints)" || goto incomplete_results

if not exist "%ARCHIVE_DIR%" mkdir "%ARCHIVE_DIR%" || goto failed
for %%F in (results.jsonl progress.jsonl history.csv metadata.json phase_a_gate.json summary.json gate.json checkpoint_manifest.json) do copy /Y "%RESULTS_DIR%\%%F" "%ARCHIVE_DIR%\%%F" > nul || goto failed
