...
set RUN_ID=i1_uknit_family_ctspn_position_residual_k1u_medium_65536_seed3_seed4_20260728
...
set PLAN=configs\experiment\innovation1\innovation1_uknit_family_ctspn_position_residual_k1u_medium_65536_seed3_seed4.csv
set REMOTE_CONFIG=configs\remote\innovation1_uknit_k1u_position_residual_medium_65536_seed3_seed4_gpu1_20260728.json
...
"%PY%" scripts\validate-results ^
  ...
  --expected-rows 6 ^
  --output "%RESULTS_DIR%\validation-plan.json" ^
  > "%LOG_DIR%\%RUN_ID%_validation_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_validation_stderr.txt"
if errorlevel 1 goto failed

"%PY%" scripts\gate-uknit-family-ctspn-k1u ^
  --plan "%PLAN%" ^
  --results "%RESULTS_DIR%\results.jsonl" ^
  --progress "%LOG_DIR%\progress.jsonl" ^
  --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" ^
  --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" ^
...
"%PY%" scripts\package-uknit-family-ctspn-k1u ^
  --run-root "%RUN_ROOT%" ^
  --source-root "%SOURCE_ROOT%" ^
  --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" ^
  --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" ^
  --archive-root "%ARCHIVE_DIR%" ^
  > "%LOG_DIR%\%RUN_ID%_package_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_package_stderr.txt"
if errorlevel 1 goto failed
...
git rev-parse HEAD > "%LOG_DIR%\%RUN_ID%_result_branch_revision.txt" 2>&1 || goto result_sync_failed
