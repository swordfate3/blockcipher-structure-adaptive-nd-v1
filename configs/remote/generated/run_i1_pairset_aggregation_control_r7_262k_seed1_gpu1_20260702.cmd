...
set RUN_ID=i1_pairset_aggregation_control_r7_262k_seed1_gpu1_20260702
set STAGE_A_ID=i1_pairset_single_pair_scorer_r7_262k_seed1_gpu1_20260702
...
echo stage_a_id=%STAGE_A_ID%>>"%LOG_DIR%\%RUN_ID%_launch_env.txt"
...
echo stage_a_started>"%LOG_DIR%\%STAGE_A_ID%_started.marker"
"%PYTHON_EXE%" scripts\train ^
  --plan configs\experiment\innovation1\innovation1_spn_present_pairset_aggregation_control_single_pair_r7_262k_seed1.csv ^
  --epochs 20 ^
  --batch-size 1024 ^
  --hidden-bits 32 ^
  --device cuda:1 ^
  ...
  --checkpoint-output "%CHECKPOINT_DIR%\single_pair_invp.pt" ^
  ...
  --output "%RESULTS_DIR%\%STAGE_A_ID%.jsonl" ^
  --progress-output "%LOG_DIR%\%STAGE_A_ID%_progress.jsonl" ^
  > "%LOG_DIR%\%STAGE_A_ID%_stdout.txt" 2> "%LOG_DIR%\%STAGE_A_ID%_stderr.txt"

if errorlevel 1 goto failed
if not exist "%CHECKPOINT_DIR%\single_pair_invp.pt" goto failed
echo stage_a_done>"%LOG_DIR%\%STAGE_A_ID%_done.marker"

echo stage_b_started>"%LOG_DIR%\%RUN_ID%_stage_b_started.marker"
"%PYTHON_EXE%" scripts\train ^
  --plan configs\experiment\innovation1\innovation1_spn_present_pairset_aggregation_control_r7_262k_seed1.csv ^
  --epochs 20 ^
  --batch-size 1024 ^
  --hidden-bits 32 ^
  --device cuda:1 ^
...
if errorlevel 1 goto failed
echo stage_b_done>"%LOG_DIR%\%RUN_ID%_stage_b_done.marker"

"%PYTHON_EXE%" scripts\evaluate-pairset-aggregation ^
  --checkpoint "%CHECKPOINT_DIR%\single_pair_invp.pt" ^
  --eval-plan configs\experiment\innovation1\innovation1_spn_present_pairset_aggregation_control_r7_262k_seed1.csv ^
  --eval-row-index 0 ^
  ...
  --batch-size 1024 ^
  --device cuda:1 ^
  --output "%RESULTS_DIR%\frozen_aggregation_summary.json" ^
  > "%LOG_DIR%\%RUN_ID%_frozen_aggregation_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_frozen_aggregation_stderr.txt"

if errorlevel 1 goto failed
echo frozen_aggregation_done>"%LOG_DIR%\%RUN_ID%_frozen_aggregation_done.marker"
