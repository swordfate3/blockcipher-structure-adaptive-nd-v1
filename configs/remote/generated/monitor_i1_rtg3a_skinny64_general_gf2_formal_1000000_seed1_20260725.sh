...
JOINT_ID="i1_rtg3a_skinny64_general_gf2_formal_1000000_joint_seed0_seed1_20260725"
...
LAUNCH_SCRIPT="configs\\remote\\generated\\launch_i1_rtg3a_skinny64_general_gf2_formal_1000000_seed1_20260725.cmd"
...
SOURCE_COMMIT="${1:-}"
LAUNCH_GATE_PATH="${2:-outputs/local_readiness/i1_rtg3a_skinny64_general_gf2_formal_1000000_seed1_launch_gate_20260725/gate.json}"
...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE_PATH}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_rtg3a_seed1_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || {
...
retrieve_and_readjudicate() {
  ...
  if [[ -e "${DESTINATION}" ]]; then
    if [[ -f "${DESTINATION}/retrieved_from_verified_result_branch.marker" \
      ...
      && -f "${DESTINATION}/gate.local.json" \
      && -f "${DESTINATION}/checkpoint-verification.local.json" ]]; then
...
wait_for_visual_qa() {
  while [[ ! -f "${DESTINATION}/visual_qa_passed.marker" ]]; do
    if [[ -f "${DESTINATION}/visual_qa_failed.marker" ]]; then
      echo "$(timestamp) visual_qa_failed_joint_not_created" >> "${MONITOR_ROOT}/monitor.log"
      return 1
    fi
...
if [[ -f "${DESTINATION}/gate.local.json" ]]; then
  ...
  echo "$(timestamp) result_already_retrieved_visual_qa_and_joint_complete" >> "${MONITOR_ROOT}/monitor.log"
  exit 0
fi
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    ...
    adjudicate_joint || exit 5
    touch "${MONITOR_ROOT}/two_seed_complete.marker"
    echo "$(timestamp) visual_qa_passed_joint_adjudicated_and_indexed" >> "${MONITOR_ROOT}/monitor.log"
    exit 0
  fi
