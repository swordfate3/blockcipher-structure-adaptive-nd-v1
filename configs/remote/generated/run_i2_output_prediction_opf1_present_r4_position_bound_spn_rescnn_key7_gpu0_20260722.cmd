...
set RUN_ID=i2_output_prediction_opf1_present_r4_position_bound_spn_rescnn_key7_gpu0_20260722
set REMOTE_DIR=i2_opf1_r4_poshead_k7_20260722
set ARCHIVE_NAME=i2_opf1_r4_poshead_k7_20260722
...
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_opf1_present_r4_position_bound_spn_rescnn_key7.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-opf1-present-r4-position-bound-boundary-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_opf1_present_r4_position_bound_spn_rescnn_key7_gpu0_20260722.json
...
"%PY%" -c "import hashlib,json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); paths=[pathlib.Path(r'%OPC1_GATE%'),pathlib.Path(r'%OPN1_GATE%'),pathlib.Path(r'%OPD1_GATE%')]; gates=[json.loads(p.read_text(encoding='utf-8')) for p in paths]; hashes=[hashlib.sha256(p.read_bytes()).hexdigest() for p in paths]; assert hashes==[cfg['opc1_gate_sha256'],cfg['opn1_gate_sha256'],cfg['opd1_gate_sha256']]; assert gates[0]['status']=='hold' and gates[0]['decision']=='innovation2_spn_rescnn_hybrid_not_supported'; assert gates[1]['status']=='pass' and gates[1]['decision']=='innovation2_spn_rescnn_final_routing_absorbable_by_global_head'; assert gates[2]['status']=='hold' and gates[2]['decision']=='innovation2_position_bound_spn_rescnn_not_supported'; assert all(all(g[group].values()) for g in gates for group in ('protocol_checks','execution_checks')); common=plan['common']; assert cfg['rounds']==4 and common['rounds']==4 and plan['only_changed_variable']=={'field':'rounds','opd1_value':3,'opf1_value':4} and cfg['seed']==7 and common['seed']==7 and cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['epochs']==100 and cfg['sample_classification'] is False and common['sample_classification'] is False and len(plan['rows'])==5; print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto readiness_failed
...
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opc1_gate.json opn1_gate.json opd1_gate.json summary.json gate.json checkpoint_manifest.json) do if not exist "%RESULTS_DIR%\%%F" goto incomplete_results
...
"%PY%" -c "import csv,hashlib,json,pathlib; root=pathlib.Path(r'%RESULTS_DIR%'); gate=json.loads((root/'gate.json').read_text(encoding='utf-8')); meta=json.loads((root/'metadata.json').read_text(encoding='utf-8')); cache=json.loads((root/'data'/'cache_metadata.json').read_text(encoding='utf-8')); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text(encoding='utf-8')); history=list(csv.DictReader((root/'history.csv').open(encoding='utf-8'))); source=[root/'opc1_gate.json',root/'opn1_gate.json',root/'opd1_gate.json']; expected=['ebb86a9feab6d2d9993937f5c0a7f4afe1bfe3597c8c1dff083956381e0310b4','887a7db3643e73bdda67958bcaae470881a09db25ab0ba5ff6c3d6bb0a2503d7','3d63163ab94e95b6c8c859be0867cc0a6b1f91382bd842e32dd3adbe04863579']; assert [hashlib.sha256(p.read_bytes()).hexdigest() for p in source]==expected and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and gate['protocol_checks']['plaintext_file_sha256_matches_opd1'] and meta['sample_classification'] is False and meta['config']['seed']==7 and meta['config']['rounds']==4 and meta['config']['mode']=='round_extension' and cache['status']=='complete' and cache['completed_rows']==196608 and hashlib.sha256((root/'data'/'plaintexts.npy').read_bytes()).hexdigest()=='0f08d171c5b833ee1223da07bfc80e10d7ea99bbc0bef1b068547d3a7e8120e1' and len(history)==500 and len(checkpoints)==5 and all(row['sha256'] for row in checkpoints)" || goto incomplete_results

if not exist "%ARCHIVE_DIR%" mkdir "%ARCHIVE_DIR%" || goto failed
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opc1_gate.json opn1_gate.json opd1_gate.json summary.json gate.json checkpoint_manifest.json) do copy /Y "%RESULTS_DIR%\%%F" "%ARCHIVE_DIR%\%%F" > nul || goto failed
...
echo Frozen OPC1, OPN1, or OPD1 readiness authority did not match the remote package.>"%LOG_DIR%\%RUN_ID%_failure_reason.txt"
exit /b 8
