...
set RUN_ID=i2_speck32_hwang_phase_c_32plus32_gpu0_20260717
...
"%PY%" scripts\audit-innovation2-speck-hwang-phase-c ^
  --run-id "%RUN_ID%" ^
...
if not exist "%ARCHIVE_DIR%\cache\anchor" mkdir "%ARCHIVE_DIR%\cache\anchor"
if not exist "%ARCHIVE_DIR%\cache\control" mkdir "%ARCHIVE_DIR%\cache\control"
...
copy /Y "%ARTIFACT_DIR%\cache\anchor\metadata.json" "%ARCHIVE_DIR%\cache\anchor\metadata.json" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\anchor\parity_rows.npy" "%ARCHIVE_DIR%\cache\anchor\parity_rows.npy" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\anchor\completed.npy" "%ARCHIVE_DIR%\cache\anchor\completed.npy" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\control\metadata.json" "%ARCHIVE_DIR%\cache\control\metadata.json" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\control\parity_rows.npy" "%ARCHIVE_DIR%\cache\control\parity_rows.npy" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\control\completed.npy" "%ARCHIVE_DIR%\cache\control\completed.npy" > nul || goto failed
...
git commit -m "results: %RUN_ID% exact-kernel readiness" > "%LOG_DIR%\%RUN_ID%_branch_commit.txt" 2>&1 || goto failed
