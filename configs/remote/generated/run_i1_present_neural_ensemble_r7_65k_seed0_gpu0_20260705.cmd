...
set RUN_ID=i1_present_neural_ensemble_r7_65k_seed0_gpu0_20260705
...
set DATASET_CACHE_ROOT=G:\lxy\blockcipher-structure-adaptive-nd-runs\shared_dataset_cache
...
"%PYTHON_EXE%" scripts\check-remote-readiness --config configs\remote\innovation1_spn_present_neural_ensemble_r7_65k_seed0_gpu0_20260705.json > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed
...
"%PYTHON_EXE%" scripts\train ^
  ...
  --progress-output "%LOG_DIR%\train_matrix_progress.jsonl" ^
  > "%LOG_DIR%\%RUN_ID%_train_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_train_stderr.txt"

if errorlevel 1 goto failed
if not exist "%CHECKPOINT_DIR%\row0001_present_zhang_wang_keras_mcnd_seed0.pt" goto failed
if not exist "%CHECKPOINT_DIR%\row0002_present_nibble_invp_only_spn_only_seed0.pt" goto failed
if not exist "%CHECKPOINT_DIR%\row0003_present_nibble_ddt_graph_seed0.pt" goto failed
echo train_done>"%LOG_DIR%\%RUN_ID%_train_done.marker"

"%PYTHON_EXE%" scripts\export-checkpoint-scores ^
  --checkpoint "%CHECKPOINT_DIR%\row0001_present_zhang_wang_keras_mcnd_seed0.pt" ^
  ...
  --batch-size 1024 ^
  --device cuda:0 ^
  --output-dir "%SCORE_ARTIFACT_DIR%\zhang_wang" ^
  > "%LOG_DIR%\%RUN_ID%_export_zhang_wang_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_export_zhang_wang_stderr.txt"
if errorlevel 1 goto failed

"%PYTHON_EXE%" scripts\export-checkpoint-scores ^
  --checkpoint "%CHECKPOINT_DIR%\row0002_present_nibble_invp_only_spn_only_seed0.pt" ^
  --eval-plan configs\experiment\innovation1\innovation1_spn_present_neural_ensemble_r7_65k_seed0.csv ^
  --eval-row-index 1 ^
  ...
  --batch-size 1024 ^
  --device cuda:0 ^
  --output-dir "%SCORE_ARTIFACT_DIR%\invp_only" ^
  > "%LOG_DIR%\%RUN_ID%_export_invp_only_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_export_invp_only_stderr.txt"
if errorlevel 1 goto failed

"%PYTHON_EXE%" scripts\export-checkpoint-scores ^
  --checkpoint "%CHECKPOINT_DIR%\row0003_present_nibble_ddt_graph_seed0.pt" ^
  --eval-plan configs\experiment\innovation1\innovation1_spn_present_neural_ensemble_r7_65k_seed0.csv ^
  --eval-row-index 2 ^
  ...
  --batch-size 1024 ^
  --device cuda:0 ^
  --output-dir "%SCORE_ARTIFACT_DIR%\ddt_graph" ^
  > "%LOG_DIR%\%RUN_ID%_export_ddt_graph_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_export_ddt_graph_stderr.txt"
if errorlevel 1 goto failed

echo score_export_done>"%LOG_DIR%\%RUN_ID%_score_export_done.marker"
"%PYTHON_EXE%" scripts\evaluate-neural-ensemble ^
  --artifacts "%SCORE_ARTIFACT_DIR%\zhang_wang" "%SCORE_ARTIFACT_DIR%\invp_only" "%SCORE_ARTIFACT_DIR%\ddt_graph" ^
  --output "%RESULTS_DIR%\neural_ensemble_summary.json" ^
  > "%LOG_DIR%\%RUN_ID%_ensemble_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_ensemble_stderr.txt"

if errorlevel 1 goto failed
echo ensemble_done>"%LOG_DIR%\%RUN_ID%_ensemble_done.marker"
