...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-op12-structured-xor-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); history=list(csv.DictReader((root/'history.csv').open())); revision=(root/'git_revision.txt').read_text().strip(); assert revision=='${SOURCE_COMMIT}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['rounds']==4 and meta['config']['seed']==1 and cache['status']=='complete' and cache['completed_rows']==196608 and len(checkpoints)==4 and len(history)==400" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      scripts/plot-innovation2-structured-output-xor \
