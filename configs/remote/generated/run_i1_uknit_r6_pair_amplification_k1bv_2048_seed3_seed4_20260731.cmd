...
set RUN_ID=i1_uknit_r6_pair_amplification_k1bv_2048_seed3_seed4_20260731
...
set PLAN=configs\experiment\innovation1\innovation1_uknit_r6_pair_amplification_k1bv_2048_seed3_seed4.csv
set REMOTE_CONFIG=configs\remote\innovation1_uknit_r6_pair_amplification_k1bv_2048_seed3_seed4_gpu0_20260731.json
...
"%PY%" scripts\check-remote-readiness --config "%REMOTE_CONFIG%" > "%LOG_DIR%\%RUN_ID%_remote_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_remote_readiness_stderr.txt" || goto failed
"%PY%" scripts\check-uknit-r6-pair-amplification-k1bv --plan "%PLAN%" --output "%RESULTS_DIR%\readiness.json" > "%LOG_DIR%\%RUN_ID%_model_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_model_readiness_stderr.txt" || goto failed
...
if errorlevel 1 goto failed
"%PY%" scripts\gate-uknit-r6-pair-amplification-k1bv --plan "%PLAN%" --results "%RESULTS_DIR%\results.jsonl" --progress "%LOG_DIR%\progress.jsonl" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --output-root "%RESULTS_DIR%" > "%LOG_DIR%\%RUN_ID%_gate_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_gate_stderr.txt"
if errorlevel 1 goto failed
...
"%PY%" scripts\package-uknit-r6-pair-amplification-k1bv --run-root "%RUN_ROOT%" --source-root "%SOURCE_ROOT%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --archive-root "%ARCHIVE_DIR%" > "%LOG_DIR%\%RUN_ID%_package_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_package_stderr.txt"
if errorlevel 1 goto failed
...
git commit -m "results: %RUN_ID% remote diagnostic" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto result_sync_failed
