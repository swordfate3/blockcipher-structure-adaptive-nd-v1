...
REMOTE="lxy-a6000"
RUN_ID="i1_rtg3b_present80_one_to_one_formal_1000000_seed0_retry1_20260727"
...
LAUNCH_SCRIPT="configs\\remote\\generated\\launch_i1_rtg3b_present80_one_to_one_formal_1000000_seed0_retry1_20260727.cmd"
...
SOURCE_COMMIT="${1:-}"
LAUNCH_GATE_PATH="${2:-outputs/local_readiness/i1_rtg3b_present80_one_to_one_formal_1000000_seed0_retry1_launch_gate_20260727/gate.json}"
...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE_PATH}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_rtg3b_present_seed0_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}' and g.get('live_remote_sha') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || {
