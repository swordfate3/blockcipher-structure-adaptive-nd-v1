...
if not isinstance(best_auc, (int, float)):
    ...
    decision = "stop_from_scratch_r8_route_or_plan_curriculum_transfer"
elif best_auc < 0.55:
    decision = "weak_r8_signal_repeat_seed_or_curriculum"
elif isinstance(delta_vs_baseline, (int, float)) and delta_vs_baseline >= 0.005:
    decision = "support_scale_r8_to_1m_seed0"
else:
    decision = "r8_signal_needs_manual_gate_review"
report = {
    "run_id": result_path.stem,
    "status": "medium_diagnostic_only",
    "claim_scope": "PRESENT r8 262144/class single-seed diagnostic, not formal evidence",
