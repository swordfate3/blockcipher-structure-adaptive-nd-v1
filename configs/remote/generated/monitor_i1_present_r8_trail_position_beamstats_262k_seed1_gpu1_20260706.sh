...
while true; do
  ...
  verification_summary="${LOCAL_ROOT}/score_artifacts/verification_summary.json"
  result_rows=0
  if [[ -f "${train_file}" ]]; then
    result_rows=$(grep -cve '^[[:space:]]*$' "${train_file}" || true)
  fi

  if [[ "${result_rows}" -ge "${EXPECTED_ROWS}" && -f "${global_stats_scores}" && -f "${trail_position_scores}" && -f "${verification_summary}" ]]; then
    echo "$(timestamp) result_ready" >> "${MONITOR_DIR}/monitor.log"
    echo "$(timestamp) score_artifacts_ready global_stats_control/models.json trail_position/models.json score_artifacts/verification_summary.json" >> "${MONITOR_DIR}/monitor.log"
    exit 0
  fi
