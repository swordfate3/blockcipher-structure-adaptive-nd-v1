...
LAUNCH_SCRIPT="configs\\remote\\generated\\launch_i1_rtg3b_present80_one_to_one_formal_1000000_seed1_retry1_20260727.cmd"
...
SOURCE_COMMIT="${1:-}"
LAUNCH_GATE_PATH="${2:-outputs/local_readiness/i1_rtg3b_present80_one_to_one_formal_1000000_seed1_retry1_launch_gate_20260727/gate.json}"
...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE_PATH}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_rtg3b_present_seed1_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}' and g.get('live_remote_sha') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || {
...
confirm_started_bounded() {
  ...
  for attempt in $(seq 1 30); do
    if ssh -o BatchMode=yes -o ConnectTimeout=8 "${REMOTE}" \
      "cmd.exe /c if exist G:\\lxy\\blockcipher-structure-adaptive-nd-runs\\${RUN_ID}\\logs\\${RUN_ID}_started.marker (if exist G:\\lxy\\blockcipher-structure-adaptive-nd-runs\\launcher_logs\\${RUN_ID}_schedule_disabled.marker (if exist G:\\lxy\\blockcipher-structure-adaptive-nd-runs\\${RUN_ID}\\run.lock\\NUL (type G:\\lxy\\blockcipher-structure-adaptive-nd-runs\\${RUN_ID}\\logs\\${RUN_ID}_started.marker) else exit /b 10) else exit /b 9) else exit /b 8" \
...
retrieve_and_readjudicate() {
  ...
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/gate-runtime-spn-present-transfer \
    --run-id "${RUN_ID}" \
  ...
  cp "${DESTINATION}/gate.json" "${DESTINATION}/gate.local.json" || return 1
  cp "${DESTINATION}/summary.json" "${DESTINATION}/summary.local.json" || return 1
...
wait_for_visual_qa() {
  while [[ ! -f "${DESTINATION}/visual_qa_passed.marker" ]]; do
    if [[ -f "${DESTINATION}/visual_qa_failed.marker" ]]; then
      echo "$(timestamp) visual_qa_failed" >> "${MONITOR_ROOT}/monitor.log"
      return 1
    fi
    echo "$(timestamp) retrieved_waiting_for_visual_qa" >> "${MONITOR_ROOT}/monitor.log"
    sleep 300
  done
  echo "$(timestamp) visual_qa_passed" >> "${MONITOR_ROOT}/monitor.log"
}

if [[ -f "${DESTINATION}/gate.local.json" ]]; then
  wait_for_visual_qa || exit 9
  touch "${MONITOR_ROOT}/two_seed_results_ready.marker"
  echo "$(timestamp) seed1_result_already_retrieved_and_visual_qa_passed" >> "${MONITOR_ROOT}/monitor.log"
  exit 0
fi
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    ...
    wait_for_visual_qa || exit 9
    touch "${MONITOR_ROOT}/two_seed_results_ready.marker"
    echo "$(timestamp) visual_qa_passed_two_seed_synthesis_ready" >> "${MONITOR_ROOT}/monitor.log"
    exit 0
  fi
