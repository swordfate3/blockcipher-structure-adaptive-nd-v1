...
set RUN_ID=i2_output_prediction_op12_present_r4_structured_xor_key1_gpu0_20260721
...
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_op12_present_r4_structured_xor_key1.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-op12-present-r4-structured-xor-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_op12_present_r4_structured_xor_key1_gpu0_20260721.json
...
"%PY%" -c "import json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); common=plan['common']; assert cfg['rounds']==4 and cfg['seed']==1 and cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['expected_result_rows']==26 and cfg['expected_history_rows']==400 and cfg['epochs']==100 and cfg['batch_size']==250 and cfg['sample_classification'] is False and common['target']=='six_preregistered_true_ciphertext_output_xor_values' and len(plan['structured_masks'])==6 and len(plan['geometry_control_masks'])==6 and len(plan['rows'])==4; print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed

echo started>"%LOG_DIR%\%RUN_ID%_started.marker"
"%PY%" scripts\run-innovation2-structured-output-xor ^
...
for %%F in (selected8_mlp_true_output_final.pt structured6_mlp_true_xor_final.pt geometry6_mlp_true_xor_final.pt structured6_mlp_label_shuffle_final.pt) do if not exist "%RESULTS_DIR%\models\%%F" goto incomplete_results
set RESULT_LINES=0
