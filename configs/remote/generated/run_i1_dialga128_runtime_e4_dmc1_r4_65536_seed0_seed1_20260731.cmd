...
set RUN_ID=i1_dialga128_runtime_e4_dmc1_r4_65536_seed0_seed1_20260731
...
set PLAN=configs\experiment\innovation1\innovation1_spn_dialga128_runtime_e4_dmc1_r4_65536_seed0_seed1.csv
set REMOTE_CONFIG=configs\remote\innovation1_dialga_dmc1_r4_medium_65536_seed0_seed1_gpu1_20260731.json
...
if errorlevel 1 goto failed
"%PY%" scripts\gate-dialga-r4-dmc1 --plan "%PLAN%" --results "%RESULTS_DIR%\results.jsonl" --progress "%LOG_DIR%\progress.jsonl" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --output-root "%RESULTS_DIR%" > "%LOG_DIR%\%RUN_ID%_gate_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_gate_stderr.txt"
if errorlevel 1 goto failed
...
"%PY%" scripts\package-dialga-r4-dmc1 --run-root "%RUN_ROOT%" --source-root "%SOURCE_ROOT%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --archive-root "%ARCHIVE_DIR%" > "%LOG_DIR%\%RUN_ID%_package_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_package_stderr.txt"
if errorlevel 1 goto failed
