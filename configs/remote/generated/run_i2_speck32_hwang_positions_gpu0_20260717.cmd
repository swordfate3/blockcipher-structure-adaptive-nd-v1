...
set RUN_ID=i2_speck32_hwang_positions_gpu0_20260717
...
"%PY%" scripts\audit-innovation2-speck-hwang-positions ^
  --run-id "%RUN_ID%" ^
...
copy /Y "%ARTIFACT_DIR%\selected_candidates.npy" "%ARCHIVE_DIR%\selected_candidates.npy" > nul || goto failed
...
git commit -m "results: %RUN_ID% position family" > "%LOG_DIR%\%RUN_ID%_branch_commit.txt" 2>&1 || goto failed
