...
set RUN_ID=i1_rtg2b_skinny64_general_gf2_scale_262144_seed0_20260724
...
set PLAN=configs\experiment\innovation1\innovation1_spn_skinny64_runtime_e4_scale_rtg2b_262144_seed0.csv
set REMOTE_CONFIG=configs\remote\innovation1_rtg2b_skinny64_general_gf2_scale_262144_seed0_gpu0_20260724.json
...
"%PY%" scripts\gate-runtime-spn-skinny-medium ^
  --run-id "%RUN_ID%" ^
...
copy /Y "%RESULTS_DIR%\validation-plan.json" "%ARCHIVE_DIR%\validation-plan.json" > nul || goto failed
...
copy /Y "%RESULTS_DIR%\summary.json" "%ARCHIVE_DIR%\summary.json" > nul || goto failed
...
copy /Y "%LOG_DIR%\%RUN_ID%_readiness.txt" "%ARCHIVE_DIR%\readiness.txt" > nul || goto failed
...
git commit -m "results: %RUN_ID% remote scale replication" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto failed
