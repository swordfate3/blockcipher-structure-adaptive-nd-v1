...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/simeck-curriculum-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/gate-feistel-balanced \
      --plan "${DESTINATION}/plan.csv" \
      --results "${DESTINATION}/results.jsonl" \
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-results \
      ...
      --title "创新1 Feistel：SIMECK 低轮到目标轮课程迁移 65536/类规模裁决" \
