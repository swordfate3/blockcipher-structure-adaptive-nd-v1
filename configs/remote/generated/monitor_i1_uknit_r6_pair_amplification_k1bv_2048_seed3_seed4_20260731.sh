...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_uknit_k1bv_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || exit 7
...
retrieve_archive() {
  ...
  if [[ "${mode}" == "raw" ]]; then
    printf '%s\n' "RAW FALLBACK RETRIEVAL: result branch unavailable; archive retrieved directly from the approved G:/lxy run root." "Treat as fallback-retrieved until local gates pass." > "${destination}/RAW_RETRIEVAL_NOTICE.txt"
  fi
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/validate-results --plan "${destination}/innovation1_uknit_r6_pair_amplification_k1bv_2048_seed3_seed4.csv" --results "${destination}/results.jsonl" --expected-rows 6 --output "${destination}/validation.local.json" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || return 1
  mkdir -p "${destination}/local_adjudication"
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/gate-uknit-r6-pair-amplification-k1bv --plan "${destination}/innovation1_uknit_r6_pair_amplification_k1bv_2048_seed3_seed4.csv" --results "${destination}/results.jsonl" --progress "${destination}/progress.jsonl" --source-commit-file "${destination}/git_revision.txt" --expected-source-commit-file "${destination}/source_expected_commit.txt" --output-root "${destination}/local_adjudication" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || true
  ...
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-uknit-r6-pair-amplification-k1bv --gate "${destination}/gate.local.json" --output "${destination}/curves.svg" --report "${destination}/plot_report.json" >> "${MONITOR_ROOT}/plot.log" 2>> "${MONITOR_ROOT}/plot_stderr.log" || return 1
