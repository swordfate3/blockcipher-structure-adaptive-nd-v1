...
REMOTE_DIR="i2_opa2_pspn_k3_20260722"
ARCHIVE_NAME="i2_opa2_pspn_k3_20260722"
...
PHASE_A_GATE_SHA256="9936d740f9af9c71e469edf4d23a2aa185776a8d3021335064e0a44cb79b7bd1"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opa2-pspn-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,hashlib,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); phase_a=root/'phase_a_gate.json'; revision=(root/'git_revision.txt').read_text().strip(); history=list(csv.DictReader((root/'history.csv').open())); assert revision=='${SOURCE_COMMIT}' and hashlib.sha256(phase_a.read_bytes()).hexdigest()=='${PHASE_A_GATE_SHA256}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and gate['candidate_architecture']=='present_spn' and meta['sample_classification'] is False and meta['config']['seed']==3 and meta['config']['mode']=='phase_b_confirmation' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==400 and len(checkpoints)==4" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      scripts/plot-innovation2-selected-output-architecture-confirmation \
