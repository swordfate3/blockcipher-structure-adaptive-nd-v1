...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE}').read_text(encoding='utf-8')); ok=g.get('status')=='pass' and g.get('decision')=='innovation1_uknit_r6_k1br_remote_launch_authorized' and g.get('launch_authorized') is True and g.get('source_commit')=='${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || exit 7
...
retrieve() {
  ...
  if [[ "${mode}" == "verified" ]]; then
    git fetch --force origin "refs/heads/results/${RUN_ID}:refs/remotes/origin/results/${RUN_ID}" >> "${MONITOR_ROOT}/branch.log" 2>> "${MONITOR_ROOT}/branch_stderr.log" || return 1
    git archive "refs/remotes/origin/results/${RUN_ID}" "results_archive/${RUN_ID}" | tar -x -C "${staging}/${RUN_ID}" --strip-components=2 || return 1
  else
  ...
  if [[ "${mode}" == "raw" ]]; then
    printf '%s\n' "RAW FALLBACK RETRIEVAL: result branch was unavailable; archive was retrieved from the completed G:\\lxy run root." > "${destination}/RAW_RETRIEVAL_NOTICE.txt"
  fi
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/validate-results --plan "${destination}/innovation1_uknit_r6_last2_neural_scale_k1br_262144_seed3.csv" --results "${destination}/results.jsonl" --expected-rows 3 --output "${destination}/validation.local.json" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || return 1
  mkdir -p "${destination}/local_adjudication"
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/gate-uknit-r6-last2-neural-scale-k1br --plan "${destination}/innovation1_uknit_r6_last2_neural_scale_k1br_262144_seed3.csv" --results "${destination}/results.jsonl" --progress "${destination}/progress.jsonl" --source-commit-file "${destination}/git_revision.txt" --expected-source-commit-file "${destination}/source_expected_commit.txt" --output-root "${destination}/local_adjudication" >> "${MONITOR_ROOT}/readjudication.log" 2>> "${MONITOR_ROOT}/readjudication_stderr.log" || true
  ...
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-uknit-r6-last2-neural-scale-k1br --gate "${destination}/gate.local.json" --output "${destination}/curves.svg" --report "${destination}/plot_report.json" || return 1
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    git ls-remote --exit-code origin "refs/heads/results/${RUN_ID}" >> "${MONITOR_ROOT}/branch.log" 2>> "${MONITOR_ROOT}/branch_stderr.log" || { sleep 300; continue; }
    retrieve verified || exit 2
