...
"%PY%" scripts\audit-innovation2-speck-hwang-phase-b ^
  --run-id "%RUN_ID%" ^
...
"%PY%" -c "import json,pathlib; p=pathlib.Path(r'%ARTIFACT_DIR%\gate.json'); g=json.loads(p.read_text()); assert g['status']=='pass'; assert all(g['readiness_checks'].values()); assert all(g['performance_checks'].values())" > "%LOG_DIR%\%RUN_ID%_gate_check.txt" 2> "%LOG_DIR%\%RUN_ID%_gate_check_stderr.txt" || goto failed
...
copy /Y "%ARTIFACT_DIR%\cache\metadata.json" "%ARCHIVE_DIR%\cache_metadata.json" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\parity_rows.npy" "%ARCHIVE_DIR%\parity_rows.npy" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\cache\completed.npy" "%ARCHIVE_DIR%\completed.npy" > nul || goto failed
...
git commit -m "results: %RUN_ID% exact-structure timing" > "%LOG_DIR%\%RUN_ID%_branch_commit.txt" 2>&1 || goto failed
