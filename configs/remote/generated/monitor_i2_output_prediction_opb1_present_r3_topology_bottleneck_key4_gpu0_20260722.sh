...
REMOTE_DIR="i2_opb1_tbneck_k4_20260722"
ARCHIVE_NAME="i2_opb1_tbneck_k4_20260722"
...
OPA3_GATE_SHA256="def55214d46acf0e199f465fda66e6ca394f094ceec78d419354357df1c50943"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opb1-tbneck-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,hashlib,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); opa3=root/'opa3_gate.json'; revision=(root/'git_revision.txt').read_text().strip(); history=list(csv.DictReader((root/'history.csv').open())); assert revision=='${SOURCE_COMMIT}' and hashlib.sha256(opa3.read_bytes()).hexdigest()=='${OPA3_GATE_SHA256}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==4 and meta['config']['mode']=='topology_bottleneck' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==400 and len(checkpoints)==4" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      scripts/plot-innovation2-selected-output-topology-bottleneck \
