...
REMOTE_DIR="i2_opa3_ptopo_k3_20260722"
ARCHIVE_NAME="i2_opa3_ptopo_k3_20260722"
...
OPA2_GATE_SHA256="97943c59a8d88f8bbc1b6845aa6372a8b91d5693e0abd3a23e63b41259601284"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opa3-ptopo-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,hashlib,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); opa2=root/'opa2_gate.json'; revision=(root/'git_revision.txt').read_text().strip(); history=list(csv.DictReader((root/'history.csv').open())); assert revision=='${SOURCE_COMMIT}' and hashlib.sha256(opa2.read_bytes()).hexdigest()=='${OPA2_GATE_SHA256}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==3 and meta['config']['mode']=='topology_attribution' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==300 and len(checkpoints)==3" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      scripts/plot-innovation2-selected-output-topology-attribution \
