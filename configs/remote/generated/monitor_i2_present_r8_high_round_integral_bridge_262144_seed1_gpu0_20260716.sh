...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-r8-integral-bridge-seed1-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-results \
      ...
      --title "创新2 H0：PRESENT 8轮高轮积分神经独立 seed1（262144总训练行）" \
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); local_gate=json.loads((root/'gate.local.json').read_text()); data=json.loads((root/'dataset_summary.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); assert all(gate['bridge_plan_checks'].values()); assert all(local_gate['bridge_plan_checks'].values()); assert not local_gate['readjudication']['anchor_layout_invalidated']; assert local_gate['readjudication']['source_revision_matches_expected']; assert gate['artifact_validation']['status']=='pass'; assert data['status']=='pass'; assert len(cache)==3 and all(v['status']=='complete' for v in cache.values())" \
