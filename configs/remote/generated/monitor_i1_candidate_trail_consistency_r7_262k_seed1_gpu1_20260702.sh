...
PLAN="configs/experiment/innovation1/innovation1_spn_present_candidate_trail_consistency_r7_262k_seed1.json"
...
while true; do
  ...
  if [[ "${result_rows}" -ge "${EXPECTED_ROWS}" ]]; then
    ...
    env UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/postprocess-candidate-trail \
      --plan "${PLAN}" \
      --results "${result_file}" \
