...
if not "%SEED%"=="6" if not "%SEED%"=="7" goto invalid_arguments

set RUN_ID=i1_gift64_mainstream_performance_1m_seed%SEED%
...
set PLAN=configs\experiment\innovation1\innovation1_spn_gift64_mainstream_performance_1m_seed%SEED%.csv
set REMOTE_CONFIG=configs\remote\innovation1_gift64_mainstream_performance_1m_seed%SEED%_gpu%PHYSICAL_GPU%_20260715.json
set SOURCE_MANIFEST=configs\experiment\innovation1\innovation1_spn_gift64_mainstream_performance_sources.json
...
if not exist "outputs\local_smoke\i1_present_cross_spn_source_seed_r5_8192_seed1\results.jsonl" goto missing_source_assets
if not exist "outputs\local_smoke\i1_present_cross_spn_source_seed_r5_8192_seed1\checkpoints\row0002_present_cross_spn_typed_cell_true_seed1.pt" goto missing_source_assets
...
call :export_score 1 gift_cross_spn_typed_cell_true_from_present_true_s0 row0002_gift_cross_spn_typed_cell_true_from_present_true_s0_seed%SEED%.pt typed_source0 || goto failed
call :export_score 2 gift_cross_spn_typed_cell_true_from_present_true_s1 row0003_gift_cross_spn_typed_cell_true_from_present_true_s1_seed%SEED%.pt typed_source1 || goto failed
call :export_score 3 gift64_sun_style_lstm_pairset row0004_gift64_sun_style_lstm_pairset_seed%SEED%.pt lstm || goto failed
call :export_score 4 gift64_gohr_style_resnet_pairset row0005_gift64_gohr_style_resnet_pairset_seed%SEED%.pt resnet || goto failed

"%PY%" scripts\gate-cross-spn-mainstream-performance ^
  --plan "%PLAN%" ^
  --results "%RESULTS_DIR%\results.jsonl" ^
  --expected-seed %SEED% ^
...
for %%R in (typed_scratch typed_source0 typed_source1 lstm resnet) do copy /Y "%SCORES_DIR%\%%R\models.json" "%ARCHIVE_DIR%\score_metadata\%%R.json" > nul || goto failed
...
git commit -m "results: %RUN_ID% large-scale benchmark" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto failed
...
if not exist "%SEED6_ROOT%\results\gate.json" exit /b 0
if not exist "%SEED7_ROOT%\results\gate.json" exit /b 0
...
"%PY%" scripts\gate-cross-spn-mainstream-performance-joint ^
...
git commit -m "results: %JOINT_ID% joint benchmark gate" > "%JOINT_ROOT%\joint_branch_commit.txt" 2>&1 || exit /b 1
