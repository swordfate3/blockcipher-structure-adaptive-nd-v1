...
RUN_ID="i1_spn_topology_aware_network_r7_262k_seed1_gpu1_accel_bf16_20260701"
REMOTE="lxy-a6000"
...
PLAN_DOC="docs/experiments/innovation1-training-accelerator-topology-seed1-plan.md"
EXPECTED_ROWS="3"
...
while true; do
  ...
  if [[ "${result_rows}" -ge "${EXPECTED_ROWS}" ]]; then
    ...
    env UV_CACHE_DIR=/tmp/uv-cache MPLCONFIGDIR=/tmp/mplconfig uv run python scripts/postprocess-topology-aware-result \
      --plan "${PLAN}" \
      --results "${result_file}" \
