...
set RUN_ID=i2_speck32_hwang_contexts_32plus32_gpu0_20260717
...
"%PY%" scripts\audit-innovation2-speck-hwang-contexts ^
  --run-id "%RUN_ID%" ^
...
copy /Y "%ARTIFACT_DIR%\context_parity_rows.npy" "%ARCHIVE_DIR%\context_parity_rows.npy" > nul || goto failed
copy /Y "%ARTIFACT_DIR%\direct_context11_rows.npy" "%ARCHIVE_DIR%\direct_context11_rows.npy" > nul || goto failed
...
git commit -m "results: %RUN_ID% fixed-context audit" > "%LOG_DIR%\%RUN_ID%_branch_commit.txt" 2>&1 || goto failed
