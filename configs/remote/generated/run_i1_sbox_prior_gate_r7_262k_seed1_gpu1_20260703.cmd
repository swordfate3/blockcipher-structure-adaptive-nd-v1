...
set RUN_ID=i1_sbox_prior_gate_r7_262k_seed1_gpu1_20260703
...
set SBOX_PRIOR_CACHE_ROOT=G:\lxy\blockcipher-structure-adaptive-nd-runs\sbox_prior_gate_cache
...
if not exist "%SBOX_PRIOR_CACHE_ROOT%" mkdir "%SBOX_PRIOR_CACHE_ROOT%"
...
echo sbox_prior_cache_root=%SBOX_PRIOR_CACHE_ROOT%>>"%LOG_DIR%\%RUN_ID%_launch_env.txt"
...
"%PYTHON_EXE%" scripts\check-remote-readiness --config configs\remote\innovation1_spn_present_sbox_transition_prior_gate_r7_262k_seed1_gpu1_20260703.json > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed
...
"%PYTHON_EXE%" scripts\train ^
  ...
  --progress-output "%LOG_DIR%\sbox_prior_progress.jsonl" ^
