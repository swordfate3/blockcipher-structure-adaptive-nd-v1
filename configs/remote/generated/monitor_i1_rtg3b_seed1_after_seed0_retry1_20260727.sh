...
SOURCE_COMMIT="${1:-}"
SEED0_ID="i1_rtg3b_present80_one_to_one_formal_1000000_seed0_retry1_20260727"
SEED0_ROOT="outputs/remote_results/${SEED0_ID}"
SEED0_MONITOR="outputs/remote_results_incomplete/${SEED0_ID}_monitor"
GATE_ROOT="outputs/local_readiness/i1_rtg3b_present80_one_to_one_formal_1000000_seed1_retry1_launch_gate_20260727"
MONITOR_ROOT="outputs/remote_results_incomplete/i1_rtg3b_seed1_after_seed0_retry1_20260727_monitor"
SEED1_MONITOR_SCRIPT="configs/remote/generated/monitor_i1_rtg3b_present80_one_to_one_formal_1000000_seed1_retry1_20260727.sh"
SEED1_TMUX="i1_rtg3b_present80_formal_seed1_retry1_monitor"
...
while true; do
  if [[ -f "${SEED0_ROOT}/gate.local.json" ]]; then
    ...
    if required_seed0_artifacts_ready; then
      if UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/check-runtime-spn-present-rtg3b-seed1-launch \
        ...
        >> "${MONITOR_ROOT}/gate.log" 2>> "${MONITOR_ROOT}/gate_stderr.log"; then
      ...
      gate_decision="$(python -c "import json,pathlib; p=pathlib.Path(r'${GATE_ROOT}/gate.json'); print(json.loads(p.read_text(encoding='utf-8')).get('decision','')) if p.is_file() else print('')" 2>/dev/null || true)"
      if [[ "${gate_decision}" == "innovation1_rtg3b_present_seed1_source_not_live_verified" ]]; then
        echo "$(timestamp) seed1_launch_gate_waiting_for_live_sha" >> "${MONITOR_ROOT}/monitor.log"
        sleep 300
        continue
      fi
