...
set SCHEDULE_CMD=%SCHEDULE_ROOT%\i1_rtg2b_s1.cmd
set LAUNCH_LOG_DIR=%RUNS_ROOT%\launcher_logs
set TASK_NAME=I1_RTG2B_SKINNY64_S1_GPU%PHYSICAL_GPU%
set PY=F:\Anaconda\envs\DWT\torch310\python.exe
set SEED0_ID=i1_rtg2b_skinny64_general_gf2_scale_262144_seed0_20260724
...
"%PY%" -c "import json,pathlib,sys; p=pathlib.Path(r'%SEED0_GATE%'); g=json.loads(p.read_text(encoding='utf-8')) if p.is_file() else {}; ok=g.get('run_id') == '%SEED0_ID%' and g.get('phase') == 'rtg2b' and g.get('seed') == 0 and g.get('status') == 'pass' and g.get('decision') == 'innovation1_rtg2b_skinny_scale_seed0_supported'; sys.exit(0 if ok else 1)" || exit /b 4
...
set RUN_CMD=%SOURCE_ROOT%\configs\remote\generated\run_i1_rtg2b_skinny64_general_gf2_scale_262144_seed1_20260724.cmd
