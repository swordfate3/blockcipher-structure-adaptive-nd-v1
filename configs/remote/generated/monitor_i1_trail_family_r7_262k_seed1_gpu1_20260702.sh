...
PLAN="configs/experiment/innovation1/innovation1_spn_present_trail_family_r7_262k_seed1.json"
...
while true; do
  ...
  if [[ "${result_rows}" -ge "${EXPECTED_ROWS}" ]]; then
    ...
    env UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/postprocess-trail-family \
      --plan "${PLAN}" \
      --results "${result_file}" \
