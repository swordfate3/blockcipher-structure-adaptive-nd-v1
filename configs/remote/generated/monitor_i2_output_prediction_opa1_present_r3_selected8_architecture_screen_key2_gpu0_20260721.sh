...
RUN_ID="i2_output_prediction_opa1_present_r3_selected8_architecture_screen_key2_gpu0_20260721"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opa1-architecture-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); revision=(root/'git_revision.txt').read_text().strip(); assert revision=='${SOURCE_COMMIT}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==2 and meta['config']['mode']=='phase_a_screen' and meta['selected_msb_indices']==[0,2,8,10,32,34,40,42] and cache['status']=='complete' and cache['completed_rows']==196608 and len(checkpoints)==5" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      scripts/plot-innovation2-selected-output-architecture \
