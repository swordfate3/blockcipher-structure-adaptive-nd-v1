...
if not "%SEED%"=="4" if not "%SEED%"=="5" goto invalid_arguments

set RUN_ID=i1_gift64_cross_spn_source_seed_r5_65536_seed%SEED%
...
set REMOTE_CONFIG=configs\remote\innovation1_gift64_cross_spn_source_seed_r5_65536_seed%SEED%_gpu%PHYSICAL_GPU%_20260715.json
set SOURCE_MANIFEST=configs\experiment\innovation1\innovation1_spn_cross_spn_typed_transfer_seed1_sources.json
...
if not exist "outputs\local_smoke\i1_present_cross_spn_source_seed_r5_8192_seed1\checkpoints\row0003_present_cross_spn_typed_cell_shuffled_seed1.pt" goto missing_source_assets
...
:maybe_joint
set SEED4_ID=i1_gift64_cross_spn_source_seed_r5_65536_seed4
set SEED5_ID=i1_gift64_cross_spn_source_seed_r5_65536_seed5
set SEED4_ROOT=%RUNS_ROOT%\%SEED4_ID%
set SEED5_ROOT=%RUNS_ROOT%\%SEED5_ID%
set JOINT_ID=i1_gift64_cross_spn_source_seed_r5_65536_joint_seed4_seed5
set JOINT_ROOT=%RUNS_ROOT%\%JOINT_ID%
set JOINT_LOCK=%JOINT_ROOT%\joint.lock
if not exist "%SEED4_ROOT%\results\gate.json" exit /b 0
if not exist "%SEED5_ROOT%\results\gate.json" exit /b 0
...
"%PY%" scripts\gate-cross-spn-source-seed-robustness-joint ^
  --seed4-gate "%SEED4_ROOT%\results\gate.json" ^
  --seed5-gate "%SEED5_ROOT%\results\gate.json" ^
...
copy /Y "%SEED4_ROOT%\results\gate.json" "%JOINT_ARCHIVE%\seed4_gate.json" > nul || exit /b 1
copy /Y "%SEED5_ROOT%\results\gate.json" "%JOINT_ARCHIVE%\seed5_gate.json" > nul || exit /b 1
copy /Y "%SEED4_ROOT%\results\results.jsonl" "%JOINT_ARCHIVE%\seed4_results.jsonl" > nul || exit /b 1
copy /Y "%SEED5_ROOT%\results\results.jsonl" "%JOINT_ARCHIVE%\seed5_results.jsonl" > nul || exit /b 1
