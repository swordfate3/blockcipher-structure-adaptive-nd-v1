...
REMOTE_DIR="i2_opc1_hybrid_k6_20260722"
ARCHIVE_NAME="i2_opc1_hybrid_k6_20260722"
...
OPB1_GATE_SHA256="776a43a7e0b13e9db17d825ec20f83fc6ce54ca8a36408849d7007a8ec46a549"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opc1-hybrid-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,hashlib,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); opb1=root/'opb1_gate.json'; revision=(root/'git_revision.txt').read_text().strip(); history=list(csv.DictReader((root/'history.csv').open())); assert revision=='${SOURCE_COMMIT}' and hashlib.sha256(opb1.read_bytes()).hexdigest()=='${OPB1_GATE_SHA256}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==6 and meta['config']['mode']=='spn_rescnn_hybrid' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==400 and len(checkpoints)==4" \
    ...
    MPLCONFIGDIR=/tmp/mplconfig UV_CACHE_DIR=/tmp/uv-cache uv run python \
      -m blockcipher_nd.cli.plot_innovation2_selected_output_spn_rescnn_hybrid \
