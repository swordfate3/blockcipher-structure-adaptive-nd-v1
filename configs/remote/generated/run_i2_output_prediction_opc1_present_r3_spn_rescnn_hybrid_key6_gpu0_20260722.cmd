...
set RUN_ID=i2_output_prediction_opc1_present_r3_spn_rescnn_hybrid_key6_gpu0_20260722
set REMOTE_DIR=i2_opc1_hybrid_k6_20260722
set ARCHIVE_NAME=i2_opc1_hybrid_k6_20260722
...
set OPB1_GATE=%RUNS_ROOT%\i2_opb1_tbneck_k4_20260722\results\gate.json
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_opc1_present_r3_spn_rescnn_hybrid_key6.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-opc1-present-r3-spn-rescnn-hybrid-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_opc1_present_r3_spn_rescnn_hybrid_key6_gpu0_20260722.json
...
"%PY%" -c "import hashlib,json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); gate_path=pathlib.Path(r'%OPB1_GATE%'); gate=json.loads(gate_path.read_text(encoding='utf-8')); digest=hashlib.sha256(gate_path.read_bytes()).hexdigest(); common=plan['common']; assert digest==cfg['opb1_gate_sha256']==plan['opb1_authority']['gate_sha256']; assert gate['run_id']==plan['opb1_authority']['run_id'] and gate['status']=='hold' and gate['decision']=='innovation2_topology_bottleneck_not_attributed' and gate['metrics']['attribution_passed'] is False and gate['metrics']['attributed_bit_count']==0 and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()); assert cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['expected_result_rows']==32 and cfg['expected_history_rows']==400 and cfg['expected_checkpoints']==4 and cfg['seed']==6 and cfg['sample_classification'] is False and common['sample_classification'] is False and len(plan['rows'])==4; print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed

echo started>"%LOG_DIR%\%RUN_ID%_started.marker"
"%PY%" scripts\run-innovation2-selected-output-spn-rescnn-hybrid ^
...
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opb1_gate.json summary.json gate.json checkpoint_manifest.json) do if not exist "%RESULTS_DIR%\%%F" goto incomplete_results
...
for %%F in (selected8_rescnn_anchor_true_output_final.pt selected8_spn_rescnn_exact_p_true_output_final.pt selected8_spn_rescnn_wrong_p_true_output_final.pt selected8_spn_rescnn_exact_p_label_shuffle_final.pt) do if not exist "%RESULTS_DIR%\models\%%F" goto incomplete_results
set RESULT_LINES=0
...
"%PY%" -c "import csv,hashlib,json,pathlib; root=pathlib.Path(r'%RESULTS_DIR%'); gate=json.loads((root/'gate.json').read_text(encoding='utf-8')); meta=json.loads((root/'metadata.json').read_text(encoding='utf-8')); cache=json.loads((root/'data'/'cache_metadata.json').read_text(encoding='utf-8')); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text(encoding='utf-8')); history=list(csv.DictReader((root/'history.csv').open(encoding='utf-8'))); opb1=root/'opb1_gate.json'; assert hashlib.sha256(opb1.read_bytes()).hexdigest()=='776a43a7e0b13e9db17d825ec20f83fc6ce54ca8a36408849d7007a8ec46a549' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==6 and meta['config']['mode']=='spn_rescnn_hybrid' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==400 and len(checkpoints)==4 and all(row['sha256'] for row in checkpoints)" || goto incomplete_results

if not exist "%ARCHIVE_DIR%" mkdir "%ARCHIVE_DIR%" || goto failed
for %%F in (results.jsonl progress.jsonl history.csv metadata.json opb1_gate.json summary.json gate.json checkpoint_manifest.json) do copy /Y "%RESULTS_DIR%\%%F" "%ARCHIVE_DIR%\%%F" > nul || goto failed
