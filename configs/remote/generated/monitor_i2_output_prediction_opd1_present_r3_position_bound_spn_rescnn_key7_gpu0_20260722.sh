...
REMOTE_DIR="i2_opd1_poshead_k7_retry1_20260722"
ARCHIVE_NAME="i2_opd1_poshead_k7_retry1_20260722"
...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-opd1-poshead-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if [[ -f "${MONITOR_ROOT}/${RUN_ID}/logs/${RUN_ID}_result_branch_pushed.marker" ]]; then
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import csv,hashlib,json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); meta=json.loads((root/'metadata.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); checkpoints=json.loads((root/'checkpoint_manifest.json').read_text()); opc1=root/'opc1_gate.json'; opn1=root/'opn1_gate.json'; revision=(root/'git_revision.txt').read_text().strip(); history=list(csv.DictReader((root/'history.csv').open())); assert revision=='${SOURCE_COMMIT}' and hashlib.sha256(opc1.read_bytes()).hexdigest()=='${OPC1_GATE_SHA256}' and hashlib.sha256(opn1.read_bytes()).hexdigest()=='${OPN1_GATE_SHA256}' and gate['status'] in {'pass','hold'} and all(gate['protocol_checks'].values()) and all(gate['execution_checks'].values()) and meta['sample_classification'] is False and meta['config']['seed']==7 and meta['config']['mode']=='position_bound_head' and cache['status']=='complete' and cache['completed_rows']==196608 and len(history)==500 and len(checkpoints)==5" \
