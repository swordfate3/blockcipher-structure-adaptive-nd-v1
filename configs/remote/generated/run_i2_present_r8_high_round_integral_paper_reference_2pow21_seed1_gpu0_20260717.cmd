...
set RUN_ID=i2_present_r8_high_round_integral_paper_reference_2pow21_seed1_gpu0_20260717
...
set PLAN=configs\experiment\innovation2\innovation2_present_r8_high_round_integral_paper_reference_2pow21_seed1.json
set REMOTE_CONFIG=configs\remote\innovation2_present_r8_high_round_integral_paper_reference_2pow21_seed1_gpu0_20260717.json
...
for %%F in (results.jsonl progress.jsonl dataset_summary.json fixed_baselines.json gate.json validation.json curves.svg history.csv memory_preflight.json) do if not exist "%RESULTS_DIR%\%%F" goto incomplete_results
set RESULT_LINES=0
...
"%PY%" -c "import json,pathlib; root=pathlib.Path(r'%RESULTS_DIR%'); gate=json.loads((root/'gate.json').read_text()); validation=json.loads((root/'validation.json').read_text()); memory=json.loads((root/'memory_preflight.json').read_text()); assert all(gate['paper_reference_plan_checks'].values()); assert validation['status']=='pass'; assert gate['artifact_validation']['status']=='pass'; assert memory['status']=='pass'; assert memory['batch_size']==2000; assert memory['max_peak_reserved_fraction']<=0.9" || goto incomplete_results

if not exist "%ARCHIVE_DIR%" mkdir "%ARCHIVE_DIR%"
for %%F in (results.jsonl progress.jsonl dataset_summary.json fixed_baselines.json gate.json validation.json curves.svg history.csv memory_preflight.json) do copy /Y "%RESULTS_DIR%\%%F" "%ARCHIVE_DIR%\%%F" > nul || goto failed
...
git commit -m "results: %RUN_ID% paper reference" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto failed
