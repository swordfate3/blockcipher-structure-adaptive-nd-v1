...
set RUN_ID=i2_output_prediction_opa1_present_r3_selected8_architecture_screen_key2_gpu0_20260721
...
set PLAN=configs\experiment\innovation2\innovation2_output_prediction_opa1_present_r3_selected8_architecture_screen_key2.json
set DOC_PLAN=docs\experiments\innovation2-output-prediction-opa1-present-r3-selected8-architecture-screen-plan.md
set REMOTE_CONFIG=configs\remote\innovation2_output_prediction_opa1_present_r3_selected8_architecture_screen_key2_gpu0_20260721.json
...
"%PY%" -c "import json,pathlib; cfg=json.loads(pathlib.Path(r'%REMOTE_CONFIG%').read_text(encoding='utf-8')); plan=json.loads(pathlib.Path(r'%PLAN%').read_text(encoding='utf-8')); common=plan['common']; assert cfg['train_total_rows']==131072 and cfg['test_total_rows']==65536 and cfg['expected_result_rows']==40 and cfg['expected_history_rows']==500 and cfg['epochs']==100 and cfg['batch_size']==250 and cfg['seed']==2 and cfg['sample_classification'] is False and cfg['selected_msb_indices']==[0,2,8,10,32,34,40,42] and common['sample_classification'] is False and common['target']=='eight_preregistered_msb_first_true_ciphertext_bits' and len(plan['rows'])==5 and all(row.get('parameters') for row in plan['rows']); print('status=pass')" > "%LOG_DIR%\%RUN_ID%_readiness.txt" 2> "%LOG_DIR%\%RUN_ID%_readiness_stderr.txt" || goto failed

echo started>"%LOG_DIR%\%RUN_ID%_started.marker"
"%PY%" scripts\run-innovation2-selected-output-architecture ^
  --mode phase_a_screen ^
  --run-id "%RUN_ID%" ^
  --device cuda ^
...
for %%F in (selected8_mlp_true_output_final.pt selected8_lstm_true_output_final.pt selected8_rescnn_true_output_final.pt selected8_transformer_true_output_final.pt selected8_present_spn_true_output_final.pt) do if not exist "%RESULTS_DIR%\models\%%F" goto incomplete_results
set RESULT_LINES=0
