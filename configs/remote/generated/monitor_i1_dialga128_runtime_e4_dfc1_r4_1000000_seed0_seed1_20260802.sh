...
python3 -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_dialga_dfc1_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || exit 7
...
retrieve_archive() {
  ...
  local staging="${MONITOR_ROOT}/staging_${mode}_${RUN_ID}_$(date +%s)_$$"
