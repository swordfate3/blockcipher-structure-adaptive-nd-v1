...
REMOTE="lxy-a6000"
RUN_ID="i1_dialga128_runtime_e4_dmc1_r4_65536_seed0_seed1_20260731"
...
python -c "import json,pathlib,sys; g=json.loads(pathlib.Path(r'${LAUNCH_GATE}').read_text(encoding='utf-8')); ok=g.get('status') == 'pass' and g.get('decision') == 'innovation1_dialga_dmc1_remote_launch_authorized' and g.get('should_ssh') is True and g.get('ssh_allowed') is True and g.get('launch_authorized') is True and g.get('source_commit') == '${SOURCE_COMMIT}'; sys.exit(0 if ok else 1)" || exit 7
...
retrieve_archive() {
  ...
  UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-dialga-r4-dmc1 --gate "${destination}/gate.local.json" --output "${destination}/curves.svg" --report "${destination}/plot_report.json" >> "${MONITOR_ROOT}/plot.log" 2>> "${MONITOR_ROOT}/plot_stderr.log" || return 1
