...
retrieve_archive() {
  local staging
  staging=$(mktemp -d /tmp/i2-r8-paper-reference-seed1-retrieval.XXXXXX) || return 1
...
while true; do
  ...
  if compgen -G "${MONITOR_ROOT}/${RUN_ID}/logs/*result_branch_pushed.marker" > /dev/null; then
    ...
    EXPECTED_SOURCE_COMMIT=$(tr -d '\r\n' < "${DESTINATION}/source_expected_commit.txt")
    test -n "${EXPECTED_SOURCE_COMMIT}" || exit 3
    UV_CACHE_DIR=/tmp/uv-cache uv run python scripts/plot-results \
      ...
      --title "创新2：PRESENT 8轮论文参考规模近似（2^21总训练行，seed1）" \
    ...
    UV_CACHE_DIR=/tmp/uv-cache uv run python -c \
      "import json,pathlib; root=pathlib.Path(r'${DESTINATION}'); gate=json.loads((root/'gate.json').read_text()); local_gate=json.loads((root/'gate.local.json').read_text()); data=json.loads((root/'dataset_summary.json').read_text()); cache=json.loads((root/'cache_metadata.json').read_text()); memory=json.loads((root/'memory_preflight.json').read_text()); assert all(gate['paper_reference_plan_checks'].values()); assert all(local_gate['paper_reference_plan_checks'].values()); assert local_gate['readjudication']['source_revision_matches_expected']; assert gate['artifact_validation']['status']=='pass'; assert data['status']=='pass'; assert memory['status']=='pass' and memory['batch_size']==2000 and memory['max_peak_reserved_fraction']<=0.9; assert len(cache)==3 and all(v['status']=='complete' for v in cache.values())" \
