...
set RUN_ID=i1_uknit_r5_k1ca_invariant_autond_262144_s3s4_20260803
...
set PLAN=configs\experiment\innovation1\innovation1_uknit_r5_k1ca_invariant_autond_closeout_262144_seed3_seed4.csv
set REMOTE_CONFIG=configs\remote\innovation1_uknit_k1ca_invariant_autond_closeout_262144_seed3_seed4_gpu0_20260803.json
...
if errorlevel 1 goto failed
"%PY%" scripts\gate-uknit-r5-k1ca-closeout --plan "%PLAN%" --results "%RESULTS_DIR%\results.jsonl" --progress "%LOG_DIR%\progress.jsonl" --checkpoint-root "%CHECKPOINT_DIR%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --output-root "%RESULTS_DIR%" > "%LOG_DIR%\%RUN_ID%_gate_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_gate_stderr.txt"
if errorlevel 1 goto failed
...
"%PY%" scripts\package-uknit-r5-k1ca-closeout --run-root "%RUN_ROOT%" --source-root "%SOURCE_ROOT%" --source-commit-file "%LOG_DIR%\%RUN_ID%_git_revision.txt" --expected-source-commit-file "%RUN_ROOT%\source_expected_commit.txt" --archive-root "%ARCHIVE_DIR%" > "%LOG_DIR%\%RUN_ID%_package_stdout.txt" 2> "%LOG_DIR%\%RUN_ID%_package_stderr.txt"
if errorlevel 1 goto failed
...
git commit -m "results: %RUN_ID% paper closeout" > "%LOG_DIR%\%RUN_ID%_result_branch_commit.txt" 2>&1 || goto result_sync_failed
