...
class StoppedActionsCallback(Protocol):
    def __call__(self, decision: str) -> list[dict[str, str]]: ...
...
class FourRoleGateSpec:
    ...
    allowed_seed_layouts: tuple[tuple[int, ...], ...]
...
def _threshold_errors(
    *,
    seed0_architecture_margin: Any,
    seed0_topology_margin: Any,
    seed0_representation_margin: Any,
    joint_architecture_margin: Any,
    joint_control_margin: Any,
    allow_none_seed0_architecture_margin: bool,
) -> list[str]:
    thresholds = {
        "seed0_architecture_margin": seed0_architecture_margin,
        "seed0_topology_margin": seed0_topology_margin,
    ...
    for field, value in thresholds.items():
        if (
            field == "seed0_architecture_margin"
            and value is None
            and allow_none_seed0_architecture_margin
        ):
            continue
...
def _expected_seed_errors(expected_seeds: Any, *, spec: FourRoleGateSpec) -> list[str]:
    ...
    if expected_seeds not in spec.allowed_seed_layouts:
        return [
            f"expected_seeds={expected_seeds!r} "
            f"allowed_seed_layouts={spec.allowed_seed_layouts!r}"
        ]
    return []
...
def _protocol_errors(
    ...
    *,
    ...
    for index, row in enumerate(rows):
        ...
        for field in ("parameter_count", "trainable_parameter_count"):
            ...
            if not _is_positive_integer(value):
                ...
                capacity_counts[field].add(value)
    ...
    if len(capacity_counts["parameter_count"]) != 1:
        errors.append(
            f"{spec.capacity_label}_parameter_count_mismatch "
            f"values={sorted(capacity_counts['parameter_count'])}"
        )
    if len(capacity_counts["trainable_parameter_count"]) != 1:
        errors.append(
            f"{spec.capacity_label}_trainable_parameter_count_mismatch "
            f"values={sorted(capacity_counts['trainable_parameter_count'])}"
        )
    return errors
