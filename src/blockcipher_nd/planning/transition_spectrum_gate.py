...
DEFAULT_TRANSITION_SPECTRUM_MARGIN = 0.001
...
DEFAULT_SHUFFLED_MODEL = "bit_transition_spectrum_shuffled_p"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gate bit-transition-spectrum routes against an InvP anchor."
    )
...
def gate_transition_spectrum_result(
    results_path: Path,
    *,
    ...
    decision = "invalid"
    action = "fix_transition_spectrum_gate_inputs_before_claim"
    ...
    if not errors and margin_vs_anchor is not None:
        ...
        if _at_least(margin_vs_anchor, margin) and calibration_not_worse and shuffled_not_matching:
            ...
            interpretation = (
                "bit-transition-spectrum features beat the InvP anchor by the required "
                "diagnostic margin and beat the shuffled-P control without "
            ...
            interpretation = (
                "bit-transition-spectrum features are at or above the InvP anchor but "
                "below the required margin, or lack a strong shuffled-control margin"
            )
        else:
            decision = "stop_transition_spectrum_route"
            action = "record_tied_or_negative_evidence_and_switch_hypothesis"
            interpretation = (
                "bit-transition-spectrum features do not beat the InvP anchor, calibration regresses, "
                "or the shuffled-P control matches/exceeds the true route; do not scale this route"
            )
    ...
    return {
        ...
        "claim_scope": (
            "bit-transition-spectrum diagnostic gate; not paper-scale, formal, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = gate_transition_spectrum_result(
        args.results,
