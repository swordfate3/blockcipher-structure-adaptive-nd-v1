...
DEFAULT_ACTIVE_AUXILIARY_MARGIN = 0.001
FLOAT_TOLERANCE = 1e-12
DEFAULT_ANCHOR_MODEL = "present_nibble_invp_only_spn_only"
DEFAULT_CANDIDATE_MODEL = "present_nibble_invp_active_aux_spn_only"
DEFAULT_SHUFFLED_MODEL = "present_nibble_invp_active_aux_shuffled_targets"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gate active-pattern auxiliary-head routes against an InvP anchor.")
...
def gate_active_auxiliary_result(
    results_path: Path,
    *,
    ...
    margin_vs_shuffled = _delta(candidate_auc, shuffled_auc)
    calibrated_delta_vs_anchor = _delta(candidate_calibrated, resolved_anchor_calibrated)

    decision = "invalid"
    action = "fix_active_auxiliary_gate_inputs_before_claim"
    ...
    if not errors and margin_vs_anchor is not None:
        ...
        shuffled_margin_strong = margin_vs_shuffled is not None and _at_least(margin_vs_shuffled, margin)
        if _at_least(margin_vs_anchor, margin) and calibration_not_worse and shuffled_margin_strong:
            decision = "support_active_auxiliary_route"
            action = "prepare_262k_seed1_confirmation_before_1m_scale"
            interpretation = (
                "active-pattern auxiliary supervision beats the InvP anchor by the required diagnostic margin "
                "and separates from the shuffled-target control"
            )
        elif _at_least(margin_vs_anchor, 0.0) and calibration_not_worse and candidate_beats_shuffled:
            decision = "weak_active_auxiliary_signal"
            action = "prepare_262k_variance_check_only_if_branch_selected"
            interpretation = (
                "active-pattern auxiliary supervision is at or above the InvP anchor but below the required margin, "
                "or lacks a strong shuffled-target-control margin"
            )
        else:
            decision = "stop_active_auxiliary_route"
            action = "record_tied_or_negative_evidence_and_switch_hypothesis"
            interpretation = (
                "active-pattern auxiliary supervision does not beat the InvP anchor, calibration regresses, "
                "or the shuffled-target control matches/exceeds the true route; do not scale this route"
            )
    ...
    return {
        ...
        "claim_scope": "active-pattern auxiliary diagnostic gate; not paper-scale, formal, or breakthrough evidence",
        "errors": errors,
        "warnings": warnings,
    }
...
def _metrics(row: dict[str, Any]) -> dict[str, float | None]:
    ...
    return {
        ...
        "auxiliary_loss": _optional_float(metrics.get("auxiliary_loss")),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = gate_active_auxiliary_result(
        args.results,
