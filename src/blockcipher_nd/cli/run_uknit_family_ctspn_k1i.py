...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run K1-I exact GF(2) Boolean-view training and the frozen "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-I run_id must remain frozen as {RUN_ID}")
    ...
    if not all(source_checks.values()):
        raise ValueError(f"K1-I source bindings failed: {source_checks}")

    k1h_preflight = read_json(args.k1h_root / "preflight.json")
    source_cache_root = Path(str(k1h_preflight["source_cache_root"]))
    ...
    if build_tasks(train_args) != candidate_tasks:
        raise ValueError("K1-I training parser drifted from the frozen plan")
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-I did not produce four training rows")
    cache_checks = cache_reuse_checks(read_jsonl(args.output_root / "progress.jsonl"))
    if not all(cache_checks.values()):
        raise ValueError(f"K1-I source cache reuse failed: {cache_checks}")
    ...
    progress(
        ...
        "k1i_frozen_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1i_panel(
        candidate_tasks=candidate_tasks,
...
def checkpoint_manifest(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if row.get("model") != CANDIDATE_MODEL or not checkpoint.is_file():
            raise ValueError("K1-I missing candidate checkpoint")
        entries.append(
            {
...
def validate_resume_root(args: argparse.Namespace, source_cache_root: Path) -> None:
    ...
    if (
        ...
        raise ValueError("K1-I resume root does not match frozen training")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-I output root already contains run artifacts")
