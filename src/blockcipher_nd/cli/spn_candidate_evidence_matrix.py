...
from blockcipher_nd.tasks.innovation1.spn_candidate_evidence import args_from_config, run_candidate_evidence


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a small candidate-trail JSON matrix and write one JSONL row per matrix row."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    rows = run_candidate_matrix(plan)
...
def run_candidate_matrix(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(_matrix_rows(plan)):
        ...
        if row_type != "candidate":
            raise SystemExit(f"unsupported candidate matrix row_type: {row_type}")
        result = run_candidate_evidence(args_from_config(row_config))
...
def _load_matrix_plan(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise SystemExit(f"candidate matrix config must be a JSON object: {path}")
    if "rows" not in data:
        raise SystemExit("candidate matrix config must contain rows")
    if "output" not in data:
        raise SystemExit("candidate matrix config must contain output")
    return data
...
def _common_plan(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(common, dict):
        raise SystemExit("candidate matrix common must be a JSON object")
    return dict(common)
...
def _matrix_rows(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list) or not rows:
        raise SystemExit("candidate matrix rows must be a non-empty list")
    if not all(isinstance(row, dict) for row in rows):
        raise SystemExit("candidate matrix rows must be JSON objects")
    return rows
...
def _required_str(config: dict[str, Any], key: str) -> str:
    ...
    if value in {None, ""}:
        raise SystemExit(f"candidate matrix row missing {key}")
    return str(value)
...
def _required_float(config: dict[str, Any], key: str) -> float:
    ...
    if value is None:
        raise SystemExit(f"candidate matrix row missing numeric {key}")
    return value
