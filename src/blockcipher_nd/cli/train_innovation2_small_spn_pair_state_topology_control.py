...
from blockcipher_nd.tasks.innovation2.small_spn_pair_state_topology_control import (
    adjudicate_pair_state_topology_control,
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E41 fair-topology control for no-triangle pair state."
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--source-root", required=True, type=Path)
    parser.add_argument("--e40-root", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_gate = _read_json(args.e40_root / "gate.json")
    source_rows = _read_jsonl(args.e40_root / "results.jsonl")
    ...
    for row_spec in matrix:
        ...
        trained["result"]["task"] = (
            "innovation2_small_spn_pair_state_topology_control"
        )
    ...
    gate = adjudicate_pair_state_topology_control(
        config, readiness, contract, source_gate, source_rows, control_rows
    )
    true_rows = [
        row
    ...
    metadata = {
        "run_id": config.run_id,
        "task": "innovation2_small_spn_pair_state_topology_control",
        "source_run": data["source_gate"].get("run_id"),
        "e40_run": source_gate.get("run_id"),
