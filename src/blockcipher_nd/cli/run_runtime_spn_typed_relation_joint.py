...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train and adjudicate one shared Runtime-E4 typed-relation checkpoint "
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = load_and_validate_typed_relation_config(args.config)
    ...
    progress(
        "typed_relation_joint_experiment_start",
        {
    ...
    gate = adjudicate_typed_relation_experiment(payload, project_root=project_root)
    write_joint_artifacts(payload=payload, gate=gate, output_root=args.output_root)
    progress(
        "typed_relation_joint_experiment_done",
        {
            "run_id": config["run_id"],
            "status": gate["status"],
