...
from blockcipher_nd.tasks.innovation1.runtime_spn_typed_relation_readiness import (
    build_typed_relation_readiness,
)
...
REGRESSION_TESTS = (
    ...
    "tests/test_runtime_spn_typed_relation.py",
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fail-closed Runtime-SPN typed-relation readiness gate."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest, gate, smoke = build_typed_relation_readiness(
        run_id=args.run_id,
