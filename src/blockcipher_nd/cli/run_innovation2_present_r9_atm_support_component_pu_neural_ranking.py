...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E99 local PRESENT r9 support-component-disjoint PU neural ranking."
    )
    parser.add_argument("--run-id", default=RUN_ID)
    parser.add_argument("--results-root", required=True, type=Path)
    parser.add_argument("--e98c-gate", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = PuNeuralRankingConfig(run_id=args.run_id)
    ...
    _write_progress(
        progress,
        "run_start",
        {"device": args.device, "training": True, "remote": False},
    )

    groups = load_relation_groups(args.results_root)
    folds = build_neural_folds(groups, config)
    _write_progress(progress, "fold_pools_built", folds["metrics"])
    training = train_neural_matrix(
        config,
        folds,
        device=args.device,
        progress_callback=lambda event, payload: _write_progress(progress, event, payload),
    )
    e98c_gate = json.loads(args.e98c_gate.read_text(encoding="utf-8"))
    gate = adjudicate_neural_ranking(
        config,
        fold_audit=folds,
        training=training,
        e98c_gate=e98c_gate,
        e98c_gate_hash=sha256(args.e98c_gate),
    )
