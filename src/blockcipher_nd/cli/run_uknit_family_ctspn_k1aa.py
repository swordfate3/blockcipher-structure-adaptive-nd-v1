...
from blockcipher_nd.tasks.innovation1.uknit_family_ctspn_k1w import K1Q_CACHE_ROOT
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the uKNIT K1-AA virtual-slot projection diagnostic."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-AA run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-AA plan does not match the frozen protocol")
    ...
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-AA training parser drifted from the frozen plan")
...
def prepare_bound_cache_link(cache_root: Path) -> None:
    ...
    source = K1Q_CACHE_ROOT / "uknit64"
    if not source.is_dir():
        raise ValueError(f"missing K1-AA bound source cache root: {source}")
...
def require_fresh_output_root(path: Path) -> None:
    if path.exists() and any(
        ...
        raise ValueError("K1-AA output root already contains run artifacts")
...
def write_comparison_csv(
    ...
    fields = (
        "seed",
        "virtual_slot_exact_auc",
        "virtual_slot_wrong_sbox_auc",
        "k1t_invariant_auc",
        "k1y_exact_auc",
        "retention_threshold",
