...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run K1-N exact-composition training and its frozen K1-M-anchor "
            "three-split semantic-control panel."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-N run_id must remain frozen as {RUN_ID}")
    ...
    if not readiness_valid(readiness):
        ...
        return 4
    dataset_manifest = read_jsonl(args.k1m_root / "dataset_manifest.jsonl")
    source_controls = evaluation_map(
        read_jsonl(args.k1m_root / "controls.jsonl")
    )
    source_cache_root = Path(
        str(read_json(args.k1m_root / "preflight.json")["source_cache_root"])
    )
    ...
    if build_tasks(train_args) != candidate_tasks:
        raise ValueError("K1-N training parser drifted from frozen plan")
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-N did not produce four training rows")
    ...
    progress(
        ...
        "k1n_frozen_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1n_panel(
    ...
    validation = {
        ...
        "status": (
            "pass"
            if all(gate["protocol_checks"].values()) and all(cache_checks.values())
            else "fail"
        ),
...
def readiness_valid(gate: dict[str, Any]) -> bool:
    return (
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-N output root already contains run artifacts")
