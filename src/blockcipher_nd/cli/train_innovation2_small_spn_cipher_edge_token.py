...
from blockcipher_nd.tasks.innovation2.small_spn_cipher_edge_token import (
    adjudicate_cipher_edge_token,
    cipher_edge_token_training_matrix,
    measure_cipher_edge_token_contract,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E35 small-SPN Cipher Edge-Token Transformer audit."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    contract = measure_cipher_edge_token_contract(data)
    matrix = cipher_edge_token_training_matrix(config)
    _write_progress(
        progress_path,
        "run_start",
        {
    ...
    for row_spec in matrix:
        ...
        trained["result"]["task"] = "innovation2_small_spn_cipher_edge_token"
    ...
    gate = adjudicate_cipher_edge_token(config, readiness, contract, rows)
    metadata = {
        "run_id": config.run_id,
        "task": "innovation2_small_spn_cipher_edge_token",
        "mode": config.mode,
        ...
        "config": {
            ...
            "transformer_layers": config.blocks,
            "heads": config.heads,
            "feedforward_dim": config.hidden_dim * 2,
