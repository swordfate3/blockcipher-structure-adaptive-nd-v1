...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the fail-closed four-row K1-B native-endpoint diagnostic and "
    ...
    parser.add_argument("--k1-controls", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-B run_id must remain frozen as {RUN_ID}")
    ...
    manifests, readiness = build_k1b_readiness(
        tasks=tasks,
        k1_gate=k1_gate,
        k1a_gate=k1a_gate,
    )
    ...
    preflight = {
        ...
        "k1_gate": str(args.k1_gate),
        "k1a_gate": str(args.k1a_gate),
        "k1_controls": str(args.k1_controls),
    }
    ...
    if build_tasks(train_args) != tasks:
        raise ValueError("K1-B training parser drifted from the frozen plan")
    train_main(train_argv)
    training_rows = _read_jsonl(args.output_root / "results.jsonl")
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-B did not produce four training rows")
    validation_datasets = {}
    for task in tasks:
        if task["model_key"] == CANDIDATE_MODEL:
            validation_datasets[(str(task["cipher_key"]), int(task["seed"]))] = (
                prepare_task_inputs(task, train_args).validation_dataset
            )
    _progress(
    ...
    controls = evaluate_k1b_controls(
        tasks=tasks,
        training_rows=training_rows,
        validation_datasets=validation_datasets,
        k1_controls=_read_jsonl(args.k1_controls),
        device=args.device,
    )
    ...
    _write_checkpoint_manifest(
        training_rows, args.output_root / "checkpoint_manifest.json"
    )
    write_history_csv(
...
def _require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-B output root already contains run artifacts")


def _write_checkpoint_manifest(rows: Sequence[Mapping[str, Any]], path: Path) -> None:
    entries = []
    for row in rows:
