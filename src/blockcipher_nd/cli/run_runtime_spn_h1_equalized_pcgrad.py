...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train H1 Runtime-E4 with equalized fixed-order PCGrad and perform "
            "zero-fine-tuning RECTANGLE evaluation."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = load_and_validate_h1_equalized_pcgrad_config(
        args.config,
    ...
    progress("h1_equalized_pcgrad_start", {"run_id": config["run_id"]})
    payload = run_h1_equalized_pcgrad(
        config=config,
        config_sha256_value=config_sha256(args.config),
