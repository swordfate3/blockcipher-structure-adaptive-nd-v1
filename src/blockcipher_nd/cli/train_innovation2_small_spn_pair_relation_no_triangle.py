...
from blockcipher_nd.tasks.innovation2.small_spn_pair_relation_no_triangle import (
    adjudicate_no_triangle_ablation,
    no_triangle_training_matrix,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E40 same-budget no-triangle SPN-PRR ablation."
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--source-root", required=True, type=Path)
    parser.add_argument("--phase-a-root", required=True, type=Path)
    parser.add_argument("--topology-gate-root", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    topology_gate = _read_json(args.topology_gate_root / "gate.json")
    ...
    matrix = no_triangle_training_matrix(config)
    _write_progress(
        progress_path,
        "run_start",
        {
            ...
            "phase_a_run_id": source_gate.get("run_id"),
            "topology_gate_run_id": topology_gate.get("run_id"),
            **contract,
        },
    )
    ...
    for row_spec in matrix:
        row_id = row_spec.row_id.replace(
            "pair_relation_reasoner", "pair_relation_no_triangle"
        )
        _write_progress(progress_path, "training_row_start", {"row_id": row_id})
        trained = train_pair_relation_row(
            config, row_spec, data, processor_mode="local"
        )
        trained["result"]["task"] = (
            "innovation2_small_spn_pair_relation_no_triangle"
        )
        rows.append(trained["result"])
        history.extend(trained["history"])
        torch.save(trained["state_dict"], checkpoint_root / f"{row_id}.pt")
        _write_progress(
            progress_path,
            "training_row_done",
            {
    ...
    gate = adjudicate_no_triangle_ablation(
        config,
        readiness,
        contract,
        rows,
    ...
    metadata = {
        "run_id": config.run_id,
        "task": "innovation2_small_spn_pair_relation_no_triangle",
        "mode": config.mode,
        "source_run": data["source_gate"].get("run_id"),
        "phase_a_run": source_gate.get("run_id"),
        "topology_gate_run": topology_gate.get("run_id"),
        ...
        "config": {
            "hidden_dim": config.hidden_dim,
            "path_rank": config.path_rank,
            "shared_triangle_blocks": 0,
            "shared_local_blocks": 1,
