...
from blockcipher_nd.tasks.innovation2.integral_geometry_diversity import (
    ...
    run_cyclic_geometry_diversity_audit,
)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit PRESENT r7 cyclic active-geometry kernel diversity."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_cyclic_geometry_diversity_audit(
        config,
...
def render_geometry_svg(
    ...
    ordered = sorted(rows, key=lambda row: int(row["start_nibble"]))
    starts = np.asarray([int(row["start_nibble"]) for row in ordered], dtype=np.int64)
    ...
    with plt.rc_context(
        ...
        figure, axes = plt.subplots(1, 2, figsize=(14.5, 6.9))
        ...
        figure.suptitle(
            "创新2 E14：PRESENT 7轮循环滑动活动几何的输出 kernel",
            x=0.07,
            y=0.965,
            ha="left",
            fontsize=15.5,
        ...
        figure.text(
            0.07,
            0.89,
            "每个结构活动4个连续 nibble；起点0..15；同一组128把密钥和完整 2^16 明文集合。",
            ha="left",
            va="top",
            fontsize=10.0,
            color="#526070",
        ...
        dimension_axis.set_xlabel("起始 nibble")
        dimension_axis.set_ylabel("kernel 维数")
        dimension_axis.set_xticks(starts)
        dimension_axis.grid(True, color="#E5E7EB", linewidth=0.8)
        dimension_axis.legend(loc="upper right", frameon=False, fontsize=8.5)
        ...
        for start, signature_id, dimension in zip(
            starts, signature_values, joint, strict=True
        ):
        ...
        signature_axis.set_xlabel("起始 nibble")
        signature_axis.set_ylabel("签名类别编号")
        signature_axis.set_xticks(starts)
