...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train E46 PRESENT MSPN readiness smoke matrix."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    sources = load_readiness_sources(args.atlas_root, args.e44_root, args.e45_root)
    source_checks = validate_readiness_sources(sources)
    _write_progress(progress_path, "sources_validated", source_checks)
    if not all(source_checks.values()):
        raise ValueError("E43/E44/E45 source validation failed")
    contract = measure_mspn_contract(config, sources["atlas"])
    _write_progress(progress_path, "model_contract_complete", contract)
    matrix = train_readiness_matrix(config, sources["atlas"])
    ...
    gate = adjudicate_e46(config, source_checks, contract, matrix)
    metadata = {
