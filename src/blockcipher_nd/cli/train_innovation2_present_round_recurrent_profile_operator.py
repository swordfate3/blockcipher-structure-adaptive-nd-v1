...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E71 round-recurrent profile operator readiness."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = RoundRecurrentProfileConfig(run_id=args.run_id)
    ...
    source_checks = validate_profile_operator_sources(sources, strict=True)
    if not all(
        value for value in source_checks.values() if isinstance(value, bool)
    ):
        raise ValueError("E65 profile source validation failed")
    _write_progress(progress, "sources_validated", source_checks)
    contract = measure_round_recurrent_contract(config, sources)
    training = train_round_recurrent_matrix(config, sources, args.output_root)
    ...
    metadata = {
        ...
        "profile_source_run_id": sources["profile_gate"].get("run_id"),
        "profile_source_hashes": sources["source_hashes"],
        "remote_scale": False,
