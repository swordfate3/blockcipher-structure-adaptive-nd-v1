...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the uKNIT/Dialga K1-W compact-invariant diagnostic."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-W run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-W plan does not match the frozen protocol")
    require_fresh_output_root(args.output_root)
    readiness_source_sha256 = None
    if args.readiness_artifact is None:
        readiness = build_readiness(tasks=tasks, device=args.device)
    else:
        readiness = json.loads(args.readiness_artifact.read_text(encoding="utf-8"))
        readiness_source_sha256 = file_sha256(args.readiness_artifact)
        current_cache_rows = source_cache_manifest()
        current_bindings = source_binding_checks(current_cache_rows)
        if not all(current_bindings.values()):
            raise ValueError("K1-W source bindings changed after readiness")
        if (
            readiness.get("run_id") != RUN_ID
            or readiness.get("status") != "pass"
            or readiness.get("optimizer_step_authorized") is not True
            or not all(readiness.get("protocol_checks", {}).values())
        ):
            raise ValueError("K1-W readiness artifact is not optimizer-authorizing")
    ...
    write_json(
        args.output_root / "preflight.json",
        {
            **readiness,
            "plan": str(args.plan),
            "source_cache_mode": "read_only_directory_symlinks",
            "readiness_source": (
                None
                if args.readiness_artifact is None
                else str(args.readiness_artifact)
            ),
            "readiness_source_sha256": readiness_source_sha256,
        },
    )
    write_json(
        ...
        {
            "run_id": RUN_ID,
            "rows": readiness["source_cache_manifest"],
        },
    )
    ...
    prepare_bound_cache_links(args.output_root / "cache")
    train_argv = training_argv(args)
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-W training parser drifted from the frozen plan")
    ...
    write_comparison_csv(
        args.output_root / "comparison.csv",
...
def prepare_bound_cache_links(cache_root: Path) -> None:
    cache_root.mkdir(parents=True, exist_ok=False)
    sources = {
        "uknit64": K1Q_CACHE_ROOT / "uknit64",
        "dialga128": D1_CACHE_ROOT / "dialga128",
    }
    for cipher, source in sources.items():
        if not source.is_dir():
            raise ValueError(f"missing K1-W bound source cache root: {source}")
        (cache_root / cipher).symlink_to(source, target_is_directory=True)
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-W output root already contains run artifacts")
