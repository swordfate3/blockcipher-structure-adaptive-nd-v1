...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the H1 Runtime-E4 candidate with per-task representation "
            "gradient L2 equalization and zero-fine-tuning RECTANGLE evaluation."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    progress("h1_gradient_equalization_start", {"run_id": config["run_id"]})
