...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run K1-K bounded topology edge-residual training and the frozen "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-K run_id must remain frozen as {RUN_ID}")
    ...
    anchor_manifest = read_json(args.k1i_root / "checkpoint_manifest.json")
    ...
    if not all(source_checks.values()):
        raise ValueError(f"K1-K source bindings failed: {source_checks}")

    k1i_preflight = read_json(args.k1i_root / "preflight.json")
    source_cache_root = Path(str(k1i_preflight["source_cache_root"]))
    ...
    if build_tasks(train_args) != candidate_tasks:
        raise ValueError("K1-K training parser drifted from the frozen plan")
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-K did not produce four training rows")
    cache_checks = cache_reuse_checks(read_jsonl(args.output_root / "progress.jsonl"))
    if not all(cache_checks.values()):
        raise ValueError(f"K1-K source cache reuse failed: {cache_checks}")
    ...
    progress(
        ...
        "k1k_frozen_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1k_panel(
        candidate_tasks=candidate_tasks,
...
def validate_resume_root(args: argparse.Namespace, source_cache_root: Path) -> None:
    ...
    if (
        ...
        raise ValueError("K1-K resume root does not match frozen training")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-K output root already contains run artifacts")
