...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit K1-BY10 source-bundle partition collisions."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-BY10 run_id must remain frozen as {RUN_ID}")
    config = load_and_validate_config(args.config)
    if args.device != config["audit"]["device"]:
        raise ValueError("K1-BY10 device drifted from frozen config")
    readiness = build_readiness(config)
    if readiness.get("execution_authorized") is not True:
        raise ValueError(f"K1-BY10 readiness failed: {readiness}")
    ...
    progress(args.output_root / "progress.jsonl", "k1by10_audit_start")
    result_rows, partition, model_metadata = evaluate(config)
    source_after = authority_digests(config)
    gate, effects = adjudicate(
        config,
    ...
    summary = {
        ...
        "supported_target_cells": gate["supported_target_cells"],
        "partition_summary": gate["partition_summary"],
        "extreme_effects": gate["extreme_effects"],
    ...
    write_jsonl(args.output_root / "effects.jsonl", effects)
    write_jsonl(args.output_root / "partition_cells.jsonl", partition)
    write_effect_csv(args.output_root / "cell_effects.csv", effects)
    write_json(
        args.output_root / "partition_summary.json",
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-BY10 output root already contains audit artifacts")


def write_effect_csv(
    path: Path,
    ...
    fields = (
        ...
        "anchor_correct_auc",
        "anchor_affine_auc",
        "anchor_runtime_margin",
        "candidate_correct_auc",
        "candidate_affine_auc",
        "candidate_runtime_margin",
...
__all__ = ["main", "parse_args", "write_effect_csv"]
