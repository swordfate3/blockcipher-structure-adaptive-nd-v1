...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E83 SKINNY-64 r4-only sparse profile readiness."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Skinny64SparseProfileReadinessConfig(run_id=args.run_id)
    ...
    full_sources = load_skinny_profile_sources(args.profile_root)
    source_checks = validate_skinny_profile_sources(full_sources)
    if not all(source_checks.values()):
        raise ValueError("E82 SKINNY profile source validation failed")
    ...
    source_checks["r4_features_match_full_columns_39_to_51"] = bool(
        (
            sources["prefix_features"]
            == full_sources["prefix_features"][:, :, 39:52]
        ).all()
    )
    _write_progress(progress, "sources_validated", source_checks)
    ridges = evaluate_sparse_ridges(sources)
    _write_progress(progress, "deterministic_ridges_complete", ridges)
    contract = measure_sparse_operator_contract(config, sources)
    _write_progress(progress, "model_contract_complete", contract)
    training = train_sparse_profile_matrix(config, sources, args.output_root)
    ...
    metadata = {
        ...
        "architecture": "Sparse Linear-Layer Profile Operator",
