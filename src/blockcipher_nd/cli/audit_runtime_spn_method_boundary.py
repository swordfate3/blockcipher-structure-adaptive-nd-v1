...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Freeze the evidence-backed capability boundary of the runtime-SPN "
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_method_boundary_audit(
        config=config,
        project_root=project_root,
        progress_callback=progress,
    )
    write_method_boundary_artifacts(payload=payload, output_root=args.output_root)
    gate = payload["gate"]
    progress(
        "run_done",
        {
