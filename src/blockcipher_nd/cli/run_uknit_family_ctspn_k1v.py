...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the uKNIT K1-V four-to-sixteen-pair diagnostic."
    )
    parser.add_argument("--plan", required=True, type=Path)
    parser.add_argument("--k1t-gate", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-V run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-V plan does not match the frozen protocol")
    anchor_gate = read_json(args.k1t_gate)
    readiness = build_readiness(
        tasks=tasks,
        anchor_gate=anchor_gate,
        anchor_gate_sha256=file_sha256(args.k1t_gate),
    )
    if readiness.get("optimizer_step_authorized") is not True:
        raise ValueError(f"K1-V readiness failed: {readiness}")
    ...
    write_json(
        args.output_root / "preflight.json",
        {
            **readiness,
            ...
            "k1t_gate": str(args.k1t_gate),
            "k1t_gate_sha256": file_sha256(args.k1t_gate),
        },
    )
    ...
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-V training parser drifted from the frozen plan")
    ...
    write_comparison_csv(
        args.output_root / "pair_comparison.csv", comparison_rows(gate)
    )
    write_history_csv(
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-V output root already contains run artifacts")
...
def write_comparison_csv(
    ...
    fields = (
        "seed",
        "exact_16pair_auc",
        "exact_4pair_anchor_auc",
        "exact_16pair_minus_exact_4pair",
        "wrong_sbox_16pair_auc",
        "exact_minus_wrong_sbox",
        "invariant_16pair_auc",
