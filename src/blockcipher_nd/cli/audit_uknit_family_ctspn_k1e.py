...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay four frozen K1-D checkpoints on their exact train and "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-E run_id must remain frozen as {RUN_ID}")
    ...
    source_checks = validate_k1e_source(
        tasks=tasks,
            ...
            json.dumps(
                {
                    "run_id": RUN_ID,
                    "status": "invalid",
                    "decision": "innovation1_uknit_family_ctspn_k1e_source_invalid",
    ...
    _progress(args.output_root / "progress.jsonl", "k1e_preflight_passed")

    train_argv = _cache_argv(args)
    train_args = parse_train_args(train_argv)
    if build_tasks(train_args) != tasks:
        raise ValueError("K1-E cache parser drifted from the frozen K1-D plan")
    datasets = {}
    ...
    if not all(source_checks.values()):
        raise ValueError("K1-E source cache was not reused exactly")

    _progress(
        args.output_root / "progress.jsonl",
        "k1e_frozen_replay_start",
    ...
    gate = adjudicate_k1e(rows=rows, source_checks=source_checks)
...
def _require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-E output root already contains audit artifacts")
