...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the uKNIT r5 K1-BS neural architecture ablation."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-BS run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-BS plan does not match the frozen protocol")
    readiness = build_readiness(tasks=tasks)
    if readiness.get("optimizer_step_authorized") is not True:
        raise ValueError(f"K1-BS readiness failed: {readiness}")
    ...
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-BS training parser drifted from the frozen plan")
    ...
    write_comparison_csv(
        args.output_root / "architecture_comparison.csv", comparison_rows(gate)
    )
    write_history_csv(
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-BS output root already contains run artifacts")
...
def write_comparison_csv(
    ...
    fields = (
        "seed",
        *(f"{name}_auc" for name in ARCHITECTURES),
        "best_generic_architecture",
        "best_generic_auc",
