...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit PRESENT r7 high16 inactive-context kernel diversity."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_context_diversity_audit(
        config,
        ...
        json.dumps(
            {
                ...
                "nontrivial_joint_kernel_contexts": result["gate"][
                    "nontrivial_joint_kernel_contexts"
                ],
...
def render_context_svg(
    ...
    with plt.rc_context(
        ...
        figure.suptitle(
            "创新2 E16：PRESENT 7轮高16位积分的固定上下文审计",
            x=0.07,
            y=0.965,
            ha="left",
            fontsize=15.5,
        ...
        figure.text(
            0.07,
            0.89,
            "活动位固定为48..63；只改变其余48位常量；每个 context 使用同一组128把密钥。",
            ha="left",
            va="top",
            fontsize=10.0,
            color="#526070",
        ...
        dimension_axis.axvline(
            ...
            linestyle="--",
            linewidth=1.1,
            label="全零 Hwang anchor",
        )
        legend_handles, legend_labels = dimension_axis.get_legend_handles_labels()
        figure.legend(
            legend_handles,
            legend_labels,
            loc="upper left",
        ...
        dimension_axis.set_xticks(context_ids)
        ...
        for context_id, signature_id, dimension in zip(
            context_ids, signature_values, joint, strict=True
        ):
            signature_axis.annotate(
                f"d={dimension}",
                (context_id, signature_id),
                xytext=(0, 8),
        ...
        signature_axis.set_xticks(context_ids)
        ...
        figure.text(
            ...
            (
                ...
                "这是结构条件输出性质审计，不是神经训练，也不是积分/随机二分类。"
            ),
            ha="left",
            va="bottom",
            fontsize=9.5,
