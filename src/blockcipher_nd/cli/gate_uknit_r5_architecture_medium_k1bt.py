...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gate the remote uKNIT K1-BT medium architecture result.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-BT run_id must remain frozen as {RUN_ID}")
...
def _write_history_csv(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    for index, row in enumerate(rows, start=1):
        label = f"run{index}: {row.get('cipher', '')} r{row.get('rounds', '')} {row.get('model', '')} seed{row.get('seed', '')}"
        for item in row.get("history", []):
            if isinstance(item, dict):
                records.append({
                    "run_index": index, "run_label": label,
                    "cipher": row.get("cipher", ""), "model": row.get("model", ""),
                    "rounds": row.get("rounds", ""), "seed": row.get("seed", ""),
                    "samples_per_class": row.get("samples_per_class", ""),
                    "pairs_per_sample": row.get("pairs_per_sample", ""), **item,
                })
