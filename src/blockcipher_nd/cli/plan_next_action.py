...
from blockcipher_nd.planning.next_action_readiness import (
    launch_artifact_checklist,
    launch_artifacts_passed,
    next_action_readiness_errors,
    readiness_reports_for_next_action,
)
...
def plan_next_action(summary_path: Path) -> dict[str, Any]:
    ...
    readiness_reports = readiness_reports_for_next_action(
        next_action,
        config_keys=(("stage_a", "stage_a_remote_config"), ("primary", "launch_remote_config")),
    )
    ...
    launch_artifacts_pass = bool(readiness_reports) and all(
        launch_artifacts_passed(report.get("launch_artifacts", {})) for report in readiness_reports
    )
    ...
    *,
    ...
    if not readiness_pass:
        return [
            ...
            *launch_artifact_checklist(readiness_reports),
        ]
    configs = [report["config"] for report in readiness_reports]
    artifact_steps = launch_artifact_checklist(readiness_reports)
...
def _default_plan_doc_for_branch(branch: str) -> str | None:
    return {
        ...
        "stop_trail_family_route": "docs/experiments/innovation1-active-pattern-auxiliary-head-plan.md",
    }.get(branch)
