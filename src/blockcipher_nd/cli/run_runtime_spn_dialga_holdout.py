...
from blockcipher_nd.tasks.innovation1.runtime_spn_dialga_holdout import (
    ...
    write_dialga_holdout_artifacts,
    write_dialga_holdout_readiness_artifacts,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run A8 Dialga whole-cipher holdout readiness or the frozen "
            "two-seed diagnostic."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.readiness_only:
        progress("dialga_holdout_readiness_start", {})
    ...
    progress("dialga_holdout_start", {"run_id": config["run_id"]})
    ...
    gate = adjudicate_dialga_holdout(payload)
    write_dialga_holdout_artifacts(
        payload=payload,
        gate=gate,
        output_root=args.output_root,
