...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit frozen H1/A2/A3 Runtime-E4 representation accessibility "
            "without neural training or RECTANGLE rows."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    progress("h1_representation_accessibility_start", {"run_id": config["run_id"]})
    payload = run_h1_representation_accessibility_audit(
        config=config,
        project_root=project_root,
    ...
    progress(
        "h1_representation_accessibility_done",
        {
