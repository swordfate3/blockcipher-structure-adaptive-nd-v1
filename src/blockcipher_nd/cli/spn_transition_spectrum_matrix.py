...
from blockcipher_nd.tasks.innovation1.spn_transition_spectrum import args_from_config, run_transition_spectrum


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a bit-transition-spectrum JSON matrix and write one JSONL row per matrix row."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    rows = run_transition_spectrum_matrix(plan)
...
def run_transition_spectrum_matrix(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(_matrix_rows(plan)):
        ...
        if row_type != "candidate":
            raise SystemExit(f"unsupported transition-spectrum matrix row_type: {row_type}")
        result = run_transition_spectrum(args_from_config(row_config))
...
def _load_matrix_plan(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise SystemExit(f"transition-spectrum matrix config must be a JSON object: {path}")
    if "rows" not in data:
        raise SystemExit("transition-spectrum matrix config must contain rows")
    if "output" not in data:
        raise SystemExit("transition-spectrum matrix config must contain output")
    return data
...
def _common_plan(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(common, dict):
        raise SystemExit("transition-spectrum matrix common must be a JSON object")
    return dict(common)
...
def _matrix_rows(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list) or not rows:
        raise SystemExit("transition-spectrum matrix rows must be a non-empty list")
    if not all(isinstance(row, dict) for row in rows):
        raise SystemExit("transition-spectrum matrix rows must be JSON objects")
    return rows
...
def _base_result_fields(config: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "feature_route": config.get("feature_route", "bit_transition_spectrum"),
    }
...
def _required_str(config: dict[str, Any], key: str) -> str:
    ...
    if value in {None, ""}:
        raise SystemExit(f"transition-spectrum matrix row missing {key}")
    return str(value)
...
def _required_float(config: dict[str, Any], key: str) -> float:
    ...
    if value is None:
        raise SystemExit(f"transition-spectrum matrix row missing numeric {key}")
    return value
