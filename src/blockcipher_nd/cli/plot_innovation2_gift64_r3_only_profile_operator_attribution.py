...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render E78 GIFT-64 r3-only seed0 attribution."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    render_gift_r3_attribution(summary, args.output)
    print(json.dumps({"output": str(args.output)}, sort_keys=True))
    return 0


def render_gift_r3_attribution(summary: dict[str, Any], output: Path) -> None:
    gate = summary["gate"]
    ...
    margins = (
        ...
        gate["metrics"]["true_minus_e77_topology_ridge"],
    )
    decisions = {
        "innovation2_gift64_r3_only_neural_gain_attributed": (
            "真实GIFT P的30轮质量与拓扑归因通过；可运行相同协议seed1。"
        ),
    ...
    with plt.rc_context(
        ...
        figure.text(
            0.075,
            0.955,
            "创新2 E78：GIFT-64真实P-layer的30轮seed0神经增益能否正式归因",
            ha="left",
            va="top",
            fontsize=15.0,
            fontweight="bold",
        ...
        figure.text(
            0.075,
            0.848,
            "E77真实P topology-expanded ridge AUC=0.743496，作为信息范围公平的确定性安全锚点。",
            ha="left",
            va="top",
            fontsize=9.8,
            color="#526070",
        ...
        axes[1].set_ylim(min(-0.08, min(margins) - 0.04), max(0.22, max(margins) + 0.05))
        ...
        figure.text(
            ...
            (
                ...
                f"最佳epoch={rows['true']['best_epoch']}；E75 observed坐标=620。"
            ),
            ha="left",
            va="bottom",
            fontsize=9.4,
        ...
        figure.text(
            0.075,
            0.053,
            "证据范围：GIFT-64四轮严格unit-profile的30轮seed0归因；不是双seed、高轮、跨密码、攻击或SOTA。",
            ha="left",
            va="bottom",
            fontsize=9.0,
            color="#526070",
