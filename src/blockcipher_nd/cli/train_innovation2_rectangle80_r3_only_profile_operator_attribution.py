...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E90 RECTANGLE-80 r3-only 30-epoch seed0 attribution."
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--profile-root", required=True, type=Path)
    parser.add_argument("--e89-root", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = Rectangle80R3AttributionConfig(run_id=args.run_id)
    ...
    e89_source = load_e89_source(args.e89_root)
    e89_checks = validate_e89_source(e89_source)
    if (
        not all(profile_checks.values())
        or not all(model_order_checks.values())
        or not all(e89_checks.values())
    ):
        raise ValueError("E88/E89 source or cell-major replay validation failed")
    ...
    metadata = {
        ...
        "e89_source_run_id": e89_source["gate"].get("run_id"),
        "e89_source_hashes": e89_source["hashes"],
