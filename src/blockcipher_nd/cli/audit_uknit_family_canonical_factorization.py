...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit exact MANTIS/MIDORI canonical component factorizations for "
            "uKNIT-BC and Dialga-128 without data generation or neural training."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = load_and_validate_factorization_config(args.config)
    ...
    payload = run_canonical_factorization_audit(
        config, progress_callback=progress
    )
    write_factorization_artifacts(payload, args.output_root)
    gate = payload["gate"]
    progress(
        "run_done",
        {
