...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E60 PRESENT r2 cone panel.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    queries = list(two_round_cone_matched_queries())
    _write_json(args.output_root / "query_plan.json", {"queries": queries})
    _write_progress(
        progress,
        "worker_start",
        {"planned_queries": len(queries), "wall_clock_cap_seconds": args.wall_clock_cap_seconds},
    )
    command = [
        sys.executable,
    ...
    evaluation = evaluate_cone_matched_panel(
        config,
    ...
    metadata = {
        ...
        "rounds": 2,
        "output_exponent_hex": "0x0000000000000001",
        "two_round_input_cone": list(range(16)),
...
def _worker_main(args: argparse.Namespace) -> int:
    ...
    run_present_relation_queries(
        args.atm_root,
        rounds=2,
        query_specs=two_round_cone_matched_queries(),
        projected_key_cap=args.projected_key_cap,
        trail_model_cap=args.trail_model_cap,
