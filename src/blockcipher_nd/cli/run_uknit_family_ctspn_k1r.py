...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the K1-R independently trained uKNIT r5 cell11 neural "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-R run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-R plan does not match the frozen protocol")

    source_gate_path = args.k1q_root / "gate.json"
    source_manifest_path = args.k1q_root / "dataset_manifest.jsonl"
    source_validation_path = args.k1q_root / "validation.json"
    ...
    datasets = load_k1r_datasets(manifest_rows)
    source_checks["six_cache_payload_digests_verified"] = (
        set(datasets) == expected_dataset_keys()
    )
    if not all(source_checks.values()):
        raise ValueError(f"K1-R K1-Q source binding failed: {source_checks}")

    source_cache_root = args.k1q_root / "cache"
    train_argv = training_argv(args, source_cache_root)
    train_args = parse_train_args(train_argv)
    if build_tasks(train_args) != tasks:
        raise ValueError("K1-R training parser drifted from the frozen plan")
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-R did not produce eight training rows")
    cache_checks = cache_reuse_checks(read_jsonl(args.output_root / "progress.jsonl"))
    if not all(cache_checks.values()):
        raise ValueError(f"K1-R source cache reuse failed: {cache_checks}")
    ...
    progress(
        ...
        "k1r_three_split_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1r_panel(
        tasks=tasks,
...
def load_k1r_datasets(
    ...
    for row in manifest_rows:
        ...
        if key in datasets:
            raise ValueError(f"duplicate K1-R source cache: {key}")
        ...
        if not all(
            ...
            raise ValueError(f"missing K1-R source cache payload: {cache_dir}")
        ...
        if int(dataset.features.shape[0]) != int(row["rows"]):
            raise ValueError(f"K1-R source cache row count mismatch: {cache_dir}")
        if differential_dataset_sha256(dataset) != row.get("dataset_sha256"):
            raise ValueError(f"K1-R source cache digest mismatch: {cache_dir}")
        datasets[key] = dataset
    if set(datasets) != expected_dataset_keys():
        raise ValueError("K1-R requires exactly six cell11 K1-Q source caches")
    return datasets
...
def build_checkpoint_manifest(
    ...
    for row in rows:
        ...
        if condition not in CONTROL_CONDITIONS:
            raise ValueError("K1-R training row has an unknown model")
        key = (int(row["seed"]), condition)
        if key in observed:
            raise ValueError(f"duplicate K1-R checkpoint source: {key}")
        observed.add(key)
        checkpoint = Path(str(row["training"]["checkpoint_output"]))
        if not checkpoint.is_file():
            raise ValueError(f"missing K1-R checkpoint: {checkpoint}")
        entries.append(
            {
                "cipher_key": "uknit64",
                "seed": int(row["seed"]),
    ...
    if observed != expected_condition_keys():
        raise ValueError("K1-R checkpoint sources are incomplete")
...
def validate_resume_root(args: argparse.Namespace, source_cache_root: Path) -> None:
    ...
    if (
        ...
        raise ValueError("K1-R resume root does not match the frozen training")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-R output root already contains run artifacts")
