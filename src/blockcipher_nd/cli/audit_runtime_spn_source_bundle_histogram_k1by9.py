...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit the K1-BY9 source-bundle linear histogram."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-BY9 run_id must remain frozen as {RUN_ID}")
    config = load_and_validate_config(args.config)
    if args.device != config["audit"]["device"]:
        raise ValueError("K1-BY9 device drifted from frozen config")
    readiness = build_readiness(config)
    if readiness.get("execution_authorized") is not True:
        raise ValueError(f"K1-BY9 readiness failed: {readiness}")
    ...
    progress(args.output_root / "progress.jsonl", "k1by9_audit_start")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-BY9 output root already contains audit artifacts")
