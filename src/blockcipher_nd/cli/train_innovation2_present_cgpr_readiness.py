...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train E50 PRESENT certificate-guided pair residual readiness."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    sources = load_e50_sources(
        args.atlas_root, args.e44_root, args.e45_root, args.e49_root
    )
    source_checks = validate_e50_sources(sources)
    _write_progress(progress_path, "sources_validated", source_checks)
    if not all(source_checks.values()):
        raise ValueError("E43/E44/E45/E49 source validation failed")
    ...
    matrix = train_cgpr_matrix(config, sources["atlas"], ridge)
    for row in matrix["rows"]:
        _write_progress(
            ...
            "matrix_row_complete",
            {
                ...
                "ridge_weight_max_delta": row["ridge_weight_max_delta"],
                "training_performed": row["training_performed"],
            },
        )
    gate = adjudicate_e50(config, source_checks, contract, matrix)
    metadata = {
