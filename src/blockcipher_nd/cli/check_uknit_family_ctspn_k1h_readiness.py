...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check zero-training readiness for K1-H exact operator-tied latent propagation."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != READINESS_RUN_ID:
        raise ValueError(f"K1-H readiness run_id must be {READINESS_RUN_ID}")
    ...
    manifests, gate = build_k1h_readiness(
        ...
        k1f_gate=read_json(args.k1f_root / "gate.json"),
        k1g_gate=read_json(args.k1g_root / "gate.json"),
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-H readiness output root already contains artifacts")
