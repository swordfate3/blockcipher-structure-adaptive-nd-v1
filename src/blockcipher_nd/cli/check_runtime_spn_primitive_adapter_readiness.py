...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the fail-closed ten-check readiness gate for the five-cipher "
            "Runtime-SPN primitive-adapter experiment."
        )
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument(
        "--primitive-adapter-effect",
        choices=("additive", "multiplicative_gate"),
        default="additive",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest, gate, smoke = build_primitive_adapter_readiness(
        run_id=args.run_id,
        ...
        primitive_adapter_effect=args.primitive_adapter_effect,
