...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the K1-AK fixed-budget Midori64 r4 cell-shared S-box "
            "transition-residual panel."
        )
    )
    parser.add_argument("--plan", required=True, type=Path)
    parser.add_argument("--k1ai-root", required=True, type=Path)
    parser.add_argument("--k1aj-root", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-AK run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-AK plan does not match the frozen protocol")

    paths = {
        "k1ai_gate": args.k1ai_root / "gate.json",
        "k1ai_validation": args.k1ai_root / "validation.json",
        "k1ai_controls": args.k1ai_root / "controls.jsonl",
        "k1ai_dataset_manifest": args.k1ai_root / "dataset_manifest.jsonl",
        "k1aj_gate": args.k1aj_root / "gate.json",
        "k1aj_validation": args.k1aj_root / "validation.json",
    }
    source_digests = {name: file_sha256(path) for name, path in paths.items()}
    k1ai_gate = read_json(paths["k1ai_gate"])
    k1ai_validation = read_json(paths["k1ai_validation"])
    anchor_rows = read_jsonl(paths["k1ai_controls"])
    manifest_rows = read_jsonl(paths["k1ai_dataset_manifest"])
    k1aj_gate = read_json(paths["k1aj_gate"])
    k1aj_validation = read_json(paths["k1aj_validation"])
    source_checks = source_binding_checks(
        k1ai_gate=k1ai_gate,
        k1ai_validation=k1ai_validation,
        k1ai_controls=anchor_rows,
        dataset_manifest=manifest_rows,
        k1aj_gate=k1aj_gate,
        k1aj_validation=k1aj_validation,
    ...
    if not all(source_checks.values()):
        raise ValueError(f"K1-AK source binding failed: {source_checks}")
    if not all(control_checks.values()):
        raise ValueError(f"K1-AK structure-control binding failed: {control_checks}")

    k1ai_preflight = read_json(args.k1ai_root / "preflight.json")
    source_cache_root = Path(str(k1ai_preflight["source_cache_root"]))
    train_argv = training_argv(args, source_cache_root)
    train_args = parse_train_args(train_argv)
    if build_tasks(train_args) != tasks:
        raise ValueError("K1-AK training parser drifted from the frozen plan")
    ...
    if args.resume_evaluation:
        ...
        write_json(
            args.output_root / "preflight.json",
            {
                ...
                "k1ai_root": str(args.k1ai_root),
                "k1aj_root": str(args.k1aj_root),
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-AK did not produce eight training rows")
    cache_checks = cache_reuse_checks(read_jsonl(args.output_root / "progress.jsonl"))
    if not all(cache_checks.values()):
        raise ValueError(f"K1-AK source cache reuse failed: {cache_checks}")
    ...
    progress(
        ...
        "k1ak_three_split_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1ak_panel(
        tasks=tasks,
...
def build_checkpoint_manifest(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if condition not in CONTROL_CONDITIONS:
            raise ValueError("K1-AK training row has an unknown model")
        key = (int(row["seed"]), condition)
        if key in observed:
            raise ValueError(f"duplicate K1-AK checkpoint source: {key}")
        observed.add(key)
        checkpoint = Path(str(row["training"]["checkpoint_output"]))
        if not checkpoint.is_file():
            raise ValueError(f"missing K1-AK checkpoint: {checkpoint}")
        entries.append(
            {
                "cipher_key": "midori64",
                "seed": int(row["seed"]),
    ...
    if observed != expected_condition_keys():
        raise ValueError("K1-AK checkpoint sources are incomplete")
...
def validate_resume_root(args: argparse.Namespace, source_cache_root: Path) -> None:
    ...
    if (
        ...
        raise ValueError("K1-AK resume root does not match the frozen training")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-AK output root already contains run artifacts")
