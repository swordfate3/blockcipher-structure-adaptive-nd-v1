...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Check K1-M effective-gate-0.05 geometry, gradients, controls, "
            "and frozen K1-K/K1-L source bindings before training."
        )
    )
    parser.add_argument("--plan", required=True, type=Path)
    parser.add_argument("--k1k-root", required=True, type=Path)
    parser.add_argument("--k1l-root", required=True, type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != READINESS_RUN_ID:
        raise ValueError(f"K1-M readiness run_id must remain {READINESS_RUN_ID}")
    require_fresh_output_root(args.output_root)
    paths = {
        "k1k_gate": args.k1k_root / "gate.json",
        "k1k_checkpoint_manifest": args.k1k_root / "checkpoint_manifest.json",
        "k1k_dataset_manifest": args.k1k_root / "dataset_manifest.jsonl",
        "k1k_controls": args.k1k_root / "controls.jsonl",
        "k1l_gate": args.k1l_root / "gate.json",
    }
    digests = {name: file_sha256(path) for name, path in paths.items()}
    k1k_gate = read_json(paths["k1k_gate"])
    k1l_gate = read_json(paths["k1l_gate"])
    dataset_manifest = read_jsonl(paths["k1k_dataset_manifest"])
    datasets = load_bound_datasets(dataset_manifest)
    source_checks = {
        "source_artifact_digests_exact": digests == EXPECTED_SOURCE_DIGESTS,
        "k1k_hold_decision_exact": (
            k1k_gate.get("status") == "hold"
            and k1k_gate.get("decision") == K1K_DECISION
            and all(k1k_gate.get("protocol_checks", {}).values())
        ),
        "k1l_starvation_decision_exact": (
            k1l_gate.get("status") == "pass"
            and k1l_gate.get("decision") == K1L_DECISION
            and all(k1l_gate.get("protocol_checks", {}).values())
        ),
    }
...
def require_fresh_output_root(path: Path) -> None:
    if path.exists() and any(
        ...
        raise ValueError("K1-M readiness output already exists")
