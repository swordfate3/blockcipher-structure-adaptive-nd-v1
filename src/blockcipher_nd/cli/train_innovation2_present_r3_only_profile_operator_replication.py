...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E73 r3-only seed1 replication.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = R3OnlyReplicationConfig(run_id=args.run_id)
    training_config = config.as_training_config()
    args.output_root.mkdir(parents=True, exist_ok=True)
    progress = args.output_root / "progress.jsonl"
    progress.write_text("", encoding="utf-8")
    _write_progress(progress, "run_start", {"epochs": 30, "seed": 1, "training": True})
    seed0_source = load_r3_seed0_source(args.seed0_root)
    seed0_checks = validate_r3_seed0_source(seed0_source)
    ...
    if not all(seed0_checks.values()) or not all(
        ...
        raise ValueError("E65/E73 seed0 source validation failed")
    ...
    _write_progress(
        progress,
        "sources_validated",
        {"seed0": seed0_checks, "profile": source_checks},
    )
    ...
    for row in training["trained_rows"]:
        ...
        row["task"] = "innovation2_present_r3_only_profile_replication"
    metadata = {
        "run_id": config.run_id,
        "task": "innovation2_present_r3_only_profile_replication",
        "config": serializable_config(config),
        "seed0_source_run_id": seed0_source["gate"].get("run_id"),
        "seed0_hashes": seed0_source["hashes"],
