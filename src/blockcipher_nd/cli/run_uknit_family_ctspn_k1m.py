...
from blockcipher_nd.cli.run_uknit_family_ctspn_k1k import (
    cache_reuse_checks,
    checkpoint_manifest,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run K1-M effective-gate-0.05 training and its frozen K1-K-anchor "
            "three-split topology-control panel."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-M run_id must remain frozen as {RUN_ID}")
    candidate_tasks = read_tasks(args.plan)
    ...
    if not readiness_valid(readiness):
        ...
        return 4
    dataset_manifest = read_jsonl(args.k1k_root / "dataset_manifest.jsonl")
    source_controls = evaluation_map(
        read_jsonl(args.k1k_root / "controls.jsonl")
    )
    source_cache_root = Path(
        str(read_json(args.k1k_root / "preflight.json")["source_cache_root"])
    )
    ...
    if build_tasks(train_args) != candidate_tasks:
        raise ValueError("K1-M training parser drifted from frozen plan")
    ...
    if args.resume_evaluation:
        ...
        write_json(
            args.output_root / "preflight.json",
            {
                ...
                "readiness_gate_sha256": file_sha256(
                    args.readiness_root / "gate.json"
                ),
                "k1k_root": str(args.k1k_root),
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-M did not produce four training rows")
    cache_checks = cache_reuse_checks(
        read_jsonl(args.output_root / "progress.jsonl")
    )
    if not all(cache_checks.values()):
        raise ValueError(f"K1-M source cache reuse failed: {cache_checks}")
    ...
    progress(
        ...
        "k1m_frozen_panel_start",
        expected_rows=EXPECTED_EVALUATION_ROWS,
    )
    evaluation_rows = evaluate_k1m_panel(
    ...
    validation = {
        ...
        "checks": {**gate["protocol_checks"], **cache_checks},
...
def validate_resume_root(args: argparse.Namespace, source_cache_root: Path) -> None:
    ...
    if (
        ...
        raise ValueError("K1-M resume root does not match frozen training")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-M output root already contains run artifacts")
