...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit whether deterministic invertible synthetic GF(2) layers expand "
            "Runtime-SPN topology coverage beyond cell-relabel controls."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_topology_diversity_audit(
        config,
