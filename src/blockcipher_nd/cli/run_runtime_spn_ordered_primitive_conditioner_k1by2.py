...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the K1-BY2 fresh-seed primitive conditioner confirmation."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-BY2 run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-BY2 plan does not match the frozen protocol")
    readiness = build_readiness(tasks=tasks)
    if readiness.get("optimizer_step_authorized") is not True:
        raise ValueError(f"K1-BY2 readiness failed: {readiness}")
    ...
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-BY2 training parser drifted from the frozen plan")
...
def require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-BY2 output root already contains run artifacts")
