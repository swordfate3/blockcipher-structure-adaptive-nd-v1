...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Adjudicate the RTG1-R1c E4 view-encoder audit."
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--run-root", type=Path, required=True)
    parser.add_argument("--r1b-root", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    gate = adjudicate_runtime_spn_r1c_view_encoder(
        run_id=args.run_id,
        rows=rows,
        r1b_gate=_read_json(args.r1b_root / "gate.json"),
    )
    validation = {
        "run_id": args.run_id,
    ...
    summary = {
        "run_id": args.run_id,
        "task": "innovation1_runtime_spn_e4_view_encoder_r1c",
        "cipher": "GIFT-64",
    ...
    render_view_encoder_audit_svg(gate, args.run_root / "curves.svg")
    _append_progress(
...
def render_view_encoder_audit_svg(gate: dict[str, Any], output_path: Path) -> None:
    values = [float(gate["aucs"][role]) for role in ("separate", "shared")]
    margin = float(gate["margins"]["separate_minus_shared"])
    supported = gate["decision"] == "innovation1_runtime_spn_typed_view_identity_supported"
    conclusion = (
        "双视图独立编码有贡献；下一步在运行时共享编码器中加入view-role。"
        if supported
        else "双视图独立编码未过门；下一步审计固定cell Token-Mixer。"
    )
    with plt.rc_context(
        {
        ...
        figure.text(
            0.075,
            0.89,
            "GIFT-64 r6，2048/class训练，1024/class验证，5 epochs，seed0；两行都关闭绝对位置。",
            ha="left",
            va="top",
            color="#475569",
        )
        ...
        margin_axis.set_title("独立视图编码的贡献", loc="left", fontweight="bold")
        margin_axis.set_xlabel("独立编码 - 共享编码（AUC）")
        margin_axis.set_yticks((0,), ("视图编码贡献",))
        ...
        figure.text(
            0.075,
            0.055,
            "证据边界：同参数几何、单密码、单seed、小规模审计；不等于运行时拓扑优于控制。",
            ha="left",
            va="bottom",
            color="#334155",
        )
