...
DEFAULT_MODEL_KEY = "compressed_span_low_rank_interaction_logistic_expert"
DEFAULT_EXPERT_FAMILY = "compressed_spn_span_low_rank_interaction_summary"
DEFAULT_CANDIDATE_STATUS = "compressed_span_low_rank_interaction_screen"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit a raw compressed span-summary expert with semantic low-rank block interactions."
    )
...
def fit_compressed_span_low_rank_interaction_expert(
    *,
    ...
    common_metadata = {
        ...
        "low_rank_interaction_feature_count": int(low_rank_metadata["feature_count"]),
        "low_rank_group_ranks": low_rank_metadata["group_ranks"],
        "block_groups": _group_metadata(groups),
        "claim_scope": (
            "train-fitted raw compressed SPN span-summary low-rank block interaction expert diagnostic only; "
            "semantic block projections are train-feature unsupervised and label-independent, "
    ...
    report = {
        "status": "pass",
        "decision": (
            "compressed_span_low_rank_interaction_shuffle_train_labels_control"
            if shuffle_train_labels
            else "compressed_span_low_rank_interaction_local_screen_positive_needs_controls"
        ),
        "model_key": model_key,
        "expert_family": expert_family,
        ...
        "low_rank_projection_rank": int(rank),
        "low_rank_interaction_feature_count": int(low_rank_metadata["feature_count"]),
        "low_rank_interaction_feature_names": low_rank_metadata["feature_names"],
        "low_rank_group_ranks": low_rank_metadata["group_ranks"],
        "block_groups": _group_metadata(groups),
        "fit": {
            "steps": int(steps),
        ...
        "guardrails": [
            "low_rank_projections_must_be_train_feature_unsupervised",
            "low_rank_interactions_must_be_label_independent",
...
def _augment_with_low_rank_interactions(
    ...
    *,
    ...
    for group in [*primary_groups, *auxiliary_groups]:
        train_projection, validation_projection = _low_rank_projection(
            train_standardized[:, group["indices"]],
            validation_standardized[:, group["indices"]],
            max_rank=rank,
        )
        projections[str(group["name"])] = (train_projection, validation_projection)
        group_ranks[str(group["name"])] = int(train_projection.shape[1])

    train_interactions: list[np.ndarray] = []
    validation_interactions: list[np.ndarray] = []
    feature_names: list[str] = []
    for primary_group in primary_groups:
        primary_train, primary_validation = projections[str(primary_group["name"])]
        for auxiliary_group in auxiliary_groups:
            auxiliary_train, auxiliary_validation = projections[str(auxiliary_group["name"])]
            train_pair = _outer_products(primary_train, auxiliary_train)
            validation_pair = _outer_products(primary_validation, auxiliary_validation)
            train_interactions.append(train_pair)
            validation_interactions.append(validation_pair)
            for primary_rank in range(primary_train.shape[1]):
                for auxiliary_rank in range(auxiliary_train.shape[1]):
...
def _low_rank_projection(
    train_block: np.ndarray,
    validation_block: np.ndarray,
    *,
    max_rank: int,
) -> tuple[np.ndarray, np.ndarray]:
    actual_rank = min(max_rank, int(train_block.shape[0]), int(train_block.shape[1]))
    if actual_rank <= 0:
        raise ValueError("low-rank projection requires a non-empty block")
    _, _, vh = np.linalg.svd(train_block, full_matrices=False)
    components = vh[:actual_rank].T
    return train_block @ components, validation_block @ components


def _outer_products(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    return (left[:, :, None] * right[:, None, :]).reshape(left.shape[0], -1)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    train_artifact, validation_artifact, report = fit_compressed_span_low_rank_interaction_expert(
