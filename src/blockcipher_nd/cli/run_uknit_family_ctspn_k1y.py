...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the uKNIT K1-Y compact projection update diagnostic."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-Y run_id must remain frozen as {RUN_ID}")
    tasks = read_tasks(args.plan)
    if not candidate_protocol_frozen(tasks):
        raise ValueError("K1-Y plan does not match the frozen protocol")
    ...
    if build_tasks(parse_train_args(train_argv)) != tasks:
        raise ValueError("K1-Y training parser drifted from the frozen plan")
...
def prepare_bound_cache_link(cache_root: Path) -> None:
    ...
    if not source.is_dir():
        raise ValueError(f"missing K1-Y bound source cache root: {source}")
...
def require_fresh_output_root(path: Path) -> None:
    if path.exists() and any(
        ...
        raise ValueError("K1-Y output root already contains run artifacts")
...
def write_comparison_csv(
    ...
    fields = (
        "seed",
        "projection16x_exact_auc",
        "projection16x_wrong_sbox_auc",
        "k1w_exact_auc",
        "k1t_invariant_auc",
        "retention_threshold",
