...
from blockcipher_nd.tasks.innovation2.small_spn_pair_relation_reasoner import (
    ...
    adjudicate_pair_relation_reasoner,
    measure_pair_relation_contract,
    pair_relation_training_matrix,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E39 small-SPN pair-relation path reasoner screen."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    matrix = pair_relation_training_matrix(config)
    _write_progress(
        progress_path,
        "run_start",
        {
    ...
    gate = adjudicate_pair_relation_reasoner(config, readiness, contract, rows)
    metadata = {
        "run_id": config.run_id,
