...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit SKINNY-64/64 r7 single-active-cell kernel diversity."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_skinny_r7_single_cell_diversity_audit(
        config,
    ...
    render_single_cell_diversity_svg(
...
def render_single_cell_diversity_svg(
    rows: list[dict[str, Any]], gate: dict[str, Any], output_path: Path
) -> None:
    ordered = sorted(rows, key=lambda row: int(row["active_cell"]))
    ...
    with plt.rc_context(
        ...
        figure.suptitle(
            "创新2 E24：SKINNY-64/64 7轮单活动 cell kernel 多样性",
            x=0.07,
            y=0.965,
            ha="left",
            fontsize=15.5,
        ...
        figure.text(
            0.07,
            0.89,
            "16个活动 cell；64+64把全新密钥；左图用低值放大轴同时看清0-2维与18维anchor。",
            ha="left",
            va="top",
            fontsize=10.0,
            color="#526070",
        ...
        dimension_axis.axvline(
            ...
            linestyle="--",
            linewidth=1.2,
            label="论文 anchor cell15",
        )
        dimension_axis.set_title(
            "各活动 cell 的经验 kernel 维数（低值放大）",
            loc="left",
            fontweight="bold",
            pad=10,
        )
        dimension_axis.set_xlabel("活动 plaintext cell")
        dimension_axis.set_ylabel("kernel 维数 / nullity（symlog）")
        dimension_axis.set_xticks(x, [str(index) for index in range(16)])
        dimension_axis.set_yscale("symlog", linthresh=1.0, linscale=1.0, base=2)
        dimension_axis.set_yticks([0, 1, 2, 4, 8, 18])
        dimension_axis.set_yticklabels(["0", "1", "2", "4", "8", "18"])
        dimension_axis.set_ylim(-0.1, 20)
        dimension_axis.grid(True, color="#E5E7EB", linewidth=0.8)
        handles, labels = dimension_axis.get_legend_handles_labels()
        figure.legend(
            handles,
            labels,
            loc="upper left",
        ...
        signature_axis.axvline(CONTROL_CELL, color="#64748B", linestyle=":", linewidth=1.2)
        signature_axis.axvline(ANCHOR_CELL, color="#D97706", linestyle="--", linewidth=1.2)
        ...
        signature_axis.set_xlabel("活动 plaintext cell")
        signature_axis.set_ylabel("签名类别（0表示空 kernel）")
        signature_axis.set_xticks(x, [str(index) for index in range(16)])
        ...
        signature_axis.scatter([], [], color="#D97706", label="Hwang anchor")
        signature_axis.scatter([], [], color="#64748B", label="同预算 control")
        signature_axis.scatter([], [], color="#2563EB", label="其他 cell")
        ...
        figure.text(
            ...
            (
                ...
                f"{metrics['nontrivial_joint_kernel_structures']}/16（门槛≥6）；"
                "不同签名 "
