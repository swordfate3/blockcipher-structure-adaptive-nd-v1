...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the fail-closed four-row K1-D relative-path diagnostic and "
            "twenty-row frozen-checkpoint control panel."
        )
    )
    parser.add_argument("--plan", required=True, type=Path)
    parser.add_argument("--source-plan", required=True, type=Path)
    parser.add_argument("--k1b-gate", required=True, type=Path)
    parser.add_argument("--k1b-controls", required=True, type=Path)
    ...
    parser.add_argument(
        "--resume-controls",
        action="store_true",
        help="Reuse four completed training rows/checkpoints and resume post-training controls.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_id != RUN_ID:
        raise ValueError(f"K1-D run_id must remain frozen as {RUN_ID}")
    tasks = _tasks(args.plan)
    source_tasks = _tasks(args.source_plan)
    k1b_gate = _read_json(args.k1b_gate)
    k1c_gate = _read_json(args.k1c_gate)
    manifests, readiness = build_k1d_readiness(
    ...
    if build_tasks(train_args) != tasks:
        raise ValueError("K1-D training parser drifted from the frozen plan")
    if args.resume_controls:
        ...
        preflight = {
            ...
            "k1b_gate": str(args.k1b_gate),
            "k1b_controls": str(args.k1b_controls),
            "k1c_gate": str(args.k1c_gate),
        }
    ...
    if len(training_rows) != EXPECTED_TRAINING_ROWS:
        raise ValueError("K1-D did not produce four training rows")
    validation_datasets = {
    ...
    controls = evaluate_k1d_controls(
        tasks=tasks,
        training_rows=training_rows,
        validation_datasets=validation_datasets,
        k1b_controls=_read_jsonl(args.k1b_controls),
        device=args.device,
    )
...
def _require_fresh_output_root(path: Path) -> None:
    ...
    if path.exists() and any((path / name).exists() for name in protected):
        raise ValueError("K1-D output root already contains run artifacts")
...
def _validate_resume_root(
    *,
    ...
    if (
        ...
        raise ValueError(
            "K1-D resume root does not match the completed frozen training"
        )
    for row in rows:
        checkpoint = Path(str(row.get("training", {}).get("checkpoint_output", "")))
        if row.get("model") != CANDIDATE_MODEL or not checkpoint.is_file():
            raise ValueError(
                "K1-D resume root is missing a completed candidate checkpoint"
            )
