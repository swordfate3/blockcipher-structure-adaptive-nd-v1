...
from blockcipher_nd.tasks.innovation2.small_spn_cell_equivariance import (
    adjudicate_cell_equivariance,
    equivariance_training_matrix,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run E33-R small-SPN cell-equivariant GraphGPS audit."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    relabeling_error = measure_cell_relabeling_error(data)
    matrix = equivariance_training_matrix(config)
    _write_progress(
        progress_path,
        "run_start",
        {
    ...
    for row_spec in matrix:
        ...
        trained["result"]["task"] = "innovation2_small_spn_cell_equivariance_audit"
    ...
    gate = adjudicate_cell_equivariance(
        config, readiness, relabeling_error, rows
    )
    metadata = {
        "run_id": config.run_id,
        "task": "innovation2_small_spn_cell_equivariance_audit",
        "mode": config.mode,
