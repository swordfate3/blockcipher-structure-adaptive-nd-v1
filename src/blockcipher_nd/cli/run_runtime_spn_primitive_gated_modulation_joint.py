...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train and adjudicate one shared Runtime-E4 primitive-gated checkpoint "
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = load_and_validate_gated_config(args.config)
    ...
    gate = adjudicate_gated_modulation_experiment(payload, project_root=project_root)
    write_joint_artifacts(payload=payload, gate=gate, output_root=args.output_root)
    progress(
        "gated_joint_experiment_done",
        {
            "run_id": config["run_id"],
            "status": gate["status"],
