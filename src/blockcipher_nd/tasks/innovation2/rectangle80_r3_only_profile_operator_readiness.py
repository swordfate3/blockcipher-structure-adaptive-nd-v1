...
from blockcipher_nd.ciphers.spn.rectangle import rectangle_player
...
SOURCE_RUN_ID = "i2_rectangle80_r4_unit_balance_profile_192_structures_20260719"
SOURCE_DECISION = "innovation2_rectangle80_unit_profile_expansion_ready"
RELATION_MODES = ("independent", "true", "corrupted")
R3_SLICE = slice(26, 39)
RIDGE_LAMBDA = 1e-3
EXPECTED_PARAMETER_COUNT = 4_795
EXPECTED_MATCHED_ROWS = 3_192


@dataclass(frozen=True)
class Rectangle80R3ProfileReadinessConfig:
    run_id: str
    epochs: int = 2
    batch_size: int = 8
    hidden_dim: int = 32
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError("E89 readiness protocol is frozen")


def load_rectangle_profile_sources(root: Path) -> dict[str, Any]:
...
def validate_rectangle_profile_sources(sources: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "matched_rows_are_3192": len(rows) == EXPECTED_MATCHED_ROWS,
        "observed_edges_are_3192": int(np.sum(observed)) == EXPECTED_MATCHED_ROWS,
        "matched_edges_unique": unique
        and int(np.sum(reconstructed_observed)) == len(rows),
        ...
        "train_structures_are_144": len(train_structures) == 144,
        "validation_structures_are_48": len(validation_structures) == 48,
...
def rectangle_cell_major_order() -> np.ndarray:
    return np.asarray(
        [16 * row + column for column in range(16) for row in range(4)],
        dtype=np.int64,
    )


def rectangle_model_player() -> np.ndarray:
    physical_by_model = rectangle_cell_major_order()
    model_by_physical = np.empty(64, dtype=np.int64)
    model_by_physical[physical_by_model] = np.arange(64)
    physical_player = np.asarray(rectangle_player(), dtype=np.int64)
    return model_by_physical[physical_player[physical_by_model]]


def to_rectangle_model_order(sources: dict[str, Any]) -> dict[str, Any]:
    physical_by_model = rectangle_cell_major_order()
    model_by_physical = np.empty(64, dtype=np.int64)
    model_by_physical[physical_by_model] = np.arange(64)
    rows = [
        {**row, "output_bit": int(model_by_physical[row["output_bit"]])}
        for row in sources["matched_rows"]
    ]
    return {
        **sources,
        "profile_targets": sources["profile_targets"][:, physical_by_model].copy(),
        "profile_observed": sources["profile_observed"][:, physical_by_model].copy(),
        "prefix_features": sources["prefix_features"][:, physical_by_model].copy(),
        "matched_rows": rows,
        "physical_by_model": physical_by_model,
        "model_by_physical": model_by_physical,
    }


def validate_rectangle_model_order(
    physical: dict[str, Any], model: dict[str, Any]
) -> dict[str, bool]:
    physical_by_model = model["physical_by_model"]
    model_by_physical = model["model_by_physical"]
    mapped_rows = {
        (
            row["split"],
            row["structure_index"],
            int(model_by_physical[row["output_bit"]]),
            row["label"],
        )
        for row in physical["matched_rows"]
    }
    actual_rows = {
        (row["split"], row["structure_index"], row["output_bit"], row["label"])
        for row in model["matched_rows"]
    }
    return {
        "cell_major_order_is_permutation": np.array_equal(
            np.sort(physical_by_model), np.arange(64)
        ),
        "cell_major_inverse_matches": np.array_equal(
            model_by_physical[physical_by_model], np.arange(64)
        ),
        "targets_reorder_exact": np.array_equal(
            model["profile_targets"],
            physical["profile_targets"][:, physical_by_model],
        ),
        "observed_reorder_exact": np.array_equal(
            model["profile_observed"],
            physical["profile_observed"][:, physical_by_model],
        ),
        "prefix_reorder_exact": np.array_equal(
            model["prefix_features"],
            physical["prefix_features"][:, physical_by_model],
        ),
        "matched_rows_reorder_exact": mapped_rows == actual_rows,
        "model_player_is_permutation": np.array_equal(
            np.sort(rectangle_model_player()), np.arange(64)
        ),
    }
...
def r3_only_sources(sources: dict[str, Any]) -> dict[str, Any]:
    ...
    if prefix.shape != (192, 64, 39):
        raise ValueError("E89 requires the frozen 192x64x39 E88 prefix")
    return {**sources, "prefix_features": prefix[:, :, R3_SLICE].copy()}


def fair_topology_ridges(
    config: Rectangle80R3ProfileReadinessConfig,
    sources: dict[str, Any],
) -> dict[str, Any]:
    rows = sources["matched_rows"]
    prefix = sources["prefix_features"][:, :, R3_SLICE]
    ...
    for name, player in (("true", true_player), ("corrupted", corrupted_player)):
...
def make_rectangle_r3_model(
    ...
    *,
    ...
    if mode not in RELATION_MODES:
        raise ValueError(f"unsupported E89 mode: {mode}")
    true_player = rectangle_model_player()
    corrupted_player = topology_players(true_player[None, :], "corrupted")[0]
    selected_player = (
...
def measure_rectangle_r3_contract(
    config: Rectangle80R3ProfileReadinessConfig, sources: dict[str, Any]
) -> dict[str, Any]:
    models = {
        mode: make_rectangle_r3_model(config, mode, dropout=0.0)
    ...
    relabeled = make_rectangle_r3_model(
...
def train_rectangle_r3_matrix(
    ...
    for mode in RELATION_MODES:
        ...
        if best is None or best_state is None:
            raise RuntimeError("E89 training produced no checkpoint")
...
def adjudicate_rectangle_r3_readiness(
    config: Rectangle80R3ProfileReadinessConfig,
    ...
    corrupted_ridge = float(ridges["corrupted"]["validation_auc"])
    protocol_checks = {
        **source_checks,
        **model_order_checks,
    ...
    deterministic_checks = {
        ...
        "true_minus_corrupted_ridge_at_least_0p03": true_ridge
        - corrupted_ridge
        >= 0.03,
    }
    readiness_checks = {
        ...
        "true_minus_fair_ridge_at_least_minus_0p03": true_auc - true_ridge >= -0.03,
    }
    if not all(protocol_checks.values()):
        status = "fail"
        decision = "innovation2_rectangle80_r3_only_profile_protocol_invalid"
        action = "repair E88 source, cell-major topology, fairness, or training"
    elif not all(deterministic_checks.values()):
        status = "hold"
        decision = "innovation2_rectangle80_r3_only_topology_baseline_not_ready"
        action = "close RECTANGLE r3-only neural training and retain E88 labels"
    elif not all(readiness_checks.values()):
        status = "hold"
        decision = "innovation2_rectangle80_r3_only_profile_readiness_not_passed"
        action = "stop RECTANGLE formal training without architecture tuning"
    else:
        status = "pass"
        decision = "innovation2_rectangle80_r3_only_profile_readiness_passed"
        action = "run the frozen 30-epoch seed0 RECTANGLE attribution matrix"
    return {
        "run_id": config.run_id,
        "status": status,
        "decision": decision,
        ...
        "metrics": {
            ...
            "true_minus_corrupted_ridge": true_ridge - corrupted_ridge,
            "true_minus_independent": true_auc - independent_auc,
            "true_minus_corrupted": true_auc - corrupted_auc,
            "true_minus_fair_ridge": true_auc - true_ridge,
            "contract": contract,
        },
        "claim_scope": (
            "two-epoch local RECTANGLE-80 r4 r3-only profile-operator readiness "
            "with fair topology-expanded deterministic controls; no formal neural "
            "gain, seven-round reproduction, high-round, transfer, attack, or SOTA"
        ),
        "next_action": {
            "action": action,
