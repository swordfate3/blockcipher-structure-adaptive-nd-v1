...
from blockcipher_nd.tasks.innovation1.uknit_family_dual_path_structure_modulation_k1av import (
    ...
    build_candidate,
    load_and_validate_config as load_k1av_config,
    migrate_k1at_state,
)
...
RUN_ID = (
    "i1_uknit_family_dual_path_structure_modulation_k1aw_"
    "2048_replica0_replica1_20260729"
)
CONFIG_PATH = ROOT / (
    "configs/experiment/innovation1/"
    "innovation1_uknit_family_dual_path_structure_modulation_"
    "k1aw_2048_replica0_replica1_20260729.json"
)
EXPECTED_CONFIG_SHA256 = (
    "7481325a498160ab9d54e831ca5c6174ce815ee3b62bdf611b5efa87f3fbea78"
)
...
def load_and_validate_config(path: Path = CONFIG_PATH) -> dict[str, Any]:
    ...
    if file_sha256(path) != EXPECTED_CONFIG_SHA256:
        raise ValueError("K1-AW config digest drifted")
    if config.get("schema_version") != 1 or config.get("run_id") != RUN_ID:
        raise ValueError("K1-AW identity drifted")
    if config.get("experiment") != (
        "innovation1_uknit_family_dual_path_structure_modulation_k1aw"
    ):
        raise ValueError("K1-AW experiment name drifted")
    if config.get("replicas") != [
        ...
        raise ValueError("K1-AW replica binding drifted")
    if config.get("training") != {
        ...
        raise ValueError("K1-AW training protocol drifted")
    ...
    if controls.get("mismatch_order") != {
        ...
        raise ValueError("K1-AW mismatch order drifted")
    if {
        ...
        raise ValueError("K1-AW control protocol drifted")
    if config.get("gates") != {
        ...
        raise ValueError("K1-AW gate drifted")
    return config
...
def load_sources(
    ...
    *,
    ...
    k1av_config = load_k1av_config(project_root / str(readiness["config"]))
    ...
    k1at_config = load_k1at_config(project_root / str(anchor["config"]))
    (
        base_readiness,
        k1as_config,
        dataset_rows,
    ...
    anchors = _extract_k1at_anchors(anchor_controls)
    ...
    checks = {
        ...
        "k1av_three_structure_summaries_bound": (
            readiness_summaries.get("run_id") == readiness["run_id"]
            and len(readiness_summaries.get("rows", [])) == 3
        ),
        "k1av_two_migrations_bound": readiness_manifest.get("status") == "pass"
        and len(readiness_manifest.get("migrations", [])) == 2,
        "k1at_artifact_digests_exact": all(
            path.is_file() and file_sha256(path) == anchor["digests"][name]
            for name, path in anchor_paths.items()
        ),
        "k1at_gate_is_valid_same_budget_hold": (
        ...
        "k1at_two_training_rows_and_checkpoints_bound": len(anchor_results) == 2
        and len(anchor_manifest.get("entries", [])) == 2,
        "k1at_sixty_controls_bound": len(anchor_controls) == 60,
        "k1at_twelve_correct_descriptor_anchors_exact": set(anchors)
        == expected_anchor_keys,
        "k1at_anchor_aucs_finite": all(math.isfinite(value) for value in anchors.values()),
        **{f"k1at_source_{name}": bool(value) for name, value in inherited.items()},
    }
    return (
        base_readiness,
        k1as_config,
...
def train_shared_replicas(
    *,
    ...
    for replica_config in config["replicas"]:
        ...
        migration = migrate_k1at_state(model, k1at_initial.state_dict())
        if not all(
            (
            ...
            raise ValueError(f"K1-AW initial migration failed: {migration}")
        ...
        for epoch in range(1, EXPECTED_EPOCHS + 1):
            ...
            _append_progress(output_root / "progress.jsonl", "epoch_done", **history_row)

        if best_state is None:
            raise RuntimeError("K1-AW shared training did not select a checkpoint")
        ...
        result_rows.append(
            {
                "run_id": RUN_ID,
                "replica": replica,
                "model": "runtime_spn_ct_k1av_dual_path_structure_gate_true",
                ...
                "best_epoch": best_epoch,
                "selected_effective_gates_by_cipher": selected_gates,
                "metrics": {
                    ...
                    rows.append(
                        {
                            ...
                            "k1at_anchor_auc": anchor_auc,
                            "correct_minus_k1at_auc": correct_auc - anchor_auc,
...
def evaluate_descriptor_auc(
    *,
    ...
    if summary is None:
        raise ValueError("K1-AW descriptor summary cannot be absent")
    model.eval()
...
def adjudicate_training(
    *,
    ...
    protocol_checks = {
        ...
        "candidate_geometry_identity_and_initial_migration_exact": all(
            ...
            and row.get("initial_migration", {}).get(
                "only_final_projection_expanded"
            )
            is True
            and row.get("initial_migration", {}).get("transition_row_exact") is True
    ...
    for replica, cipher, split in sorted(expected_panels):
        ...
        anchor_auc = float(correct.get("k1at_anchor_auc", math.nan))
        prefix = f"replica{replica}_{cipher}_{split}"
        no_harm_checks[f"{prefix}_no_harm_vs_k1at"] = (
        ...
        panel_results[prefix] = {
            ...
            "k1at_anchor_auc": anchor_auc,
            "correct_minus_k1at": correct_auc - anchor_auc,
            "condition_aucs": aucs,
            "correct_minus_condition": margins,
            "no_harm_pass": no_harm_checks[f"{prefix}_no_harm_vs_k1at"],
        }
    ...
    if failed_protocol:
        status = "invalid"
        decision = "innovation1_uknit_family_k1aw_protocol_invalid"
        next_action = (
            "Repair only the failed source, migration, data, step, checkpoint, "
            "runtime or evaluation binding and replay K1-AW unchanged."
        )
    elif macro_pass and no_harm_pass and mismatch_pass:
        status = "pass"
        decision = "innovation1_uknit_family_k1aw_dual_path_training_supported"
        next_action = (
            "Retain dual-path modulation as the local family candidate and open a "
            "separate 65536/class/cipher remote cache/resume readiness audit; do "
            "not launch remotely before that gate passes."
        )
    else:
        status = "hold"
        decision = "innovation1_uknit_family_k1aw_dual_path_training_not_supported"
        if not mismatch_pass:
            next_action = (
                "Freeze both selected checkpoints and run one zero-update channel-"
                "orientation audit for correct versus mismatched descriptors; do "
                "not add pairs, data, epochs, experts or remote scale."
            )
        elif not no_harm_pass:
            next_action = (
                "Freeze both selected checkpoints and audit which learned channel "
                "causes the harmed cipher panels; do not mechanically scale."
            )
        else:
            next_action = (
                "Audit the epoch-wise two-channel trajectory against K1-AT; both "
                "replicas did not retain macro AUC, so do not increase scale."
            )
    return {
        "run_id": RUN_ID,
        "status": status,
...
def run_training(
    ...
    *,
    ...
    if not all(source_checks.values()) or not all(structure_checks.values()):
        raise ValueError(
            f"K1-AW preflight failed: source={source_checks}, "
...
def _extract_k1at_anchors(
    ...
    for row in rows:
        ...
        key = (int(row["replica"]), str(row["cipher_key"]), str(row["split"]))
        if key in anchors:
            raise ValueError(f"duplicate K1-AT anchor: {key}")
...
def _path_gate_values(
    ...
    *,
    ...
    if summary is None:
        raise ValueError("K1-AW path gate summary cannot be absent")
...
def _write_history_csv(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    if not rows:
        raise ValueError("K1-AW history rows are empty")
...
def _require_fresh_output_root(path: Path) -> None:
    if path.exists() and any(
        ...
        raise ValueError("K1-AW output already exists")
