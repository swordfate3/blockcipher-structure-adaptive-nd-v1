...
def _map_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        if key in mapped:
            raise ValueError(f"duplicate DMC1 row: {key}")
        mapped[key] = row
    if fail_closed and set(mapped) != expected_keys():
        raise ValueError("DMC1 matrix is incomplete")
    return mapped
...
def adjudicate(
    *,
    ...
    if not protocol_valid:
        status = "invalid"
        decision = "innovation1_dialga_dmc1_medium_protocol_invalid"
        scale = "no"
        next_action = (
        ...
        next_action = (
            "run Dialga DMC2 remotely at 262144/class with the same models, "
            "seeds, data protocol, pairs, epochs, and gates"
        )
    else:
        status = "hold"
        decision = "innovation1_dialga_dmc1_medium_topology_not_supported"
        scale = "no"
        next_action = (
            "inspect restored-best histories and cache equivalence; do not "
            "mechanically scale, add pairs, or move to Dialga r5"
        )
    return {
        "run_id": RUN_ID,
        "status": status,
        ...
        "claim_scope": (
            "two-seed remote 65536/class Dialga prefix-r4 medium confirmation; "
        ...
        "blocked_actions": [
            ...
            "advancing unless both seeds pass every frozen gate",
