...
RUN_ID = "i1_dialga128_runtime_e4_dfc1_r4_1000000_launch_gate_20260802"
REMOTE_RUN_ID = "i1_dialga128_runtime_e4_dfc1_r4_1000000_seed0_seed1_20260802"
DMC2_GATE = Path(
    "outputs/remote_results_incomplete/"
    "i1_dialga128_runtime_e4_dmc2_r4_262144_seed0_seed1_20260801/gate.local.json"
)
DMC2_GATE_SHA256 = "fb92b743157072f8a6aa2209d50ea9cd372f6eec08d8c5f96f479d037f03e1b0"
PLAN = Path(
    "configs/experiment/innovation1/"
    "innovation1_spn_dialga128_runtime_e4_dfc1_r4_1000000_seed0_seed1.csv"
)
REMOTE_CONFIG = Path(
    "configs/remote/"
    "innovation1_dialga_dfc1_r4_formal_1000000_seed0_seed1_gpu0_20260802.json"
)
...
REQUIRED_SOURCE_ASSETS = (
    ...
    Path("docs/experiments/innovation1-dialga128-runtime-e4-dfc1-r4-formal-plan.md"),
    GENERATED / f"run_{REMOTE_RUN_ID}.cmd",
    GENERATED / f"launch_{REMOTE_RUN_ID}.cmd",
    GENERATED / f"monitor_{REMOTE_RUN_ID}.sh",
    Path("scripts/check-dialga-r4-dfc1-launch"),
    Path("scripts/gate-dialga-r4-dfc1"),
    Path("scripts/package-dialga-r4-dfc1"),
    Path("src/blockcipher_nd/cli/check_dialga_r4_dfc1_launch.py"),
    Path("src/blockcipher_nd/cli/gate_dialga_r4_dfc1.py"),
    Path("src/blockcipher_nd/cli/package_dialga_r4_dmc1.py"),
    Path("src/blockcipher_nd/cli/package_dialga_r4_dfc1.py"),
    Path("src/blockcipher_nd/tasks/innovation1/dialga_r4_dfc1.py"),
    Path("src/blockcipher_nd/tasks/innovation1/dialga_r4_dfc1_launch.py"),
...
def adjudicate_launch(
    *,
    ...
    evidence = {"dmc2_local_gate_exact_pass": bool(authority) and all(authority.values())}
    readiness = {
    ...
    if not should_ssh:
        status = "fail"
        decision = "innovation1_dialga_dfc1_launch_evidence_invalid"
        next_action = "repair only failed DMC2 authority, readiness, or source checks"
    elif not ssh_allowed:
        status = "hold"
        decision = "innovation1_dialga_dfc1_source_not_published"
        next_action = "publish and verify the exact DFC1 source commit on GitHub main"
    else:
        status = "pass"
        decision = "innovation1_dialga_dfc1_remote_launch_authorized"
        next_action = (
    ...
    return {
        "run_id": RUN_ID,
        "task": "innovation1_dialga_dfc1_remote_launch_gate",
        ...
        "claim_scope": "local DFC1 remote-launch authorization only; no remote result claim",
        "blocked_actions": [
            "launching unpublished or worktree-drifted source",
            "using scp or dirty overlay for source publication",
            "running 1000000/class locally",
            "changing the frozen DFC1 protocol",
        ],
    }


def _authority_checks(repository: Path) -> dict[str, bool]:
    path = repository / DMC2_GATE
    checks = {"dmc2_gate_sha256_exact": _sha256(path) == DMC2_GATE_SHA256}
    try:
        gate = _read_json(path)
    except (OSError, json.JSONDecodeError):
        checks["dmc2_gate_json_readable"] = False
        return checks
    checks.update(
        {
            "dmc2_gate_json_readable": True,
            "dmc2_gate_exact_pass": gate.get("status") == "pass"
            and gate.get("decision")
            == "innovation1_dialga_dmc2_scale_topology_supported",
            "dmc2_formal_preregistration_authorized": gate.get("formal_scale")
            == "authorized_dfc1_preregistration",
            "dmc2_protocol_checks_complete": not gate.get("failed_protocol_checks"),
            "dmc2_research_checks_complete": not gate.get("failed_research_checks"),
        }
    )
    return checks
