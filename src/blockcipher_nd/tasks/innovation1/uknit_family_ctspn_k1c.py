...
RUN_ID = "i1_uknit_family_ctspn_train_validation_attribution_k1c_20260728"
K1B_DECISION = "innovation1_uknit_family_ctspn_k1b_native_endpoint_not_supported"
...
def validate_k1c_source(
    *,
    ...
    checks = {
        "source_k1b_gate_exact": (
            source_gate.get("run_id") == K1B_RUN_ID
            and source_gate.get("status") == "hold"
            and source_gate.get("decision") == K1B_DECISION
        ),
        "source_k1b_protocol_clean": bool(source_gate.get("protocol_checks"))
        ...
        "four_selected_checkpoints_complete": (
            checkpoint_manifest.get("run_id") == K1B_RUN_ID
        ...
        "source_plan_sha256_matches": (
            source_preflight.get("run_id") == K1B_RUN_ID
...
def evaluate_k1c(
    *,
    ...
    if set(datasets) != expected_datasets:
        raise ValueError("K1-C requires all four train and validation datasets")
    ...
    for cipher in EXPECTED_CIPHERS:
        for seed in EXPECTED_SEEDS:
            ...
            if checkpoint_sha256 != checkpoint.get("sha256"):
                raise ValueError(f"K1-C checkpoint hash mismatch: {checkpoint_path}")
            payload = torch.load(
                checkpoint_path,
                map_location="cpu",
                weights_only=False,
            ...
            for split in EXPECTED_SPLITS:
                ...
                for condition in CONTROL_CONDITIONS:
                    ...
                    if tensor_mapping_sha256(model.state_dict()) != state_sha256:
                        raise ValueError("K1-C strict load changed learned state")
    ...
    *,
    ...
    protocol_checks = {
        **source_checks,
        ...
        "validation_replays_k1b_exactly": all(
    ...
    if not protocol_valid:
        status = "invalid"
        decision = "innovation1_uknit_family_ctspn_k1c_protocol_invalid"
        next_action = "repair only the failed source, dataset, checkpoint, or replay binding and rerun K1-C unchanged"
        ...
        next_action = (
            "replace absolute endpoint summaries with one same-budget relative "
            "cross-transition path representation; retain all K1-B controls"
        )
    elif uknit_train_pass and uknit_validation_pass:
        status = "hold"
        decision = "innovation1_uknit_family_ctspn_k1c_source_replay_inconsistency"
        next_action = (
            "audit K1-B checkpoint selection and gate binding before any training"
        )
    else:
        status = "pass"
        decision = "innovation1_uknit_family_ctspn_k1c_endpoint_summary_not_attributed_on_training"
        next_action = (
            "close learned per-transition endpoint summaries and run a zero-training "
            "exact cross-transition endpoint-composition feasibility audit"
        )
    ...
    return {
        ...
        "optimizer_steps": 0,
        "claim_scope": (
            "zero-training same-checkpoint train-versus-validation topology "
            "attribution audit of the completed uKNIT-BC r5 and Dialga-128 r4 "
            "K1-B 2048/class local diagnostic; not new model, scale, attack, "
            "SOTA, transfer, or uKNIT ceiling evidence"
        ),
...
def _task_map(
    ...
    for task in tasks:
        ...
        if key in result:
            raise ValueError(f"duplicate K1-C task: {key}")
        result[key] = task
    return result
...
def _training_map(
    ...
    for row in rows:
        ...
        if key in result:
            raise ValueError(f"duplicate K1-B source result: {key}")
        result[key] = row
    return result
...
def _source_control_map(
    ...
    for row in rows:
        ...
        if key in result:
            raise ValueError(f"duplicate K1-B source control: {key}")
        result[key] = row
    return result
...
def _checkpoint_map(
    ...
    for entry in manifest.get("entries", []):
        ...
        if key in result:
            raise ValueError(f"duplicate K1-B checkpoint manifest entry: {key}")
        result[key] = entry
    return result
...
def _result_map(
    ...
    for row in rows:
        ...
        if key in result:
            raise ValueError(f"duplicate K1-C result row: {key}")
        result[key] = row
    return result
...
def _validate_dataset(dataset: DifferentialDataset, *, cipher: str, split: str) -> None:
    ...
    if dataset.features.shape != (expected_rows, expected_bits):
        raise ValueError(f"unexpected K1-C {cipher} {split} feature shape")
    if labels.shape != (expected_rows,):
        raise ValueError(f"unexpected K1-C {cipher} {split} label shape")
    if int(labels.sum()) != expected_rows // 2:
        raise ValueError(f"K1-C {cipher} {split} labels are not balanced")
