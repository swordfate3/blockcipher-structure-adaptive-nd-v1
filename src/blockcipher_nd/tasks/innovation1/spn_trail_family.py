...
def main(argv: list[str] | None = None) -> None:
    ...
    result = run_trail_family(args)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, sort_keys=True) + "\n", encoding="utf-8")


def run_trail_family(args: argparse.Namespace) -> dict[str, object]:
    input_difference = difference_for_profile(args.difference_profile, args.difference_member)
    train_features, train_labels = make_trail_family_dataset(
        rounds=args.rounds,
        key=args.train_key,
        ...
        depth=args.depth,
        false_family=args.model == "false_family",
    ...
    validation_features, validation_labels = make_trail_family_dataset(
        rounds=args.rounds,
...
def make_trail_family_dataset(
    *,
    ...
    if feature_cache_root is None:
        _write_progress(progress_output, "trail_family_cache_disabled", metadata)
    ...
    *,
    ...
    if features_path.exists() and labels_path.exists() and metadata_path.exists():
        ...
        if observed_metadata == metadata:
            _write_progress(progress_output, "trail_family_cache_reuse", {**metadata, "cache_dir": str(cache_dir)})
    ...
    _write_progress(progress_output, "trail_family_cache_flush_start", {**metadata, "cache_dir": str(cache_dir)})
    features.flush()
    labels.flush()
    metadata_path.write_text(json.dumps(metadata, sort_keys=True, indent=2) + "\n", encoding="utf-8")
    _write_progress(
        progress_output,
        "trail_family_cache_done",
        {
            **metadata,
...
def _generate_chunk(
    ...
    for offset in range(count):
        ...
        chunk[offset] = present_pairset_trail_family_features(
            compact_pairs,
            width=config.cipher.block_bits,
            cipher=row_cipher,
            beam_width=int(metadata["beam_width"]),
            depth=int(metadata["depth"]),
            false_family=bool(metadata["false_family"]),
        )
    return chunk
...
def _cache_metadata(
    *,
    ...
    return {
        ...
        "false_family": false_family,
        "width": width,
        "feature_dim": _feature_dim(width=width, beam_width=beam_width, depth=depth, pairs_per_sample=pairs_per_sample),
...
def _feature_dim(*, width: int, beam_width: int, depth: int, pairs_per_sample: int) -> int:
    cells = width // 4
    pair_dim = depth * (12 + cells)
    family_dim = depth * (13 + cells)
    return family_dim + pair_dim * 3 + 8
