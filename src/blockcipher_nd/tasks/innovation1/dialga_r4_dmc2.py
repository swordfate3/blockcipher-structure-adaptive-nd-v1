...
def _map_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        if key in mapped:
            raise ValueError(f"duplicate DMC2 row: {key}")
        mapped[key] = row
    if fail_closed and set(mapped) != expected_keys():
        raise ValueError("DMC2 matrix is incomplete")
    return mapped
...
def result_protocol_frozen(rows: Sequence[Mapping[str, Any]]) -> bool:
    ...
    return (
        ...
        and all(
            ...
            for (_, architecture), row in mapped.items()
        )
    )
...
def cache_protocol_checks(events: Sequence[Mapping[str, Any]]) -> dict[str, bool]:
    ...
    return {
        ...
        "eight_parameter_matched_cache_reuses": len(reuses)
...
def adjudicate(
    *,
    ...
    if not protocol_valid:
        status = "invalid"
        decision = "innovation1_dialga_dmc2_scale_protocol_invalid"
        next_action = (
            "repair only the failed source, plan, cache, checkpoint, or result "
            "binding and rerun DMC2 unchanged"
        )
    elif research_pass:
        status = "pass"
        decision = "innovation1_dialga_dmc2_scale_topology_supported"
        next_action = (
            "preregister Dialga DFC1 at at least 1000000/class with multiple "
            "seeds and frozen-checkpoint independent final tests"
        )
    else:
        status = "hold"
        decision = "innovation1_dialga_dmc2_scale_topology_not_supported"
        next_action = (
            "inspect restored-best histories and cache equivalence; stop "
            "mechanical scale-up and do not preregister DFC1"
        )
    return {
        "run_id": RUN_ID,
        "status": status,
        "decision": decision,
        "formal_scale": "authorized_dfc1_preregistration" if status == "pass" else "no",
        ...
        "claim_scope": (
            "two-seed remote 262144/class Dialga prefix-r4 scale confirmation; "
            "not formal, paper-scale, full-round, attack, SOTA, or universal-SPN evidence"
        ),
        "blocked_actions": [
            "calling 262144/class formal or paper-scale evidence",
            "advancing to DFC1 unless both seeds pass every frozen gate",
