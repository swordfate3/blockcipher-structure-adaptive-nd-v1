...
def adjudicate_runtime_spn_present_transfer(
    *,
    ...
    expected_seed: int = 0,
    expected_samples_per_class: int = 2048,
    phase: str = "t1",
) -> dict[str, Any]:
    if expected_samples_per_class < 1:
        raise ValueError("expected_samples_per_class must be positive")
    if phase not in {"t1", "rtg3b"}:
        raise ValueError(f"unsupported PRESENT runtime phase: {phase}")
    expected_models = {
    ...
    protocol_checks = {
        ...
        "frozen_transfer_scale": all(
            ...
            and row.get("samples_per_class") == expected_samples_per_class
            and row.get("pairs_per_sample") == 16
            and row.get("training", {}).get("train_rows")
            == 2 * expected_samples_per_class
    ...
    if not all(protocol_checks.values()):
        ...
        if phase == "rtg3b":
            decision = (
                f"innovation1_runtime_spn_present_formal_seed{expected_seed}_supported"
            )
            next_action = (
                "prepare the identical project-formal PRESENT RTG3-B seed1 replication"
                if expected_seed == 0
                else "synthesize the two-seed formal one-to-one topology decision"
            )
        else:
            decision = f"innovation1_runtime_spn_present_transfer_seed{expected_seed}_supported"
            next_action = (
        ...
        decision = (
            ...
            if phase == "rtg3b"
            else "innovation1_runtime_spn_present_transfer_not_supported"
        )
        next_action = (
            "stop PRESENT replication; diagnose the failed signal or topology-control "
            "gate without changing the frozen protocol after seeing the result"
        )
    claim_scope = (
        f"PRESENT-80 r7 seed{expected_seed} project-formal "
        f"{expected_samples_per_class}/class runtime-topology attribution; not a "
        "Zhang/Wang reproduction, paper-scale comparison, zero-step transfer, "
    ...
    return {
        ...
        "validation_samples_per_class": expected_samples_per_class // 2,
        "status": status,
        "decision": decision,
        ...
        "blocked_actions": [
            "increase samples, epochs, or pairs after seeing this gate",
            "change architecture together with scale",
