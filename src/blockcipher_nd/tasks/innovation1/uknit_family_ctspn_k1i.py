...
READINESS_RUN_ID = "i1_uknit_family_ctspn_gf2_boolean_view_k1i_readiness_20260728"
RUN_ID = "i1_uknit_family_ctspn_gf2_boolean_view_k1i_2048_seed0_seed1_20260728"
K1H_DECISION = "innovation1_uknit_family_ctspn_k1h_operator_tied_latent_not_supported"
CANDIDATE_MODEL = "runtime_spn_ct_k1i_boolean_view_true"
REVERSED_MODEL = "runtime_spn_ct_k1i_boolean_view_reversed"
CORRUPTED_MODEL = "runtime_spn_ct_k1i_boolean_view_corrupted"
NO_TOPOLOGY_MODEL = "runtime_spn_ct_k1i_boolean_view_none"
EXPECTED_PARAMETER_COUNT = 94754
...
def build_k1i_control(
    *,
    ...
    if condition not in model_keys:
        raise ValueError("unknown K1-I operator condition")
...
def build_k1i_readiness(
    *,
    tasks: Sequence[Mapping[str, Any]],
    k1h_gate: Mapping[str, Any],
    source_checks: Mapping[str, bool],
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    task_map = candidate_task_map(tasks, fail_closed=False)
    protocol_checks = {
        "k1h_operator_mean_hold_exact": (
            k1h_gate.get("run_id") == K1H_RUN_ID
            and k1h_gate.get("status") == "hold"
            and k1h_gate.get("decision") == K1H_DECISION
            and bool(k1h_gate.get("protocol_checks"))
            and all(k1h_gate.get("protocol_checks", {}).values())
        ),
    ...
    return manifests, {
        ...
        "decision": (
            "innovation1_uknit_family_ctspn_k1i_execution_authorized"
            if ready
            else "innovation1_uknit_family_ctspn_k1i_not_ready"
        ),
        ...
        "optimizer_steps": 0,
        "claim_scope": (
            "zero-training exact GF(2) Boolean-view, cache, anchor, geometry, "
            "composition, control, and invariance readiness only"
        ),
        "next_action": (
            "run the frozen four-row local K1-I diagnostic and the sixty-row "
            "three-split candidate/control/Runtime-E4 panel"
            if ready
            else "repair only the failed K1-I invariant or source binding and rerun readiness unchanged"
        ),
    }
...
def structural_readiness(
    ...
    for cipher in EXPECTED_CIPHERS:
        ...
        checks[f"{prefix}_no_forbidden_identity_or_semantics"] = (
            ...
            and correct.uses_learned_message_passing is False
...
def evaluate_k1i_panel(
    *,
    ...
    if set(datasets) != expected_datasets:
        raise ValueError("K1-I requires all three cached splits")
            ...
            for split in EXPECTED_SPLITS:
                ...
                for condition in CONTROL_CONDITIONS:
                    ...
                    if tensor_mapping_sha256(
                        ...
                        raise ValueError("K1-I candidate strict load changed state")
...
def validate_k1i_source_bindings(
    *,
    candidate_tasks: Sequence[Mapping[str, Any]],
...
def adjudicate_k1i(
    *,
    ...
    if not protocol_valid:
        ...
        next_action = (
            "repair only the failed K1-I protocol or binding and rerun unchanged"
        )
    elif all_research:
        status = "pass"
        decision = "innovation1_uknit_family_ctspn_k1i_gf2_boolean_view_supported"
        next_action = (
            "retain K1-I and preregister one disk-cached remote 65536/class "
            "diagnostic before any larger or family-wide claim"
        )
    elif dialga_retained:
        status = "hold"
        decision = (
            "innovation1_uknit_family_ctspn_k1i_linear_boolean_execution_"
            "insufficient_for_uknit"
        )
        next_action = (
        ...
        status = "hold"
        decision = (
            "innovation1_uknit_family_ctspn_k1i_invariant_boolean_view_"
            "bottleneck_not_supported"
        )
        next_action = (
            "close K1-I at this diagnostic scale and audit which Runtime-E4 "
            "position/cell interactions carry Dialga's calibrated signal"
        )
    return {
        "run_id": RUN_ID,
        "status": status,
        ...
        "claim_scope": (
            ...
            "exact GF(2) Boolean-view diagnostic; not formal scale, attack, SOTA, "
            "arbitrary-SPN transfer, or uKNIT ceiling evidence"
        ),
        "blocked_actions": [
            "remote scale or more data, epochs, width, pairs, seeds, or experts unless every K1-I gate passes",
            "raw bypass, S-box, DDT, trail, partial decryption, key, or cipher identity inside K1-I",
...
def candidate_task_map(
    ...
    if fail_closed and set(mapped) != expected_task_keys():
        raise ValueError("K1-I candidate tasks are incomplete")
    return mapped
