...
class PresentNibbleCase3TopologyResidualSpnOnlyDistinguisher(nn.Module):
    def __init__(
        self,
        input_bits: int,
        pair_bits: int = 128,
        ...
        if pair_bits != 128:
            raise ValueError(
                "PresentNibbleCase3TopologyResidual expects raw 128-bit ciphertext pairs"
            )
        if input_bits <= 0:
    ...
    def local_case3_view(self, features: torch.Tensor) -> torch.Tensor:
        ...
        channels = torch.stack(
            [pairs[:, :, 0], pairs[:, :, 1], mapped_difference],
            dim=2,
        )
    ...
    def encode_local_pairs(self, case3_matrices: torch.Tensor) -> torch.Tensor:
        expected_shape = (self.pairs_per_sample, 3, 4, 16)
        if (
            case3_matrices.ndim != 5
            or tuple(case3_matrices.shape[1:]) != expected_shape
        ):
            raise ValueError(
                "expected Case 3 matrices with shape "
                f"[batch, {self.pairs_per_sample}, 3, 4, 16], "
                f"got {tuple(case3_matrices.shape)}"
            )
        batch = case3_matrices.shape[0]
        hidden = case3_matrices.reshape(batch * self.pairs_per_sample, 3, 4, 16)
    ...
    def encode_fused_pairs(self, features: torch.Tensor) -> torch.Tensor:
        ...
        local_pair_embedding = self.encode_local_pairs(self.local_case3_view(features))
...
class PresentNibbleCase3InvPTopologyResidualSpnOnlyDistinguisher(
...
class PresentNibbleCase3ShuffledPTopologyResidualSpnOnlyDistinguisher(
    PresentNibbleCase3TopologyResidualSpnOnlyDistinguisher
):
    def __init__(self, *args, **kwargs) -> None:
...
class PresentNibbleCase3RawTopologyResidualSpnOnlyDistinguisher(
    PresentNibbleCase3TopologyResidualSpnOnlyDistinguisher
):
    def __init__(self, *args, **kwargs) -> None:
