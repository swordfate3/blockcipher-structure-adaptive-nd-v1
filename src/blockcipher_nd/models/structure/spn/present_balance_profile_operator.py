...
class PresentBalanceProfileOperatorSpec:
    input_dim: int = 39
    hidden_dim: int = 32
    steps: int = 2
    dropout: float = 0.10
...
class _SharedProfileBlock(nn.Module):
    ...
    def forward(
        self, state: torch.Tensor, cell_context: torch.Tensor, player_context: torch.Tensor
    ) -> torch.Tensor:
        update = self.update(torch.cat((state, cell_context, player_context), dim=-1))
        return self.norm(state + update)


class PresentBalanceProfileOperator(nn.Module):
    def __init__(
        self,
        spec: PresentBalanceProfileOperatorSpec,
        player: torch.Tensor,
    ) -> None:
        super().__init__()
