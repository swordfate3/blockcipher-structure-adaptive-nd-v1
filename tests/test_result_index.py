...
def test_result_index_labels_innovation2_output_parity_present_r2(
    tmp_path: Path,
    decision: str,
    expected_decision: str,
) -> None:
    ...
    assert entries[0]["display_name"] == (
        "创新2 OP4：PRESENT二轮结构对齐密文输出parity双密钥门"
    )
    assert entries[0]["decision_display"] == expected_decision
...
def test_result_index_labels_innovation2_present_profile_operator_readiness(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E66：PRESENT四轮prefix引导平衡谱算子readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_present_profile_operator_attribution(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E67：PRESENT四轮prefix引导平衡谱算子正式归因"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_present_multibit_mask_profile(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E69：PRESENT四轮多bit linear-mask profile审计"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_present_active_dimension_transfer(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E70：PRESENT四轮unit-profile跨活动维度零样本迁移"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_present_r3_only_replication(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    run_id = "i2_present_r4_r3_only_profile_operator_seed1_20260718"
    run_root = outputs / "local_diagnostic" / run_id
    _write_json(
        run_root / "gate.json",
        {
    ...
    assert entries[0]["display_name"] == (
        "创新2 E73：PRESENT四轮r3-only平衡谱算子双seed复核"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_gift64_unit_profile_readiness(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E74：GIFT-64四轮严格单位输出平衡谱标签readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_gift64_unit_profile_expansion(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E75：GIFT-64四轮严格单位输出平衡谱192结构容量复核"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_gift64_r3_only_readiness(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E76：GIFT-64四轮r3-only平衡谱算子readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_skinny_true_ridge_residual(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E84：SKINNY-64五轮真实拓扑ridge引导稀疏残差readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_shared_profile_operator(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    run_id = (
    ...
    assert entries[0]["display_name"] == (
        "创新2 E85：PRESENT/GIFT拓扑参数化共享Profile Operator readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_shared_profile_attribution(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    run_id = (
    ...
    assert entries[0]["display_name"] == (
        "创新2 E86：PRESENT/GIFT共享Profile Operator 30轮seed0归因"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_rectangle80_r3_readiness(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E89：RECTANGLE-80四轮r3-only平衡谱算子readiness"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_rectangle80_nested_cube(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E94：RECTANGLE-80四轮7/8/9-bit嵌套cube单调标签门"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_e98b_support_component_pu(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
...
def test_result_index_labels_innovation2_e101_source_generation_resume(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
...
def test_result_index_labels_innovation2_e103_real_atm_compatibility(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    entries = build_result_index(outputs, limit=10)
    assert entries[0]["display_name"] == (
        "创新2 E103：PRESENT S-box 4-bit三轮真实ATM兼容性门"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_rectangle80_r3_attribution(
    ...
    assert entries[0]["display_name"] == (
        "创新2 E90：RECTANGLE-80四轮r3-only平衡谱算子30轮seed0归因"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_innovation2_rectangle80_row_typed_audit(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    ...
    assert entries[0]["display_name"] == (
        "创新2 E91：RECTANGLE row-typed ShiftRow表示无训练审计"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_runtime_spn_r2d_sbox_scale(tmp_path: Path) -> None:
    outputs = tmp_path / "outputs"
    run_id = "i1_rtg1_gift64_runtime_e4_sbox_scale_r2d_2048_seed0"
    run_root = outputs / "local_diagnostic" / run_id
    _write_json(
        run_root / "gate.json",
        {
    ...
    assert entries[0]["display_name"] == (
        "创新1 RTG1-R2d：GIFT-64运行时S盒上下文强度校准"
    )
    assert entries[0]["decision_display"] == (
...
def test_result_index_labels_runtime_spn_h1_relation_activity_readiness(
    tmp_path: Path,
) -> None:
    outputs = tmp_path / "outputs"
    run_root = (
        outputs
        / "local_readiness"
        / "i1_runtime_spn_h1_relation_activity_pooling_a5_readiness_20260726"
    )
    _write_json(
        run_root / "gate.json",
        {
...
def test_result_index_labels_runtime_spn_h1_relation_activity_diagnostic(
    tmp_path: Path,
    decision: str,
    expected_decision: str,
) -> None:
    ...
    run_root = (
        outputs
        / "local_diagnostic"
        / "i1_runtime_spn_h1_relation_activity_pooling_a5_2048_seed0_seed1_20260726"
    )
    _write_json(
        run_root / "gate.json",
        {
    ...
    assert entries[0]["display_name"] == (
        "创新1 H1-A5：关系活动池化RECTANGLE留出双seed诊断"
    )
