...
def test_e4_r4_remote_assets_lock_scoring_bootstrap_and_unattended_launch() -> None:
    ...
    hash_repair = (
        generated
        / "repair_i1_gift64_cross_spn_target_adaptation_r4_65536_hashes_20260715.cmd"
    ).read_text(encoding="utf-8")
    ...
    assert run_script.count('echo * -text>"%') == 2
    ...
    assert "DisableDelayedExpansion" in hash_repair
    assert 'echo * -text>"%ARCHIVE_DIR%\\.gitattributes"' in hash_repair
    assert "hash_repair_commit.txt" in hash_repair
    assert "git add ." not in hash_repair
    assert "cmd.exe /k" not in hash_repair.lower()
