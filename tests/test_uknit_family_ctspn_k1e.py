...
from blockcipher_nd.cli.audit_uknit_family_ctspn_k1e import main as run_k1e
from blockcipher_nd.cli.plot_uknit_family_ctspn_k1 import render_ctspn_k1e_svg
...
def test_k1e_train_attribution_without_validation_routes_hypergraph() -> None:
    gate = adjudicate_k1e(
    ...
    assert "cell/path hypergraph" in gate["next_action"]


def test_k1e_train_failure_confirms_anonymous_path_relation_collapse() -> None:
    rows = _rows(train_margin=0.020, validation_margin=-0.010)
    target = next(
        row
        for row in rows
    ...
    gate = adjudicate_k1e(rows=rows, source_checks={"source_valid": True})

    assert gate["status"] == "pass"
    assert gate["decision"] == (
        "innovation1_uknit_family_ctspn_k1e_anonymous_path_relation_collapse_confirmed"
    )
    assert gate["attribution_summary"]["uknit_train_all_seeds"] is False
    assert "hypergraph message passing" in gate["next_action"]


def test_k1e_fails_closed_when_validation_does_not_replay_k1d() -> None:
    rows = _rows(train_margin=0.020, validation_margin=-0.010)
    target = next(
        row
        for row in rows
    ...
    assert gate["protocol_checks"]["validation_replays_k1d_exactly"] is False


def test_k1e_runner_does_not_create_output_for_invalid_source(
    ...
    monkeypatch.setattr(
        "blockcipher_nd.cli.audit_uknit_family_ctspn_k1e.validate_k1e_source",
...
def test_k1e_chinese_plot_names_relative_path_and_shared_cell_question(
    tmp_path: Path,
) -> None:
    gate = adjudicate_k1e(
        rows=_rows(train_margin=0.020, validation_margin=-0.010),
    ...
    render_ctspn_k1e_svg(gate, output)

    svg = output.read_text(encoding="utf-8")
    assert "相对路径是否只记住训练样本，还是丢失共享 cell 关系" in svg
    assert "四个 K1-D 最佳权重" in svg
    assert "原训练缓存" in svg
                    ...
                    rows.append(
                        {
                            ...
                            "source_validation_dataset_sha256": (
                                dataset if split == "validation" else None
                            ),
                            "source_validation_auc": (
                                auc if split == "validation" else None
                            ),
