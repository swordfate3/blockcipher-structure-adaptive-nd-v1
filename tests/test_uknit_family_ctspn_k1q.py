...
from blockcipher_nd.cli.audit_uknit_family_ctspn_k1q import read_tasks
...
def test_k1q_plan_scans_same_bit_role_across_all_native_cells() -> None:
    ...
    assert {
        ...
        (cell, 1, 1 << (4 * cell + 2)) for cell in EXPECTED_CELLS
    }
    assert candidate_bit_index(ANCHOR_CELL) == 6
    assert candidate_difference(ANCHOR_CELL) == 0x40


def test_k1q_selection_excludes_anchor_and_uses_two_fresh_gates() -> None:
    rows = discovery_result_rows(signal_cells={1, 5, 7})

    selection = select_discovery_candidates(rows)

    assert selection["status"] == "selected"
    assert selection["selected_cells"] == [5, 7]
    assert ANCHOR_CELL not in selection["selected_cells"]
    assert [
        row["cell"] for row in selection["ranking"] if row["eligible"]
    ][:2] == [5, 7]


def test_k1q_confirmation_matrix_uses_untouched_seeds_keys_and_anchor() -> None:
    discovery = read_tasks(PLAN)

    confirmation = build_confirmation_tasks(discovery, [5, 7])
    checks = validate_confirmation_tasks(confirmation, [5, 7])
...
def test_k1q_evaluator_reuses_exact_partial_state_scorer_without_training() -> None:
    ...
    assert all(row["input_difference"] == 0x4 for row in results)


def test_k1q_gate_requires_untouched_confirmation_for_advancement() -> None:
    tasks = read_tasks(PLAN)
    result_rows = discovery_result_rows(signal_cells={5})
    selection = select_discovery_candidates(result_rows)
    add_confirmation_results(result_rows, cells=(ANCHOR_CELL, 5), passing_cells={5})
    feature_rows, scorer_rows, dataset_rows = companion_rows(result_rows)
    ...
    assert gate["decision"].endswith("confirmed_r5_difference_position_supported")
    assert all(gate["protocol_checks"].values())
    assert all(gate["research_checks"].values())

    failed = [dict(row) for row in result_rows]
    for row in failed:
        if (
...
def test_k1q_gate_stops_when_no_position_is_discovered() -> None:
    tasks = read_tasks(PLAN)
    results = discovery_result_rows(signal_cells=set())
    ...
    assert selection["selected_cells"] == []
    assert gate["status"] == "hold"
    assert gate["decision"].endswith("no_r5_difference_position_discovered")
    assert all(gate["protocol_checks"].values())


def test_k1q_plot_explains_position_scan_and_confirmation_in_chinese(
    tmp_path: Path,
) -> None:
    tasks = read_tasks(PLAN)
    ...
    report = render_k1q_svg(gate, output)

    svg = output.read_text(encoding="utf-8")
    assert "移动 uKNIT 输入差分的位置，能否恢复第 5 轮信号" in svg
    assert "16 个 native cell" in svg
    assert "未见 seed 确认" in svg
    assert "0x40" in svg
    assert report["all_sixteen_positions_visible"] is True
    assert report["confirmation_uses_heatmaps"] is True
...
def discovery_result_rows(signal_cells: set[int]) -> list[dict[str, object]]:
    ...
    for cell in EXPECTED_CELLS:
        exact_auc = 0.62 if cell in signal_cells else 0.52
        if cell == 5:
            exact_auc = 0.68
        elif cell == 7:
            exact_auc = 0.65
        raw_auc = exact_auc - (0.04 if cell in signal_cells else 0.005)
        for split in EXPECTED_SPLITS:
            for view in DISCOVERY_VIEWS:
                auc = exact_auc if view == CANDIDATE_VIEW else raw_auc
                rows.append(
                    result_row(
                        phase=DISCOVERY_PHASE,
                        cell=cell,
...
def companion_rows(
    ...
    for row in result_rows:
        ...
        if row["view"] == LABEL_SHUFFLE_VIEW:
            feature_sha = feature_sha.replace(
                LABEL_SHUFFLE_VIEW,
                CANDIDATE_VIEW,
            )
        feature_rows.append(
