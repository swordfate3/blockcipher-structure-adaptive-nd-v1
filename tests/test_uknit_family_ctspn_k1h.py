...
from blockcipher_nd.cli.plot_uknit_family_ctspn_k1 import render_ctspn_k1h_svg
...
PLAN = (
    ROOT
    / "configs/experiment/innovation1/innovation1_uknit_family_ctspn_operator_tied_latent_k1h_2048_seed0_seed1.csv"
)
...
def test_k1h_real_plan_passes_zero_training_structural_readiness() -> None:
    manifests, gate = build_k1h_readiness(
        tasks=tasks(),
        k1f_gate=k1f_gate(),
        k1g_gate=k1g_gate(),
...
def test_k1h_operator_support_and_joint_relabel_are_exact() -> None:
    ...
    assert all(
        unit_support_exact(matrix)
        for matrix in model.runtime_structure.inverse_linear_matrices
    )
    assert cell_relabel_logit_delta(model, features) <= 1e-6


def test_k1h_controls_strict_load_same_cross_width_geometry() -> None:
    task_map = candidate_task_map(tasks())
    models = {
        cipher: build_k1h_control(
    ...
    assert model_metadata(models["uknit64"])["trainable_parameter_count"] == 100450
    assert model_metadata(models["dialga128"])["trainable_parameter_count"] == 100450
    ...
    assert not any("operator" in name for name in state)


def test_k1h_structural_readiness_proves_all_controls_change_logits() -> None:
...
def test_k1h_gate_requires_every_seed_split_anchor_and_control() -> None:
...
def test_k1h_readiness_fails_closed_without_cache_and_anchor_sources() -> None:
    manifests, gate = build_k1h_readiness(
        tasks=tasks(),
        k1f_gate=k1f_gate(),
        k1g_gate=k1g_gate(),
...
def test_k1h_plot_explains_operator_routing_and_local_scale(tmp_path: Path) -> None:
    training_rows, evaluation_rows = passing_evidence()
    gate = adjudicate_k1h(
        tasks=tasks(),
        training_rows=training_rows,
    ...
    render_ctspn_k1h_svg(gate, output)

    svg = output.read_text(encoding="utf-8")
    assert "固定真实扩散算子能否让 uKNIT 类模型泛化" in svg
    assert "矩阵只规定潜变量的传递路线" in svg
    assert "纵轴按本面板数据缩放" in svg


def k1f_gate() -> dict[str, object]:
    return {
        "run_id": K1F_RUN_ID,
        "status": "hold",
        ...
        for split in ("train_seen", "same_key_fresh", "cross_key_validation"):
            ...
            for condition in (*CONTROL_CONDITIONS, ANCHOR_CONDITION):
                ...
                evaluation_rows.append(
                    {
                        "run_id": (
                            "i1_uknit_family_ctspn_operator_tied_latent_"
                            "k1h_2048_seed0_seed1_20260728"
                        ),
                        "cipher_key": cipher,
                        "seed": seed,
                        "split": split,
