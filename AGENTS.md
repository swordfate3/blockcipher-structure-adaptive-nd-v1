...
- After a meaningful local result, retrieved remote result, or completed re-adjudication changes the available evidence, run `scripts/index-results` to refresh `outputs/00_RECENT_RESULTS.md` and `outputs/00_RECENT_RESULTS.json`. In that index, `001` is the newest completed result. Do not rename original run directories to add sequence numbers because configs, docs, and gates may reference those evidence paths.
