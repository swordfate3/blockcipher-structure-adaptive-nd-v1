...
- Every completed meaningful experiment must end with an evidence-backed recommended next action in both its `docs/experiments/` record and the final user-facing report. Explain how the observed gate and controls justify the recommendation; do not stop at metrics/status or a vague "continue investigating."
- After every completed result-producing run, refresh `outputs/00_RECENT_RESULTS.md` and `outputs/00_RECENT_RESULTS.json` with `scripts/index-results` in the same turn before the final report. This includes local smoke/readiness/diagnostic/formal runs, retrieved verified remote results, fallback-retrieved incomplete remote results, and completed re-adjudication gates. Planned or still-running remote jobs enter the index only after a result, validation, or gate artifact is available locally. An index-refresh failure means result handling is incomplete and must be diagnosed or reported. In the index, `001` is the newest completed result. Do not rename original run directories to add sequence numbers because configs, docs, and gates may reference those evidence paths.
- Make the next-step plan executable: state the next research question, same-budget anchor and required controls, the one variable to change, planned scale/seeds/epochs, local or remote execution path, readiness/advance/stop gates, and routes or mechanical scale-ups that must not be pursued. If an audit is recommended instead of training, name the exact mismatch to test and the decision that resolving it will unlock.
- Treat user-proposed research routes as hypotheses to rank, not conclusions to promote automatically. Before spending a meaningful local/remote experiment slot, compare the route against current literature, strongest local evidence, same-budget baselines, and required controls; explain when an alternative route deserves priority over the user's suggestion.
...
- Use local small-scale diagnostics as the default gate before SPN/PRESENT remote scale-up. If the local same-protocol candidate is near chance, unstable, or invalidated by required controls, do not launch a larger remote version by default; redesign locally first. Remote runs after a weak local gate require an explicit diagnostic exception in the experiment plan, such as testing a plausible data-scarcity effect.
...
- All artifacts generated by the `$teach` workflow must live under `lessons/`; do not create `MISSION.md`, `RESOURCES.md`, `NOTES.md`, teaching assets, reference sheets, learning records, or lesson HTML elsewhere in the repository. Use `lessons/assets/`, `lessons/reference/`, and `lessons/learning-records/` for their respective artifact types. The only exception is passive self-improvement records, which remain under `.learnings/`.
...
- Remote `--no-plot` validation, gate, and archive paths must not import Matplotlib or other plotting-only dependencies at module load time. Cover the real CLI with a subprocess test that blocks plotting imports; defer visualization to verified local retrieval when the remote environment omits those optional dependencies.
...
- The user has preapproved normal project `git push` attempts after scoped commits; do not pause in chat to ask for push approval. Attempt the normal configured push proactively. If the platform/escalation reviewer rejects the external transfer, do not work around it with dirty overlays, alternate transfer routes, or remote launches from unpublished code; report the exact rejected payload/head and keep progressing only on safe local work.
- When a normal GitHub push fails, use `skills/github-push-recovery/SKILL.md`: retry only classified transient network failures with bounded backoff, stop on authentication/non-fast-forward/remote-policy failures, and call the push successful only after the exact remote ref SHA matches the captured local `HEAD`.
