...
controls：wrong-swap / wrong-branch mapping / matched label shuffle
initial scale：2^17训练、2^16测试、100 epochs
keys：发现密钥 + 独立确认密钥
```

先用单一通用锚点和互不重合fresh集定位低轮正控制、最后通过轮和首个失败轮；完整结构矩阵只用于边界
两轮。不得把Innovation 1的DES真假区分结果当作本任务输出预测证据。
...
ARX-A screen（同预算true rows）：
  Kimura/Jeong邻近BiLSTM锚点
  通用word-ResCNN
  rotation/carry-aware residual候选
...
第一阶段使用`2^17/2^16、100 epochs`，扫描全部32个真实输出bit并在fresh前冻结位置。候选通过且新密钥
复现后，才允许为了论文可比性另行执行`2^22`训练、300 epochs的完整输出协议；该大规模行必须报告完整
输出exact match及论文指标，不能只用selected-bit AUC冒充完整密文复现。
