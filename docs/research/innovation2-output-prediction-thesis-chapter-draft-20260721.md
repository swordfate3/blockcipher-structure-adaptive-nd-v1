...
状态：OP9--OPC1、OPM1与OPN1证据已合入 / OPD1远程正式矩阵运行中 / 可作为当前论文章节底稿
...
> `hold`；OPN1又证明其最后一次P重排可被全局线性head吸收。由此预注册的OPD1位置绑定五行矩阵已
> 通过本地readiness并在远程运行，本文只记录冻结协议和运行状态，不提前填写结果。OPM1已经排除
...
### 6.10 OPD1：位置绑定SPN-ResCNN正式矩阵运行中
...
本地`64/64`、1 epoch readiness已经验证五行数据回放、参数匹配、初始化公平性、exact/wrong映射差异、
标签打乱边界、checkpoint和产物闭环；小样本AUC不用于性能裁决。远程正式任务已从推送提交
`6a3f4fbbd7652c50be7e397c54f70f0a93dea6fe`在A6000启动，并由本地watcher自动回收。截至本文本次
更新，任务仍在运行且尚无正式结果归档，因此本节不填写AUC或通过状态。
...
因此当前运行的OPD1改用位置绑定head，专门检验约束输出读取范围后能否恢复精确拓扑差异。OPM1还
表明八个selected bit与其他输出bit具有
...
8. 用确定性重参数化审计识别全局线性head会吸收最后固定路由，并据此构造参数匹配的位置绑定归因
   实验；该项目前只能主张方法与协议，不能在OPD1归档前主张性能收益。
...
- OPD1正在运行就等于位置绑定head已经获得正结果或恢复了精确P-layer因果贡献；
...
路线分配机械扩容预算。OPD1的位置绑定head、五行控制、本地readiness、远程包、推送提交、A6000
启动和自动回收watcher均已完成；冻结矩阵正在运行。后续按以下顺序推进：

1. 等待watcher回收正式结果，校验来源提交、authority、SHA-256和`40/500/5/196608`完整性；
2. 按冻结平均门和八个逐bit联合门裁决，不修改输出位置、阈值、模型、数据、epoch或wrong-P；
3. 对真实正式SVG执行像素级视觉检查，刷新最近结果索引，并把真实指标和唯一下一动作写回实验记录；
4. 若OPD1通过，只开放全新固定密钥原样确认；若未通过，保留普通ResCNN并停止位置绑定路线；两种
   情况均不直接开放四轮；
5. 将最终裁决并入学校论文模板，补齐正式参考文献编号、图号和表号；
6. 只有独立文献或确定性密码结构分析提出新的方法级四轮输出函数机制，才允许新建四轮路线；不得
