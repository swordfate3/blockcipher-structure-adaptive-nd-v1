...
状态：OP9--OP12历史快照 / 等待OPA3后按当前证据重写

> 2026-07-22更新：本文是OP9--OP12阶段的历史章节快照，尚未合入随后完成的OPA1五模型屏、OPA2
> 第四密钥匹配控制确认，以及仍在运行的OPA3拓扑归因。当前权威证据边界见
> `docs/research/innovation2-output-prediction-thesis-boundary-20260721.md`。OPA3裁决前不得把本文直接
> 作为最终创新2章节，也不得提前写入精确P-layer或四轮成功主张。
...
第一，OP10/OP11只覆盖两把独立固定秘密密钥，没有报告更多密钥上的均值、方差或置信区间。因此，
`8/8`复现是跨两把密钥的确认，不是广泛跨密钥统计。第二，OP9复用了论文报告的网络规模和训练预算，
但框架、初始化、超参数搜索过程和密钥数量不同，只能视为论文族单密钥校准。第三，OP10的MLP候选在
发现和fresh阶段只配有跨架构LSTM标签打乱控制；OP11才补齐同架构MLP shuffle并完成归因。第四，
OP11预测三轮单bit，OP12预测四轮XOR，无法由两点直接估计同一目标的临界轮数。最后，当前最高平均
AUC为`0.530900037`，支持存在弱但受控的局部输出信号，不代表完整密文恢复、实际密钥恢复能力或
主流攻击轮数竞争力。

## 9. 可直接用于摘要或答辩的结论

> 本文针对完整密文输出预测容易受多任务干扰的问题，提出易预测输出坐标发现与专用多输出头方法。
> 在固定未知秘密密钥的PRESENT三轮任务中，首先从完整64输出模型中发现八个候选位置，并在独立
> 明文上确认；随后冻结位置集合，在第二把独立固定秘密密钥上八个位置全部复现。专用八输出头的平均
> AUC为0.530900，分别比完整64输出头和同架构标签打乱控制高0.008994和0.030112。进一步对四个
> 同末轮S-box双bit XOR及两个同角色四bit XOR进行预注册扩轮实验，六个目标均未超过几何、标签打乱、
> 单bit派生和最佳组成bit控制，表明当前结构化XOR机制不能将真实输出预测扩展到四轮。该结果给出了
> 三轮局部输出可预测性与四轮组合输出失效的清晰适用边界，而不是完整密文恢复或高轮攻击突破。
...
正文建议引用已通过像素检查的OP11与OP12正式`curves.svg`。本初稿没有生成新图，移入论文模板或重新
导出图像后，仍需重新执行视觉质量检查。

## 11. 下一步执行计划

当前不再为已失败的四轮结构化XOR分配训练预算。后续按以下顺序推进：

1. 将本初稿并入学校论文模板，补齐正式参考文献编号、图号和表号；
2. 用OP11、OP12的正式SVG制作统一字号的论文图，重新进行像素QA；
3. 若开题验收明确要求中间状态迭代预测，先单独冻结威胁模型和确定性基线，回答网络是否只是拟合公开
   轮函数；审计通过后再设计`state_t -> state_(t+1)`实验，不从OP12直接机械扩展；
4. 只有独立文献或确定性密码结构分析提出新的预注册四轮输出函数机制，才允许重新打开四轮输出值预测。
