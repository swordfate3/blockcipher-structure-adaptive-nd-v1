...
状态：OP9--OPC1、OPM1与OPN1证据已合入 / OPC1 hold并停止 / 可作为当前论文章节底稿
...
路线分配机械扩容预算。后续按以下顺序推进：

1. 预注册参数匹配的位置绑定输出头：八个独立`252 -> 64 -> 1`局部head替代
   `Flatten + Linear(252*64,8)`，只改变head可识别性；
2. 固定普通ResCNN、无路由位置头、exact-P位置头、wrong-P位置头和exact-P shuffle五行控制；先做
   参数/映射/目标回放和本地readiness，不读取小样本性能作裁决；
3. 只有readiness完整并从推送提交启动时，才用新seed7、`2^17/2^16`、100 epochs远程正式矩阵；
4. 候选必须同时超过普通ResCNN、无路由位置头、wrong-P和shuffle，失败则停止该位置头路线；
