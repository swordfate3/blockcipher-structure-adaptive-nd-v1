...
状态：OP9--OPD1、OPE1、OPM1与OPN1证据已合入 / 位置绑定与简单XOR扩轮均停止 / 可作为当前论文章节底稿
...
> SPN式架构与分层扩散结论，不主张精确P-layer因果贡献。OPC1 SPN-ResCNN四行正式矩阵已经完成并
> `hold`；OPN1又证明其最后一次P重排可被全局线性head吸收。OPD1位置绑定五行矩阵随后完成：
> exact-P与wrong-P平均AUC均接近`1.0`，差值只有`+0.000022`，因此`0/8`通过并裁决为`hold`。这说明
> 位置绑定能显著提高三轮八bit可学习性，但仍不能归因精确PRESENT连线。OPM1已经排除
> “selected bit输入锥更窄”与“单S-box坐标明显更简单”这两个粗粒度解释。
...
### 4.4 多架构筛选与SPN-ResCNN混合模型

OPA1在相同八输出、数据、训练预算和约390万参数约束下比较MLP、六层LSTM、位置保持ResCNN、
Transformer与PRESENT-SPN-aware五个模型。SPN-aware模型先在每个4-bit nibble内共享局部变换，再按
固定位置映射逐层扩大感受野；OPA3以identity与fixed-wrong-P替换exact-P，区分“需要跨nibble扩散”
和“需要精确PRESENT连线”两种解释。OPB1进一步用低秩位置条件限制模型记忆每个绝对位置的自由度。

OPC1则以非饱和ResCNN为锚点，将十个残差块组织为`3+3+4`三个阶段，在阶段间插入固定P路由。四行
矩阵保持全局输出头、参数量、初始化、数据和预算一致，只比较普通ResCNN、exact-P、wrong-P与
exact-P标签打乱。该设计测试的是“在非饱和卷积骨干中加入分层路由是否有效”，不是重新筛选输出位。

### 4.5 OPD1位置绑定输出头

OPN1证明，OPC1最后一次固定位置重排位于`Flatten + Linear(252*64, 8)`之前时，可以通过重排线性层
列被完全吸收。OPD1因此只改变输出头的可识别性：每个冻结输出bit仅从其对应最终位置读取252维特征，
再通过独立局部head输出该真实密文bit：
...
位置绑定head约有`130056`个参数，原全局head为`129032`个参数，整网参数差约`0.026%`。正式五行矩阵
同时包含普通全局头ResCNN、无P位置头、exact-P位置头、wrong-P位置头和exact-P标签打乱；第三、第四
行可训练张量初始化逐项相同，唯一差异是冻结位置映射。这样只有exact-P同时超过四类控制，才支持
“位置绑定恢复了精确拓扑贡献”的解释。
...
RQ5：同预算多架构筛选中，SPN式归纳偏置是否优于MLP、LSTM、ResCNN与Transformer？
RQ6：SPN式增益来自跨nibble扩散，还是能够进一步归因于精确PRESENT P-layer？
RQ7：约束全局输出头的重参数化自由度后，位置绑定head能否恢复精确P相对必要控制的增益？
```
...
OP10在seed0固定秘密密钥上扫描全部64个位置。共有`27`个位置通过预注册最低资格门，再按不读取
fresh标签的综合分数冻结`candidate_limit=8`个top候选；八个候选均由真实输出MLP选出，并在全新
`65536`条明文上全部复现：
...
冻结候选的发现顺序为`32,34,2,42,40,8,0,10`，表中只是按MSB位置重新排序；不能表述成其余56位
都没有信号，也不能在后续密钥上从其余19个合格位置中改选。top-8只落在MSB-first nibble
`0/2/8/10`的内部bit `0/2`，呈现结构化子集，但该图案是统计筛选后的观察，不是候选选择特征或精确
P-layer因果证据。OP10的打乱对照是LSTM，而八个位置均由MLP选出，因此OP10只支持fresh明文信号，
不能单独支持同架构归因或跨密钥稳定性。
...
仍预测同八个PRESENT三轮真实密文bit。verified result branch完整回收`32/400/4/196608`条
结果/history/checkpoint/cache证据，全部hash、协议和执行检查通过。正式平均AUC为：

```text
普通ResCNN        = 0.573571593
SPN-ResCNN真实P   = 0.546633861
SPN-ResCNN错误P   = 0.545181187
真实P标签打乱     = 0.500163496
```

真实P相对ResCNN为`-0.026937731`，相对错误P为`+0.001452674`，只有相对shuffle的
`+0.046470365`过门，联合通过位置为`0/8`。正式裁决是`hold`，保留ResCNN并停止全局头混合路线；
不做seed7确认、机械扩容或四轮扩展。正式SVG已在1920与1280像素宽下通过视觉检查。
...
### 6.9 OPN1：全局输出头限制最后路由归因

OPC1的普通ResCNN、exact-P混合模型和wrong-P混合模型均使用
`Flatten + Linear(252 * 64, 8)`输出头。该head的每个输出都连接全部64个最终位置。对于最后一次
固定位置排列`p`，线性关系满足：

```text
W * flatten(h[:, :, p]) = W_p * flatten(h)，其中 W_p[:, :, p] = W[:, :, :]
```

OPN1用实际exact-P、wrong-P和identity映射在float64下验证这一重参数化，最大绝对误差为
`1.9895e-13`，全部head形状、排列完整性和产物检查通过。故第三个、位于最后残差阶段之后的路由
不能由当前全局head单独识别。前两个路由之后仍有非线性残差块，OPN1不声称它们也可被吸收。

这个确定性结果没有改变OPC1的冻结协议。OPC1最终未过门，因此下一架构假设优先检查位置绑定或
共享选定位head，而不是机械增加当前混合模型容量；若未来新模型出现exact-P相对wrong-P增益，也
必须先排除下游head的同类重参数化对称性。

### 6.10 OPD1：位置绑定SPN-ResCNN正式结果

OPD1保持PRESENT三轮、八个冻结位置、seed7固定秘密密钥、`2^17/2^16`训练/测试明文对、100 epochs、
RMSprop与约396万参数预算不变，只把全局扁平head替换为参数匹配的位置绑定head。正式矩阵冻结为：
...
本地`64/64`、1 epoch readiness验证了五行数据回放、参数匹配、初始化公平性、exact/wrong映射差异、
标签打乱边界、checkpoint和产物闭环；小样本AUC不用于性能裁决。远程正式任务从推送提交
`6a3f4fbbd7652c50be7e397c54f70f0a93dea6fe`完成全部五模型与500个epoch。五模型平均AUC分别为：

```text
普通全局头ResCNN / 无P位置头 = 0.570093 / 0.541337
exact-P位置头 / wrong-P位置头 = 0.999996 / 0.999974
exact-P标签打乱                 = 0.500235
```

exact-P相对全局头、无P和shuffle分别提高`+0.429903/+0.458659/+0.499761`，但相对wrong-P仅
`+0.000022`，八个bit全部因错误P控制失败而未通过联合门，`passed bits=0/8`。因此位置绑定确实让三轮
八bit任务近乎完全可学，却没有恢复精确PRESENT连线归因，正式裁决为`hold`并停止该路线。

结果因authority JSON重序列化触发post-run字节哈希门，未形成verified result branch；本地按raw
fallback回收并验证了`40/500/5/196608`、来源提交与全部checkpoint hash。该归档状态不改变性能裁决，
但论文必须标注`fallback_retrieved`，不能写成verified result branch。

正式通过要求exact-P位置头平均AUC至少`0.550`，并分别超过全局头`0.010`、无P位置头`0.010`、
wrong-P位置头`0.020`和标签打乱`0.030`，且至少`4/8`个bit同时通过逐bit联合门。通过只开放全新固定
密钥原样确认；任一平均门或联合门失败即保留普通ResCNN并停止位置绑定路线，不扩容或进入四轮。
...
OPE1又把八个冻结易预测bit全部异或为同一个三轮真实输出值，并比较直接ResCNN、同目标MLP、整体
右移一位的同重量mask、匹配标签打乱、单bit派生parity和最佳组成bit。在`4096/4096`、10 epochs本地
诊断中，直接ResCNN AUC仅`0.493085`，低于MLP `0.502388`、右移控制`0.500596`和shuffle
`0.500995`，准确率还比多数基线低`0.021729`，三项低轮校准门全部失败。因此连三轮同目标校准都未
建立，不运行四轮或远程扩样本。该结果与OP12共同说明：当前八个单bit弱信号不能通过简单XOR组合成
更高轮输出函数。
...
wrong-P仍完全同分，说明该负归因不是仅靠原模型的高维位置嵌入造成。OPC1随后显示全局头exact-P
混合模型既低于普通ResCNN，也只比wrong-P高`0.001453`；OPN1进一步指出最后路由可被全局head吸收。
OPD1进一步显示，即使改用位置绑定head，exact-P与wrong-P仍都接近`1.0`且只差`0.000022`，所以约束
输出读取范围仍未隔离精确连线贡献。OPM1还表明八个selected bit与其他输出bit具有
...
4. 通过OP12预注册2/4-bit结构化XOR与OPE1八bit全parity、几何控制、派生parity和最佳组成bit，给出
   “简单XOR组合不提高预测轮数”的两级停止边界。
...
8. 用确定性重参数化审计识别全局线性head会吸收最后固定路由，并据此构造参数匹配的位置绑定归因
   实验；位置绑定使exact-P与wrong-P都近乎完美，但仍未产生可归因的精确连线差异。
...
- OP12证明所有四轮输出函数均不可预测；
- OPE1证明所有八bit parity在所有密钥或正式规模下均不可预测；它只是seed1本地校准未通过；
- 完成了`state_t -> state_(t+1)`中间状态逐轮迭代预测攻击；
- 可将本章AUC与真假样本区分器AUC直接比较并宣称SOTA。
- OPA3证明精确P-layer是`AUC=1.0`的原因；
- OPB1低秩瓶颈已经隔离出精确P-layer贡献；
- OPC1获得了SPN-ResCNN正式正结果；其正式裁决为`hold`，且exact-P低于普通ResCNN；
- OPD1的exact-P接近`1.0`就等于恢复了精确P-layer因果贡献；wrong-P同样接近`1.0`且门判定为hold；
...
> 条件式四轮SPN网络因归因门未通过而没有启动。OPN1又证明SPN-ResCNN的最后一次P重排可被全局
> 线性输出头吸收；OPC1中真实P混合AUC为0.546634，低于ResCNN的0.573572且只比错误P高0.001453，
> 因而该全局头路线停止。该结果
...
outputs/remote_results_incomplete/i2_output_prediction_opd1_present_r3_position_bound_spn_rescnn_key7_gpu0_20260722_raw_fallback/
...
正文建议引用已通过像素检查的OP11、OP12、OPA2、OPA3、OPB1、OPD1与OPE1 `curves.svg`。OPD1属于
fallback-retrieved结果而非verified result branch，图注和证据表必须保留该来源状态。移入论文模板或
重新导出图像后，仍需重新执行视觉质量检查。

## 11. 下一步执行计划

当前不再为已失败的四轮结构化XOR、未归因的精确P-layer、已关闭的OPA4/OPA5或OPC1全局头混合
路线分配机械扩容预算。OPD1已完成并因exact-P未超过wrong-P而`hold`，位置绑定路线停止。后续按以下
顺序推进：

1. 将OPD1的fallback来源、`40/500/5/196608`完整性、五模型均值和`0/8`门写入论文表格；
2. 不做OPD1 fresh-key确认，不调整位置头、错误P、数据、epoch或模型容量；
3. OPE1八bit全XOR已完成三轮校准并因`0/3`门通过而hold；按冻结计划不运行四轮或远程规模；
4. 新输出函数必须先由密码结构或代数机制导出，不能继续对已知易预测bit做后验XOR枚举；
5. 将最终裁决并入学校论文模板，补齐正式参考文献编号、图号和表号；
6. 只有独立文献或确定性密码结构分析提出新的方法级高轮输出函数机制，才允许继续扩轮；不得
