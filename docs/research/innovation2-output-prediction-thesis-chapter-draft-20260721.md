...
状态：OP9--OPD1、OPM1与OPN1证据已合入 / OPD1 hold并停止位置绑定路线 / 可作为当前论文章节底稿
...
正文建议引用已通过像素检查的OP11、OP12、OPA2、OPA3、OPB1与OPD1 `curves.svg`。OPD1属于
...
3. 对用户提出的八bit全XOR真实输出函数先做同一mask的三轮校准与四轮本地可行性门；
4. 只有四轮直接parity同时超过label shuffle、同重量几何控制、由八个单bit派生的parity和最佳组成bit，
   才准备A6000 `2^17/2^16`正式规模；否则停止，不机械扩样本、epoch、网络或轮数；
