...
> 历史路线说明（2026-07-22）：本文排序的是积分平衡、kernel、结构关系等历史标签任务上的网络，
> 不是当前固定未知密钥真实密文输出值预测的模型排名。当前模型证据为OPA1--OPA3，权威入口见
> `docs/research/innovation2-output-prediction-thesis-boundary-20260721.md`。本文中的高轮数、AUC和
> “当前架构”措辞只能在各自历史任务内解释，不能与OP9--OPA3直接比较。
...
E85转向已有最强双密码标签，测试`Topology-Parameterized Shared Profile Operator`：一套4,795参数
模型不使用cipher ID、adapter或专属head，只在forward时接收PRESENT或GIFT的真实P-layer。
每轮完整消费PRESENT 7个与GIFT 14个batch并做逐密码loss归一；三行只改变independent、真实或
same-family错误拓扑。这个问题不同于AutoND的通用架构与Zahednejad--Lyu逐密码训练/transfer；
现有本地全文和补充检索未找到同一严格任务与控制的直接先例，但这不等于完整新颖性证明。

两轮readiness全部协议与信号门通过。共享true模型在PRESENT/GIFT的AUC为
`0.856944/0.762747`，相对independent为`+0.255833/+0.259886`，相对错误拓扑为
`+0.167778/+0.068939`；macro true为`0.809846`。它还分别比两套独立readiness true锚点高
`+0.022778/+0.002081`。这是当前第一个“一套参数处理两个真实SPN拓扑”的正向readiness，
但仍只有两轮seed0，不改变E73/E79正式双seed证据的主排名。
...
E87把第三种真实P-layer型SPN扩展到RECTANGLE-80，但严格停在标签门。最终2015版RECTANGLE
四轮、96个8-bit cube结构产生`4941/952/251`个positive/negative/unknown，96个结构全部为
mixed，三态签名95种。配平后的structure-disjoint训练集为`602/602`，验证集为`182/182`，
覆盖72/24个互斥结构和37个验证输出bit；global、output-bit、active-bit与最强一元AUC全部为
`0.5`。最终版零向量、S-box ANF、轮常量、P-layer、向量/标量对拍和抽样negative witness
重放全部通过。

因此RECTANGLE已成为PRESENT、GIFT之后第三种通过同语义sound unit-profile标签readiness的真实
SPN，但尚无RECTANGLE神经结果。下一步E88只把结构数从96扩到192，其他四轮协议、标签、split、
