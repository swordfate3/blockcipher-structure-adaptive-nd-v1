...
状态：PDF原文复核完成 / 协议边界已映射 / OPB1低秩未归因 / OPC1正式远程运行中
...
| 比较ResNet、时间序列网络、Transformer等不同架构 | PDF物理第3页/报告第1页 | OPA1五模型；OPA2/OPA3确认归因；OPB1低秩瓶颈；OPC1 SPN-ResCNN改造 | OPA1--OPB1；OPC1 readiness与正式运行 | **多架构筛选完成，精确拓扑仍未归因，混合改造运行中** | “架构归纳偏置显著影响八bit预测；两次控制均未把增益归因到精确P-layer” | ResCNN/Transformer本身构成创新；低秩瓶颈已证明精确拓扑 | 回收OPC1；通过后fresh-key确认，失败则停止混合路线 |
...
3. 若OPC1通过，同时保持输出能力并超过ResCNN/wrong-P/shuffle，再用全新固定密钥原样确认；
