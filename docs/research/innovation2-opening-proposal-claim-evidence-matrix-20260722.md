...
状态：PDF原文复核完成 / OPC1与OPD1均hold并停止 / 八bit全XOR进入本地可行性门
...
|---|---|---|---|---|---|---|---|
| 使用神经网络预测真实密文输出bit | PDF物理第3页/报告第1页：“预测分组密码的密文输出比特” | 固定未知秘密密钥；输入未见明文；目标为真实PRESENT三轮完整64-bit或冻结八bit | OP9--OPD1 | **部分完成**：八个冻结bit完成；完整64-bit未恢复；OPC1与OPD1均未建立精确拓扑归因 | “建立PRESENT三轮真实密文输出bit预测流程，并确认八个易预测坐标” | 恢复完整密文；输出预测普遍成功；OPD1 exact接近1即为拓扑突破 | 以八bit全XOR作为独立输出函数做r3/r4本地控制门 |
...
| 用输出预测增加分组密码安全评估维度 | PDF物理第19页/报告第17页，正式创新（2） | 真实输出任务、协议校准、易预测坐标、独立密钥、多架构与三级控制归因已形成闭环 | OP9--OPD1、OPN1、协议审计、论文边界文档 | **PRESENT三轮窄范围实质支持** | “在神经区分器之外建立真实密文输出值预测评估流程，并给出PRESENT三轮正结果、拓扑归因边界与四轮XOR负边界” | 已形成通用分组密码安全边界；已达到SOTA攻击轮数；OPD1证明精确P | 停止位置绑定；对八bit全XOR仅做本地假设门 |
...
6. 八bit全XOR只先做r3校准与r4本地控制门；过门后才单独决定远程或跨密码计划。
