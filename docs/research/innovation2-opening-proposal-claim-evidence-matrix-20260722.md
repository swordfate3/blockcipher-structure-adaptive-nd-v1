...
状态：PDF原文复核完成 / 协议边界已映射 / OPB1低秩未归因 / OPC1正式授权
...
|---|---|---|---|---|---|---|---|
| 使用神经网络预测真实密文输出bit | PDF物理第3页/报告第1页：“预测分组密码的密文输出比特” | 固定未知秘密密钥；输入未见明文；目标为真实PRESENT三轮完整64-bit或冻结八bit | OP9--OPB1 | **部分完成**：八个冻结bit完成；完整64-bit未恢复 | “建立PRESENT三轮真实密文输出bit预测流程，并确认八个易预测坐标” | 恢复完整密文；输出预测普遍成功 | OPB1停止；运行OPC1同目标模型归因 |
| 比较ResNet、时间序列网络、Transformer等不同架构 | PDF物理第3页/报告第1页 | OPA1五模型；OPA2/OPA3确认归因；OPB1低秩瓶颈；OPC1 SPN-ResCNN改造 | OPA1--OPB1；OPC1 readiness与正式计划 | **多架构筛选完成，精确拓扑仍未归因，混合改造已授权** | “架构归纳偏置显著影响八bit预测；两次控制均未把增益归因到精确P-layer” | ResCNN/Transformer本身构成创新；低秩瓶颈已证明精确拓扑 | 运行OPC1；通过后fresh-key确认，失败则停止混合路线 |
...
| 大规模实验形成安全边界 | PDF物理第3--4页/报告第1--2页 | OP9--OPB1使用`2^17`训练、`2^16`测试、100 epochs；OPC1同规模待启动 | 各正式实验计划和远程归档 | **部分完成**：达到Kimura单密钥样本预算，不是跨多轮/跨密码大规模边界 | “完成PRESENT三轮固定密钥论文协议量级实验” | paper-scale多密钥复现；百万/类；完整临界轮图 | 精确报告train/test totals、密钥数、epoch和目标，不使用`/class`术语 |
...
2. 启动OPC1第七固定密钥SPN-ResCNN四行正式矩阵，不立即扩轮；
