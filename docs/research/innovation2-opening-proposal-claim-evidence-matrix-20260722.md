...
状态：PDF原文复核完成 / OPN1输出头审计完成 / OPC1正式远程运行中
...
|---|---|---|---|---|---|---|---|
| 使用神经网络预测真实密文输出bit | PDF物理第3页/报告第1页：“预测分组密码的密文输出比特” | 固定未知秘密密钥；输入未见明文；目标为真实PRESENT三轮完整64-bit或冻结八bit | OP9--OPB1；OPC1运行中 | **部分完成**：八个冻结bit完成；完整64-bit未恢复；混合模型正式结果待回收 | “建立PRESENT三轮真实密文输出bit预测流程，并确认八个易预测坐标” | 恢复完整密文；输出预测普遍成功 | 回收OPC1同目标模型归因结果 |
| 比较ResNet、时间序列网络、Transformer等不同架构 | PDF物理第3页/报告第1页 | OPA1五模型；OPA2/OPA3确认归因；OPB1低秩瓶颈；OPC1 SPN-ResCNN改造；OPN1输出头审计 | OPA1--OPB1、OPN1；OPC1 readiness与正式运行 | **多架构筛选完成，最后P路由可被全局head吸收，混合改造运行中** | “架构归纳偏置显著影响八bit预测；精确拓扑与可识别路由范围由匹配控制和确定性审计共同限定” | ResCNN/Transformer本身构成创新；最后一次P重排有独立贡献 | 回收OPC1；通过后fresh-key确认并只归因前两个路由，失败则停止混合路线 |
...
| 大规模实验形成安全边界 | PDF物理第3--4页/报告第1--2页 | OP9--OPB1使用`2^17`训练、`2^16`测试、100 epochs；OPC1同规模运行中 | 各正式实验计划和远程归档 | **部分完成**：达到Kimura单密钥样本预算，不是跨多轮/跨密码大规模边界 | “完成PRESENT三轮固定密钥论文协议量级实验” | paper-scale多密钥复现；百万/类；完整临界轮图 | 精确报告train/test totals、密钥数、epoch和目标，不使用`/class`术语 |
...
1. OPB1低秩瓶颈已停止，不再调整rank、数据、epoch或错误P；
2. OPC1第七固定密钥SPN-ResCNN四行正式矩阵已在远程运行，等待自动回收，不立即扩轮；
3. OPN1已证明最后一次P重排可被全局head吸收；若OPC1通过，同时保持输出能力并超过
   ResCNN/wrong-P/shuffle，再用全新固定密钥原样确认，并只归因前两个阶段间路由；
4. 若OPC1未通过，保留ResCNN发现锚点并停止混合路线，不加深、扩样本或后验换位；
