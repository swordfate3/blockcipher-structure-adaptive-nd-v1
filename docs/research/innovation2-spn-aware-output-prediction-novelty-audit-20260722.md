...
状态：核心全文与联网核验完成 / OPC1 hold并停止 / OPD1位置绑定head正式任务运行中
...
|---|---|---|---|---|
| 密钥协议 | 单固定未知密钥，逐密钥训练 | 单固定未知密钥，逐密钥训练 | 删除key addition，全零key | 单固定未知密钥，seed0--4与seed6已完成，seed7运行中 |
...
shuffle。seed7本地readiness已通过，远程`131072/65536`正式任务正在A6000运行；当前没有正式AUC，
不能把运行状态写成归因正结果。
...
P重排插入非饱和ResCNN并不能超过普通ResCNN或wrong-P。因此不能把精确拓扑写成已验证贡献；当前
新方法主张仍以OPA2的整体SPN式架构正结果和OPA3/OPB1/OPC1的归因边界为限。
...
2. OPD1位置绑定head已预注册、通过本地readiness并启动seed7正式五行矩阵；等待watcher回收后按
   冻结平均门和至少`4/8`逐bit联合门裁决，不在结果前改模型、位置、数据、epoch或阈值。
3. 继续关闭OPA4/OPA5，不通过错误排列搜索、seed、数据、epoch或网络枚举绕过归因门。
4. OPD1通过只开放全新固定密钥原样确认；hold则保留普通ResCNN并停止位置绑定路线，不开放四轮。
