...
状态：核心全文与联网核验完成 / OPN1输出头审计完成 / OPC1远程运行中 / 精确P-layer创新主张删除
...
吸收。因此未来若OPC1通过，只能把exact/wrong差异解释为前两个阶段间路由的贡献。
...
1. 等待OPC1 verified result branch自动回收，验证冻结协议、OPB1 gate hash和正式视觉门；不从训练
   中间损失推断最终AUC。
2. OPC1通过只开放全新固定密钥原样确认，并把拓扑归因限定到前两个阶段间路由；失败或hold则保留
   ResCNN发现锚点并停止混合路线，后续优先审计位置绑定输出头。
