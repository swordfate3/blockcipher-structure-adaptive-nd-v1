...
状态：核心全文与联网核验完成 / OPC1与OPD1均hold并停止 / 精确P-layer贡献未建立
...
| 维度 | Kimura 2022/2023 | Watanabe 2024 | Singh 2025 | 当前OPA1--OPD1 |
|---|---|---|---|---|
| 密钥协议 | 单固定未知密钥，逐密钥训练 | 单固定未知密钥，逐密钥训练 | 删除key addition，全零key | 单固定未知密钥，seed0--4、seed6和seed7均已完成 |
...
| 同参数错误拓扑 | 无 | 无 | 无 | OPA3、OPB1、OPC1与OPD1均含冻结错误P控制 |
| 匹配标签打乱 | 核心论文未作为主门 | 未作为主门 | 无 | OPA2、OPB1、OPC1与OPD1均含架构匹配shuffle |
| 当前证据轮数 | PRESENT三轮完整输出论文锚点 | 非PRESENT主结果 | 去密钥31轮链式仿真 | 三轮selected-bit成立；四轮结构化XOR为0/6通过 |
...
`1.0`，所以低秩条件没有恢复精确拓扑归因。OPC1采用另一种机制：以OPA1中非饱和的
位置保持ResCNN为锚点，在10个残差块的`3+3+4`阶段之间插入三次无参数P-layer重排，并比较普通
ResCNN、exact-P、wrong-P和匹配shuffle。前三个网络均为`3,955,904`个参数；正式结果分别为
`0.573572/0.546634/0.545181/0.500163`，exact低于ResCNN且只比wrong高`0.001453`，裁决为hold。

OPN1对该实际实现补充了确定性可识别性审计。OPC1的全局`Linear(252*64,8)`头让每个输出读取全部
64个最终位置；第三个、位于最后残差阶段之后的P重排可通过head列重排完全吸收，identity、exact与
wrong映射的最大数值误差仅`1.99e-13`。前两个路由之后仍存在非线性残差阶段，尚不能用同一推导
吸收。OPC1最终没有出现可归因增益，因此该全局头路线停止；下一假设改为参数匹配的位置绑定head。

OPD1保持相同ResCNN骨干、三次路由、八个输出、数据和100 epochs，只把全局head换成八个独立
`252 -> 64 -> 1`局部head；每个head只读取对应最终位置，使最后P重排不能再被读取全部位置的线性层
吸收。五行正式矩阵同时包含普通全局头ResCNN、无P位置头、exact-P位置头、wrong-P位置头和匹配
shuffle。seed7远程`131072/65536`、100 epochs正式任务已经完成：exact-P位置头平均AUC
`0.999996`，wrong-P位置头`0.999974`，差值仅`+0.000022`，八个bit均未通过wrong-P联合门，
`0/8`通过。位置绑定显著提高可学习性，但正式裁决仍为`hold`，精确连线归因不成立。结果从raw
fallback回收并完成本地完整性验证，不是verified result branch。
...
OPB1进一步证明，删除高维位置嵌入并加入低秩条件瓶颈仍不能让exact-P超过wrong-P。OPC1又证明把
P重排插入非饱和ResCNN并不能超过普通ResCNN或wrong-P；OPD1的位置绑定head虽让exact/wrong都
近乎完美，仍未拉开两者。因此不能把精确拓扑写成已验证贡献；当前新方法主张仍以OPA2的整体SPN式
架构正结果和OPA3/OPB1/OPC1/OPD1的归因边界为限。
...
- 不能把OPC1写成SPN-ResCNN正结果；其真实P候选低于ResCNN且未超过错误P门。
- 不能把OPC1最后一次P重排写成可独立归因的贡献；OPN1证明它可被全局线性head吸收。
- 不能把OPD1 exact-P接近`1.0`写成精确拓扑正结果；wrong-P同样接近`1.0`且`0/8`通过。
...
1. OPC1 verified result branch、hash、协议和视觉门均已通过；全局头混合路线按hold停止。
2. OPD1已完成并按raw fallback验证；exact-P未超过wrong-P门，位置绑定路线按hold停止。
3. 继续关闭OPA4/OPA5，不通过错误排列搜索、seed、数据、epoch或网络枚举绕过归因门。
4. 不做OPD1 fresh-key确认或位置头扩容；八bit全XOR作为独立输出函数先过r3/r4本地控制门。
5. 在最终使用“新方法”措辞前，核验联网检索新增候选的实际落地页和全文，并把任务不同的BiLSTM、
