...
当前没有开放训练。OPA3已经完成并因exact-P没有超过wrong-P而`hold`；按冻结门，OPA4与OPA5均
关闭。下一步是把OPA1/OPA2整体架构正结果、OPA3跨nibble扩散正证据和精确连线负归因写入论文，
并明确四轮八输出尚未运行。若未来重新开放训练，必须来自新的方法级假设和独立预注册，不得通过
另找wrong-P、换seed、改阈值、加数据、加epoch或临时换网络绕过OPA3。
