...
状态：OP9--OPD1、OPE1与OPN1完成 / 位置绑定和简单XOR组合均hold / 四轮远程关闭
...
正式`2^17/2^16`、100 epochs任务已经完成。exact-P位置头平均AUC为`0.999996`，wrong-P位置头为
`0.999974`，两者只差`+0.000022`；八个bit均未超过wrong-P逐bit门，`0/8`通过。正式裁决为`hold`，
位置绑定路线停止。结果由raw fallback回收并完成本地完整性验证，不得写成verified result branch。
...
| OPD1 | 位置绑定head能否恢复exact-P相对四类控制的增益 | seed7固定密钥，五行参数匹配矩阵，`2^17/2^16`、100 epochs | exact `0.999996`、wrong `0.999974`，差`+0.000022`，`0/8`通过，hold；raw fallback已验证 |
...
7. 用OPN1确定性审计证明全局线性head可吸收最后固定路由，并据此提出参数匹配的位置绑定归因设计；
   OPD1显示位置绑定可让三轮八bit近乎完美可学，但exact-P仍未超过wrong-P，精确连线归因不成立。
...
- OPD1的exact-P接近`1.0`就等于位置绑定恢复精确P-layer贡献；wrong-P同样接近`1.0`。
...
正式图已经从verified result branch回收并通过`visual-qa-redraw`；OPD1图则从raw fallback生成并完成
`1920/1280`像素质检，必须在图注中保留fallback来源，不得把它写成verified result branch。
...
全局head吸收。OPD1随后保持PRESENT三轮、八个冻结bit、`2^17/2^16`和100 epochs不变，以普通
ResCNN、无P位置头、wrong-P和shuffle为控制完成正式矩阵。exact-P与wrong-P均近乎完美但未拉开
差距，`0/8`通过，因此位置绑定路线按冻结门停止，不做fresh-key确认或四轮扩容。OPE1随后把同八bit
全部异或为一个三轮真实输出值；`4096/4096`、10 epochs诊断中直接ResCNN AUC为`0.493085`，低于
MLP、右移同重量控制和标签打乱，三轮校准`0/3`通过。故不运行四轮或远程规模，也不继续枚举XOR mask。
