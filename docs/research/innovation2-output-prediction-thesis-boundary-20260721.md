...
状态：OP9--OP12完成 / 三轮selected-bit方法成立 / 四轮结构化XOR扩轮停止
...
当前不再为这条输出值预测路线分配远程训练预算。下一步是把OP9--OP12整理成论文实验小节，并明确
“三轮selected-bit正结果 + 四轮结构化XOR负边界”的成对结论。只有出现独立文献或确定性密码结构
分析提出新的、预先限定的四轮输出函数机制，才允许新建实验计划；仅换mask、seed、网络名称或扩大
现有预算不能重新打开路线。
