...
状态：OP9--OPA3完成 / OPB1第五密钥正式训练中 / OPC1混合模型readiness通过待命 / 四轮仍关闭
...
| OPB1 | seed4，原SPN锚点/低秩拓扑瓶颈exact-P/wrong-P/shuffle四行，同预算 | 正式训练尚无结果；readiness `32/32`结果、全部协议门通过 | remote running |
| OPC1 | seed6，ResCNN锚点/SPN-ResCNN exact-P/wrong-P/shuffle四行，同预算 | 本地`64/64`、1 epoch readiness的`32/32`结果与全部协议门通过；性能未裁决 | local readiness / standby |
...
seed4同时比较原锚点、exact-P、wrong-P和匹配shuffle。OPB1只在三轮做机制归因；正式结果通过且
再经seed5确认前，四轮仍保持关闭。
...
仅当OPB1有效裁决为低秩瓶颈未归因时才能启动，因此它是待命的新方法假设，不是与OPB1并行选优。
...
当前开放的唯一训练是独立预注册的OPB1三轮低秩拓扑瓶颈正式矩阵，状态为remote running。它不改变
OPA3历史裁决，也不开放OPA4/OPA5。OPB1若同时保持相对原锚点的性能并超过wrong-P/shuffle，下一步
只能在seed5重复同一四行矩阵；若只有归因但性能下降，只允许一次训练隔离的低秩条件修复；若未
归因，停止瓶颈路线并独立排序SPN-ResCNN混合假设。任何分支都不得通过另找wrong-P、后验换bit、
加数据或加epoch绕过门。

SPN-ResCNN混合候选已完成本地readiness，但不是第二个同时运行的远程实验。若OPB1未归因，下一项
正式问题才是：在第七固定密钥、相同`2^17/2^16`总数据与100 epochs下，真实P混合模型是否同时
超过普通ResCNN、错误P和匹配shuffle；若OPB1通过或仅有性能代价，则继续OPB1自己的确认/修复
分支，OPC1保持关闭。
