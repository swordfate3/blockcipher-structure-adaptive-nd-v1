...
状态：OP9--OPB1完成 / OPN1输出头可识别性审计完成 / OPC1正式远程运行中 / 四轮关闭
...
增益，只能归因于前两个路由及其后的非线性残差阶段，不能把最后路由单列为贡献；该审计不提前改变
OPC1 gate或远程任务。
...
并停止混合路线；OPN1已证明最后一次路由可被全局head吸收，后续归因只覆盖前两个阶段间路由。
任何分支都不得通过后验换bit、增加深度、数据或epoch绕过门。
