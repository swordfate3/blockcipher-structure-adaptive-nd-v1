...
打乱下独立确认了整体架构优势。OPA3随后完成同参数归因：exact-P与wrong-P平均AUC均为`1.0`，
identity-P为`0.531989557`，因此精确P-layer没有超过最佳同容量控制，正式状态为`hold`。

本计划是揭盲前条件式预注册的历史记录。OPA3未满足唯一启动权限，所以OPA4没有实现、没有本地
readiness、没有远程包、没有启动，也没有四轮八输出测试结果。以下协议保留用于说明被关闭的假设，
不得当作当前待执行任务。
