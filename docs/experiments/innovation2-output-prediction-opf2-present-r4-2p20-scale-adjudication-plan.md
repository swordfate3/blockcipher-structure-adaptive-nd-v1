...
OPF2已经在远程A6000 GPU0完成，并从验证过的结果分支回收至本地。归档完整性门全部通过：

```text
result rows       = 40/40
history rows      = 500/500
checkpoint hashes = 5/5
SHA256SUMS        = pass
source commit     = 0d2670782f92aa4665b955fcaaf75dceed242f19
```

正式协议保持预注册值：PRESENT-80四轮、seed7固定未知密钥、`2^20=1048576`条总训练明文、
`2^16=65536`条总测试明文、八个预注册真实密文输出bit、100 epochs和五模型矩阵。这里的
`2^20`是训练总行数，不是每类样本；任务是输出值预测，不存在正负样本类别。

| 模型 | 八bit平均AUC |
| --- | ---: |
| 全局头ResCNN | `0.539942700` |
| 无P位置头 | `0.518334510` |
| 真实P位置头 | `0.999967078` |
| 错误P位置头 | `0.999852521` |
| 真实P位置头标签打乱 | `0.498882985` |

真实P位置头八个输出bit全部通过输出预测门，逐bit AUC范围为`0.999933313--0.999999994`，
平均阈值准确率为`0.999898911`，而对应多数类基线平均为`0.501176834`。相对OPF1同一四轮、
同网络、同测试集和同固定密钥的`2^17`训练锚点，平均AUC从`0.513755358`提高到
`0.999967078`，差值为`+0.486211720`。因此正式裁决是：
...
这支持一个有边界的结论：对当前seed7固定未知密钥和八个预注册输出bit，PRESENT四轮并非在
`2^17`训练规模下已经达到结构性上限；增加到`2^20`总训练行后，位置绑定ResCNN可以准确预测
这些真实输出bit。它不是完整64-bit密文恢复、跨密钥确认、五轮结果、样本分类、攻击或SOTA证据。

精确P层拓扑归因仍未通过。真实P与错误P的平均AUC差仅`+0.000114557`，低于预注册的
`+0.020`门槛，要求同时击败错误P的严格逐bit门为`0/8`。所以本实验支持“位置绑定网络在
大规模数据下恢复四轮输出预测”，但不支持“网络识别了精确P层连线”。

正式SVG已按实际像素渲染并通过`visual-qa-redraw`：标题、中文说明、热力图、差值柱图、阈值线、
标签和证据边界均无非预期重叠、裁切、缺字或结构歧义。

## 10. 推荐下一步

按预注册分支，OPF2通过后关闭OPF3，不进入五轮，也不继续机械扩到`2^22`或300 epochs。唯一
下一步是OPF2-C1：保留OPF2的明文划分、模型初始化、标签打乱、batch顺序、八个输出bit、
`2^20/2^16`规模、100 epochs和五模型矩阵，只把固定未知密钥从`key_seed=7`改为
`key_seed=8`。该实验回答“当前四轮恢复是否能在一把全新固定密钥上独立复现”；只有新密钥
确认通过后，才讨论进入五轮或跨密码扩展。
