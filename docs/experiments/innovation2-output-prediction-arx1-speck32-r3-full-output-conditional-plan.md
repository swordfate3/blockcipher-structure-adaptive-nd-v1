...
状态：条件计划冻结 / 论文协议与实现缺口已审计 / 未实现、未训练、未启动
...
plaintext RNG           = numpy.default_rng(1_220_000 + seed)
...
cipher、rounds、两个seed、64-bit key hex、MSB顺序、split、completed rows、RNG state和参数匹配恢复。
...
当前仓库已有正确`Speck32_64`标量实现、官方向量测试及多种SPECK差分/积分工具，但没有本计划需要的：

```text
32-bit固定未知密钥真实输出磁盘数据适配器
Jeong FCNN和两word BiLSTM完整输出模型
rotation/carry-aware共享递推模型及wrong rotation控制
完整32-bit BAPavg/exact-match/逐bitAUC统一runner
独立key_seed和2^20 -> 2^22非连续测试保护
ARX1来源门、SVG和远程包
```
