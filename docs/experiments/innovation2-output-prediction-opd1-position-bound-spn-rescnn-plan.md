...
正式任务运行期间进一步审计了模型初始化、epoch顺序、checkpoint恢复和最终评估路径，结论为当前
协议没有发现会阻止OPD1裁决的不一致：

```text
五模型初始化种子       = 1320000 + seed，每行训练前重置
每轮样本顺序           = 1330000 + seed + epoch
checkpoint绑定          = 完整序列化config + 唯一model_name的SHA-256
checkpoint状态          = epoch / model / optimizer / history
测试集读取              = 仅在全部100 epochs结束后一次
模型选择                = final epoch，不按测试AUC选checkpoint或阈值
```

无P位置头、真实P位置头、错误P位置头和真实P标签打乱使用相同可训练结构；每行重新设置同一初始化种子
与逐epoch采样种子。真实P和错误P的冻结映射不进入可训练状态，因此二者可训练张量初始化逐项相同；
标签打乱行只改变训练目标的确定性排列，测试目标始终是真实密文输出bit。

另用两轮微型CPU矩阵执行了受控中断测试：连续训练两轮，与第一轮保存`latest.pt`后主动中断、再从
同一配置恢复至第二轮相比，history、最终state dict全部张量和八个bit测试结果逐项相同：
