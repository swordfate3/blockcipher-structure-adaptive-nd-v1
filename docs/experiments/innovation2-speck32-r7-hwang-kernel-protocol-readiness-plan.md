...
日期：2026-07-17
状态：协议已核实 / Phase A 完成 / Phase B 远程单 key timing 待实施
...
规模必须由 Phase B 实测 ETA 决定并预注册，最低目标为：

```text
discovery keys >= 32
validation keys >= 32 fresh
6-round exact 9-dimensional Table 7 span
7-round exact 1-dimensional Table 7 span
all discovery directions survive validation
```

若最低64把总密钥仍不可承受，则停止 direct-enumeration reproduction，转向
Midori-64 `2^15` nonlinear VDS 主案例或实现代数/bit-sliced cube-sum引擎；不得用
少量 key 宣称论文 kernel 已复现。
