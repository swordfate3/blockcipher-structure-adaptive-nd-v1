...
OPA3正式结果为exact-P与fixed-wrong-P均`AUC=1.0`、identity-P为`0.531989557`，精确P-layer归因门
未通过。OPA4因此未获授权且从未实现或启动，OPA5的双重启动条件不可能成立。本计划保留为揭盲前的
条件式设计记录，不再是当前实验队列；四block模型也从未实现或训练。

预注册时，当前PRESENT-SPN-aware在三轮任务上恰好使用三个SPN block；OPA4原计划先保持模型不变，
只把数据从三轮改成四轮，以测量未改造架构的轮数边界。若OPA4性能门未通过，OPA5原计划只回答
一个预先冻结的问题：
