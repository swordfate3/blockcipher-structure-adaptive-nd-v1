...
状态：条件计划冻结 / 数据与冻结网络结构已实现并通过单元测试 / 训练与CLI未实现 / 未训练、未启动
...
截至2026-07-23，已在`src/blockcipher_nd/tasks/innovation2/feistel/`子包中分离协议、NumPy轮减DES、
磁盘数据缓存和冻结网络结构，完成固定密钥真实输出数据合同与网络结构合同，但没有生成F1-A/B/C/R正式数据。
定向测试验证了官方16轮向量、1/2/3/16轮NumPy与标量DES逐项一致、MSB-first
序列化和32-bit两半、训练/测试唯一且零重合、key31到key32只改变密钥和真实targets、训练扩容前缀、
参数错配/部分缓存拒绝，以及首个测试chunk中断后逐值恢复。网络测试冻结了FCNN、BiLSTM、正确/错误F输入
共享递推器、公开IP/E/P/FP路由、参数预算和轮共享。matched shuffle训练、训练/裁决、CLI、结果门和远程包
仍未实现；本状态只证明数据与模型结构就绪，不是readiness结果或DES可预测性证据。
...
```text
64 -> 512 -> 1024 -> 512 -> 64
hidden = BatchNorm + ReLU
output = Sigmoid
parameters = 1120320
```

该宽度沿用同论文族SPECK FCNN的公开架构字段；由于正文没有完整消除DES节点数与训练细节歧义，metadata
必须保持`paper_exact_reproduction=false`和`paper-family approximation`，不得事后改称精确复现。
...
揭盲前冻结的实现使用`[batch, 2, 32]`的序列，顺序为序列化明文的左32-bit半块、右32-bit半块；三层
双向LSTM每方向hidden 256，拼接顶层正反向最终hidden后映射到64个Sigmoid输出。参数量为`3780672`。
论文未报告的序列压缩细节继续显式标为近似项。
...
揭盲前冻结的共享F实现先按公开DES E扩展32个输入token为48个，再按八组`6-bit`局部块使用同一个
`6C -> 4C -> 4C` MLP，恢复32个token后执行公开DES P，最后使用共享channel MLP。左右异或由共享
operand mixer近似；二轮和三轮都复用同一F主体与mixer，只增加各轮独立的48位置上下文。

```text
channels = 276
r2 parameters = 3877312  (relative to BiLSTM: +2.56%)
r3 parameters = 3890560  (relative to BiLSTM: +2.91%)
capacity gate = within BiLSTM +/-5%
```

正确与错误F模型使用逐参数相同初始化、相同IP/E/P、相同交换、相同FP和相同64个位置绑定输出头；唯一
结构变量是每轮送入共享F主体的`R_t`或`L_t`。
...
```text
src/blockcipher_nd/tasks/innovation2/feistel/des_output_prediction_models.py
tests/test_innovation2_des_output_prediction_models.py
```

单元测试覆盖真实64-bit输出语义、FCNN/BiLSTM形状、左右半块顺序、公开IP/E/P/FP映射、正确/错误F模型
逐参数匹配、首轮F输入分支、二/三轮共享主体调用次数、二/三轮参数门和协议漂移拒绝：

```text
UV_CACHE_DIR=/tmp/uv-cache uv run pytest \
  tests/test_innovation2_des_output_prediction_models.py -q
13 passed
```

本次没有调用数据生成器或训练器，没有readiness、checkpoint、指标、SVG或可索引实验结果。下一执行动作
仍由第1节条件授权：等待PRESENT、GIFT和SPECK冻结分支依次闭环；条件开放后才实现五行训练/裁决与CLI并
运行`64/64、1 epoch`本地readiness。
