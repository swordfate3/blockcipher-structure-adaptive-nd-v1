...
## [LRN-20260718-005] correction

**Logged**: 2026-07-18T14:00:00+08:00
**Priority**: critical
**Status**: resolved
**Area**: research
...
A public experiment using a cipher's round function is not automatically a master-key-schedule result; independent-round-key positives and negatives transfer asymmetrically.

### Details

Innovation 2 E56 found that the public AlgebraicTransitionMatrices PRESENT
notebook constructs nine PRESENT round functions but adds fresh local 64-bit key
variables at keyed boundaries. It does not instantiate the PRESENT-80 80-bit
master key or its rotation/S-box/counter schedule.

A generalized relation proven constant for every independent round-key
assignment remains constant after restricting those round keys to values
generated by a master-key schedule, provided the round and bit conventions
match. This makes the positive relation potentially reusable after convention
validation. The converse is false for negatives: a relation that varies over
independent round keys may become constant on the smaller scheduled-key image,
and absence from an incomplete positive basis proves neither form of
key-dependence.

### Suggested Action

Every cipher-result import must record the round-key model separately from the
round function. For transfer from independent to scheduled keys, accept positive
universal properties only after bit/round alignment; require concrete scheduled
master-key witnesses for negatives. Never label a run `PRESENT-80` merely
because its nonlinear and permutation layers are PRESENT.

### Metadata

- Source: source_audit, verified_local_experiment
- Related Files: docs/experiments/innovation2-present-r9-generalized-integral-relation-contract-plan.md, src/blockcipher_nd/tasks/innovation2/generalized_integral_relation_contract.py
- Tags: innovation2, present, key-schedule, independent-round-key, generalized-integral, label-transfer
- See Also: LRN-20260718-002
- Pattern-Key: research.cipher_round_function_and_master_key_schedule_are_separate_contracts
- Recurrence-Count: 1
- First-Seen: 2026-07-18
- Last-Seen: 2026-07-18
...
- **Resolved**: 2026-07-18T14:00:00+08:00
- **Commit/PR**: pending
- **Notes**: E56 now reports the independent-round-key model explicitly and routes scheduled-key negatives to a concrete-witness gate.
